...
At this point, we have seen a great many ways that algebraic geometry exerts control over the behavior of homotopy theory, stable and unstable.  The goal in this Case Study is to explore a setting where algebraic geometry is itself tightly controlled: whereas the behavior of formal groups is quite open-ended, the behavior of \emph{abelian varieties} is comparatively extremely strict.  This encourages us to study complex-orientable cohomology theories $E$ which have been tagged with an auxiliary abelian variety $A$ via an isomorphism $\phi\co \CP^\infty_E \cong A^\wedge_0$.  In the case that $A$ is an elliptic curve, this is our definition of an \textit{elliptic cohomology theory}.  The idea, then, is not that this puts serious constraints on the formal group $\CP^\infty_E$ (although it does place some), but rather than the theory of abelian varieties endows $A$, and hence $A^\wedge_0$, with various bits of preferred data on $A^\wedge_0$.  This is the tack we take to construct a \emph{canonical} orientation $MU[6, \infty) \to E$: for any complex-orientable $E$, we identify the collection of such ring maps with ``$\theta$--structures on $\CP^\infty_E$'', and a basic theorem about abelian varieties endows the elliptic curve $A$ with a canonical such structure, altogether yielding the desired orientation.

Making the identification of $MU[6, \infty)$--orientations with $\theta$--structures requires real work, but many of the stepping stones are now in place.  We begin with a technical section about especially nice formal schemes, called \textit{coalgebraic}, and we use this to finally give the proof, announced back in \Cref{DivConstructionsAreFree}, that the scheme of stable Weil divisors on a formal curve presents the free formal group on that curve.  With that out of the way and with $MU[6, \infty)$ in mind as the eventual goal, we then summarize the behavior of the part of the Postnikov tower for complex bordism that we \emph{do} understand---the cases of $MUP$ and $MU$---and use this to make an analysis of $MSU$.  In particular, we rely heavily on the results developed in \Cref{UnstableCooperationsChapter} to access the co/homology of the layers of the Postnikov tower and on the results from the two Case Studies preceding that to understand the co/homological behaviors of $BU \times \Z$, $BU$, $MUP$, and $MU$.

The coherence of all of these statements gives us target theorems to aim for in our study of $MU[6, \infty)$, but we are required to approach them from a different vantage point: whereas we can prove a splitting principle for $SU$--bundles, the analogous statement for $U[5, \infty)$--bundles does not appear to admit a direct proof.  Consequently, the proofs of the other structure theorems for $BU[6, \infty)$ and $MU[6, \infty)$ are made considerably more complicated because we have to work with our splitting principle hands tied.  When the dust of this settles, we will have arrived at a very satisfying and complete theory of $MU[6, \infty)$--orientations.  The reader gifted with an exceptional attention span will recall from \Cref{IntroductionSection} that we were \emph{really} interested in orientations of the type \[M\String \to E,\] especially in the case of $E = K^{\Tate}$, a particular cohomology theory called \textit{Tate $K$--theory}.  We close this Case Study with an analysis of this last setting, where we finally give into placing more hypotheses on $E$, in order to gain calculational access to the non-complex-orientable setting of M$\String$---something not generally available to arbitrary $E$.
