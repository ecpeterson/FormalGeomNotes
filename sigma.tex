...
\begin{remark}[{\cite[Remark 2.32]{AHSTheoremOfTheCube}}]
...
The next case in our project is $\OS{kU}{6} = BU[6, \infty)$, and its main wrinkle is that there is not an accessible splitting principle, whether topological or algebraic.  This not only makes the study of $BU[6, \infty)$--lifted bundles harder, but it also makes proving the existence of the symmetric cube $C_3 \G$ harder, as there is no model to work from in the ``inverse'' step.  Nonetheless, we will find that there is a $BU[6, \infty)$--analogue of \Cref{BUZTriumvirate}, \Cref{BUTriumvirate}, and \Cref{BSUTriumvirate}.
