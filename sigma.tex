...
\begin{align*}
E_\Lambda & \to C, \\
z \mod \Lambda & \mapsto (1, \wp_\Lambda(z), \wp_\Lambda'(z)).
\end{align*}
This is sometimes referred to as the Weierstrass presentation of $E_\Lambda$.

\textbf{Lead into this in a smarter way.} Another special function of interest is $\zeta_\Lambda$ with $\zeta_\Lambda' = -\wp_\Lambda$.  It takes the form \[\zeta_\Lambda(z) = \frac{1}{z} + \sum_{\omega \in \Lambda \setminus \{0\}} \frac{1}{z - \omega} + \frac{1}{\omega} + \frac{z}{\omega^2}.\]  It is itself the logarithmic derivative of another special function $\sigma_\Lambda$: \[\zeta_\Lambda = \frac{\sigma_\Lambda'}{\sigma_\Lambda} = \frac{d}{dz} \log \sigma_\Lambda,\] and any elliptic function can be written in the form $c \cdot \prod_{i=1}^n \frac{\sigma(z - a_i)}{\sigma(z - b_i)}$.

With logarithms running around, one might be led to consider the exponential setting.  The normalized exponential function \[\C \xrightarrow{z \mapsto \exp(2 \pi i z)} \C^\times\] already has $\Z$ as its kernel.  Picking a basis for the lattice $\Lambda$, one basis vector can be rescaled to $1$ and the other rescaled basis vector we write as $\tau$.  There is, again, an analytic isomorphism \[E_\Lambda \xrightarrow{z \mapsto \exp(2 \pi i z)} \C^\times / q,\] where $q = \exp(2 \pi i \tau)$ is the image of the other lattice vector.  In fact, $\tau$ can be chosen \textbf{This is just in the upper half-plane, right?} so that $q$ satisfies the norm inequality $0 < |q| < 1$.
...
Exponentiate and consider the $\theta$--function, as well as its translates.  Note both the projective embedding (which is distinct!) and the relation to Weierstrass's $\zeta$ function.
...
\todo{Be sure to mention Weil pairings today as well as $\Theta^k$--structures, since you'll need both tomorrow.}
