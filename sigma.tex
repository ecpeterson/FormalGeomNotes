...
\begin{definition}
Let $C$ be a coalgebra over a field $k$.  The \emph{scheme $\Sch C$ associated to $C$} is defined by \[(\Sch C)(T) = \left\{f \in C \otimes T \middle| \begin{array}{c} \Delta f = f \otimes f \in (C \otimes T) \otimes_T (C \otimes T), \\ \eps u = 1 \end{array} \right\}.\]
\end{definition}
...
This Theorem gives us access to many constructions on formal schemes, provided we assume that the input is coalgebraic.  This covers many of the cases of interest to us, as every formal variety is coalgebra\todo{I guess you would prove this by first d\'{e}vissaging to the case of $\A^n$.  How does this work?  Then you'd just need to prove it for $\A^n$ -- Jay asked you how to do this for $\A^1$, and I think you should include the argument here.} For an example of the sort of constructions that become available, one can prove the following Corollary by analyzing the symmetric power of coalgebras:
...
\end{tikzcd}
\end{center}
This is identical information to what they call \textit{the adjoint map} of $E_*$--modules \[E_* \to E_* Y \widehat\otimes_{E_*} E^* X,\] \todo{What is $\widehat\otimes$?  How do you get the adjoint map from $f$?} which is determined by where it sends $1 \in E_*$.  This construction converts many properties of $f$ into corresponding properties of this adjoint element.\todo{Expand on this, perhaps.}
\end{remark}
...
\begin{remark}
The map $BU \to BU \times \Z$ induces a map $MU \to MUP$, and hence 
\end{remark}
...
\begin{align*}
(\L_1-1)\sheaf H(\L_2 - 1) & = (\L_1-1)(\sheaf H \L_2 - \sheaf H)
& = (\L_1 - 1)(\sheaf H \L_2 - 1) - (\L_1 - 1)(\sheaf H - 1) \\
(\sheaf H \L_1 - \sheaf H)(\L_2 - 1) & = \\
...
\begin{theorem}[{Ando--Hopkins--Strickland, unpublished}]\citeme{This is Prop 3.2 of the AHS preprint or Prop 2.13 of Strickland's FSKS preprint}
The scheme $\SDiv_0 \G$ is a model for $C_2 \G$.
\end{theorem}
\begin{proof}[Proof sketch]
Consider the map
\begin{align*}
...
\todo{Talk about $MSU$!  Talk about $\sheaf I(0)$ versus $\sheaf O$.  Also talk about how choosing a trivialization gives a comparison between the twisted thing and the untwisted thing.}
...
\begin{remark}\todo[inline]{Actually work this out.}
