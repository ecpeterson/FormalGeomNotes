...
There is a commutative square\todo{Where did $\delta$ come from? A previous day? Talk about this some, probably.}
\begin{center}
\begin{tikzcd}
...
Now, we turn to the algebra.

$\sheaf O(C^2(\G_a; \Gm)) \cong \Z_p[a_d \mid d \ge 2]$

$\sheaf O(C^3(\G_a; \Gm)) \cong \Z_p[a_d \mid d \ge 3, d \ne 1 + p^t] \otimes \Gamma[b_d \mid d \ge 3, d = 1 + p^t]$
...
\begin{center}
\begin{tikzcd}
\Spec HP_0 BSU \arrow{r} \arrow{d}{\cong} & \Spec HP_0 BU[6, \infty) \arrow{r} \arrow{d} & \Spec HP_0 \OS{H\Z}{3} \arrow{r} \arrow{d}{\cong} & 0 \\
C^2(\G_a; \Gm) \arrow{r}{\delta} & C^3(\G_a; \Gm) \arrow{r}{e} & \InternalHom{FormalGroups}(\G_a^{\wedge 2}, \Gm) \arrow{r} & 0.
\end{tikzcd}
\end{center}

The main point is that we don't actually have to compute much about the middle map.  Because the squares commute and the sequences are exact as indicated, we at least learn that $\hat \Pi_3$ is an epimorphism.  But, since both source and target are affine schemes of graded finite type with equal Poincar\'e series, an epimorphism is an isomorphism.





We can also address the cohomological point of view.
\begin{corollary}\citeme{4.41 in the AHS preprint}
The scheme $C_3 \G_a$ exists, and it is modeled by $BU[6, \infty)_{H\Z P}$.
\end{corollary}
\begin{proof}
\todo{Explicit calculation of the Verschiebung + Milnor--Moore}
\end{proof}

\begin{corollary}\citeme{``Representability'' Section 3 in the AHS preprint}
...
\section{Orientations from $\Theta$--structures on Thom sheaves}
...
There's also Lemma 2.49 and Theorem 2.50 in the real version.
