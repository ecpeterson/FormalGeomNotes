...
The $\theta$--functions version of the story has two main successes.  One is that there is a version of this story for an arbitrary abelian variety\citeme{Milne's abelian varieties, Theorem 7.1}.  It turns out that all abelian varieties are projective, and the theorems sitting at the heart of this claim are
...
\end{corollary}
\noindent However, the proof of projectivity arising from this method rests on choosing an ample line bundle on $A$ and using some generating global sections to embed into $\P(\L^{\oplus n})$.  A choice of ``$\theta$--structure'' on $A$, which is only slightly more data, gives a canonical identification of $\P(\L^{\oplus n})$ with a \emph{fixed} projective space.
