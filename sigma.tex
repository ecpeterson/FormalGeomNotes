...
The effect of Cartier duality on the \index{Dieudonn\'e module}Dieudonn\'e module of a formal group is \emph{also} described by linear duality.  Hence, the covariant and contravariant Dieudonn\'e modules described in \Cref{SectionDieudonneModules} can be taken to be related by Cartier duality.
\end{remark}
...
The Postnikov fibration\index{Postnikov tower} \[BSU \to BU \xrightarrow{B\det} BU(1)\] induces a short exact sequence of Hopf algebras
\[\pushQED{\qed}
E^0 BSU \from E^0 BU \xleftarrow{c_1 \mapsfrom c_1} E^0 BU(1). \qedhere
\popQED\]
\end{lemma}

\noindent An equivalent statement is that there is a short exact sequence of formal group schemes\todo{Is this in Lecture 2.3? It should be.}
\begin{center}
\begin{tikzcd}
...
\begin{remark}
In an algebraic direction, the reader is also advised to consider the short exact sequence \[0 \to I \to \Z[G] \to \Z \to 0\] as a model for the short exact sequence \[0 \to \Div_0 \G \to \Div \G \to \underline{\Z} \to 0.\]  The results we have pursued here (in particular, \Cref{SDivModelsC2}) have direct analogues in the setting of group--rings.
\end{remark}
...
\begin{center}
\begin{tikzcd}
\Spec E_0 BSU \arrow[densely dotted, "\widehat \Pi_2"]{r} \arrow[bend right=10]{rr} & C^2(\CP^\infty_E; \Gm) \arrow{r} & \InternalHom{FormalSchemes}((\CP^\infty_E)^{\times 2}, \Gm)
\end{tikzcd}
\end{center}
...
From the edge homomorphisms in \Cref{HF2BU6Calculation}, we can already see that the sequence of Hopf algebras \[H_* \OS{H\Z}{3} \to H_* BU[6, \infty) \to H_* BSU\] modeling the sequence of formal group schemes \[\text{``$(\OS{H\Z}{3})_{HP}$''} \to BU[6, \infty)_{HP} \to BSU_{HP}\] is neither left-exact nor right-exact.  This seems bleak.
...
The unique \index{symmetric 2 cocycle@symmetric $2$--cocycle!lemma}symmetric additive $2$--cocycle of homogeneous degree $n$ has the form
\[\pushQED{\qed}
...
Take $E$ to be \emph{complex--orientable}.  The functor $C^3(\CP^\infty_E; \sheaf I(0))$ is isomorphic to the affine scheme $\Spec E_0 MU[6, \infty)$.  Moreover, the $E_0$--points of this scheme biject with ring spectrum maps $MU[6, \infty) \to E$.\index{orientation!MU six@$MU[6, \infty)$} \qed
\end{corollary}
...
\todo{Draw an exponential curve in the plan mirroring a meridinal band on the torus.}
\begin{figure}
\begin{center}
...
% http://tex.stackexchange.com/questions/202607/drawing-discs-with-circular-holes-with-tikz
    ...
    \fill (0.5,0.866) circle (0.12) ++(0,-0.32) node {$q$};
    \fill[opacity=0.5] (2,-3.464) circle (0.12) ++(-0.26,0.26) node {$q^{-1}$};
    \fill[opacity=0.5] (-0.433,0.25) circle (0.12) ++(-0.28,0) node {$q^2$};
...
The $\theta$--functions version of the story has two main successes: it continues to make sense in algebraic geometry, without invoking transcendental functions, and in fact there is a version of this story for an \emph{arbitrary} \index{abelian variety}abelian variety.  It turns out that all abelian varieties are projective, and the theorem sitting at the heart of this claim is
\begin{corollary}[{``Theorem of the Cube'', \cite[Corollary I.6.4 and Theorem I.7.1]{Milne}}]\label{Theta3IsTrivial}\index{theorem of the cube}\index{theta structure@$\Theta$--structure}
...
Another location where this series $\psi$ appears is in the theory of \index{p divisible group@$p$--divisible group!Tate}Tate $p$--divisible groups of Ando and Morava (and, in some sense, in the sense of Katz and Mazur\citeme{Find a reference}).  For a formal group $\G$ over $\Spf R$ with group law $+_\phi$, they consider the Weierstrass product \[\Theta_{\phi}(x; q) = x \cdot \prod_{\substack{k \in \Z \\ k \ne 0}} \frac{(x +_\phi [k]_\phi(q))}{[k]_\phi(q)} \in R\ps{x,q},\] which plays the role of a kind of $\theta$--function for $\G$.  They also connect this series the \index{formal group!quotient}quotient function mapping to the group scheme $\G / q^{\Z}$ (in the sense of \Cref{NormForFns}), which they further identify as the \index{formal group!universal constant extension}universal extension of $\G$ by a constant $p$--divisible group of dimension $1$.  In the presense of a global object $\mathbb G$ specializing to $\G$ (as in the case of $\G_a$ or $\G_m$), they show how to modify this product to an analytically convergent power series, recovering the characteristic series for the \index{A hat genus@$\widehat A$--genus}$\widehat A$--genus and the \index{L genus@$L$--genus}$L$--genus.  The higher height analogues of these results seem very mysterious and very interesting.\footnote{Morava is generally full of interesting ideas about genera---see, for instance,~\cite{MoravaMotivicThomIso}.}
\end{remark}
...
For $E$ a complex-orientable ring spectrum with $1/2 \in \pi_0 E$, the functor $\Sigma^3(\CP^\infty_E; \sheaf I(0))$ is isomorphic to the affine scheme $\Spec E_0 M\String$.  Moreover, the $E_0$--points of this scheme biject with ring spectrum maps $M\String \to E$.\index{orientation!String@$\String$} \qed
\end{corollary}
...
\end{tikzcd}
\end{center}
Our program is to analyze the chromatic homology of $\Spin/SU$ as well as the maps to $BU[6, \infty)$ and $BSU$.  We hope to show that the scheme associated to $\Spin/SU$ selects exactly the relations defining $\Sigma$--structures \emph{and} that the map is flat, so that the associated \index{bar spectral sequence}bar spectral sequences
\begin{align*}
...
\end{corollary}
\begin{proof}
The same Serre spectral sequence calculation as in the proof of \Cref{SpinSUComparisonMapIsInj} shows that the source and target of the factorization have the same \index{Poincar\'e series}Poincar\'e series, and we are done.
\end{proof}
