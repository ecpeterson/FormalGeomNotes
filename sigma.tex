...
It is unfortunate, then, that when working over a ring rather than a field \Cref{kCoalgebrasAreIndFinite} fails.\citeme{Include (a reference to) an example.}  Nonetheless, it is possible to bake into the definitions the machinery needed to get a good-enough analogue of \Cref{CoalgsAndFSchsAgreeOverk}.
...
\{\text{triv\textsuperscript{ns} of $\ThomSheaf{1 - \L} \downarrow \CP^\infty_E$}\} \times \{\text{triv\textsuperscript{ns} of $1 \downarrow \CP^\infty_E$}\} \arrow{r} & \{\text{triv\textsuperscript{ns} of $\ThomSheaf{1 - \L} \otimes 1 \downarrow \CP^\infty_E$}\}.
\end{tikzcd}
\end{center}
...
\todo{Should this $(\CP^\infty_E)^{\wedge 2}$ have a $[p^\infty]$?}
\begin{center}
\begin{tikzcd}
...
\todo{It is not good to name the space $B_j$ and these maps $B_1$, $B_2$.}
\begin{align*}
...
\end{tikzcd}
\end{center}
where $y$ is the standard Euler class in $H^2 \CP^\infty$ and the first block commutes because the bottom row is the stabilization of the top row; $w$ is the Thom class associated to $T(\L^{\otimes p^j})$ and the middle block commutes because it witnesses the $H\Z$--analogue of the statements expressed by \Cref{MUConvertsTorsionPointsToTorsionPoints} and \Cref{KtheoryConvertsTorsionToTorsion}; and the last block commutes because the other two do.\textbf{Also need some shifted mapping group to be zero?}  In particular, an application of the right-most block gives}
\beta \mu (a \sm a\delta) & = \beta \mu (a \sm \rho w). \\
\intertext{Using the fact that $\beta$ is the cofiber of the ring map $\rho$, there is a juggle \textbf{This looks like a $V$/$F$ relation.  Can this be made explicit?}}
...
\todo{It is not good that the Bockstein is $\beta$ and the Bott element is $\beta$.}
\begin{center}
\begin{tikzcd}
...
\todo[inline,color=red]{This is the wrong order.  You should start by justifying the algebraic model, then show that the algebraic model has the flatness property.  This should amount to calculating the even-periodic cohomology of $\Spin/SU$ in general, then identifying $E_* \Spin/SU$ as particular subs of the even-periodic cohomology of $BU\<2k\>$.}
We embark on this project by analyzing the algebraic model and, in particular, ensuring that it has the desired flatness property.  Our models for the maps
\begin{align*}
BSU & \from \Spin/SU, &
BU[6, \infty) & \from \Spin/SU
\end{align*}
will be
\begin{align*}
C^2(\G; \Gm) & \to \Sigma^2(\G; \Gm), &
C^3(\G; \Gm) & \to \Sigma^2(\G; \Gm), \\
f(x, y) & \mapsto \frac{f(x, y)}{f(-x, -y)}, &
f(x, y, z) & \mapsto f(x, y, -x-y).
\end{align*}

Our main tool for addressing flatness is the following theorem, strongly related to the Milnor--Moore classification of Hopf algebras over a field of positive characteristic:
...
\begin{enumerate}
    ...
    \item A map of affine algebraic group schemes is flat if and only if the induced map on tangent spaces is surjective.  Although $E_0 BSU$ is \emph{infinite} polynomial, and hence not algebraic, this technicality can be overcome and the same condition applies.
\end{enumerate}

We thus set about studying the points of $\Sigma^2(\G; \Gm)$.  As in \Cref{SectionBU6AtInfiniteHeight}, we begin with the special case $\G = \G_a$ and understand the generic case in terms of a tangent space calculation.

\begin{lemma}
The map \[\Spec \Gamma_{\Z_{(2)}}[a_{2n + 1} \mid n \ge 1] \to \Sigma^2(\G_a; \Gm) \times \Spec \Z_{(2)}\] classifying the product \[\prod_{n \ge 1} \exp(a_{2n+1} c_{2n+1})\] is an isomorphism.  In turn, there is an isomorphism \[\Sigma^2(\G_a; \Gm) \times \Spec \F_2 \cong \Spec \left.\F_2\left[a_{2n+1}^{[2^j]} \middle| n \ge 1, j \ge 0\right] \middle/ \left(\left(a_{2n+1}^{[2^j]}\right)^2 = 0\right)\right. . \qed\]
\end{lemma}

In general, after picking a coordinate on $\G$ any function $f \in \Sigma^2(\G; \Gm)$ must thus have the form $1 + a c_{2n+1} + \cdots$ for some coefficient $a$.  There are three calculations:
\begin{lemma}\label{SigmaNonexistenceLemma}
...
\end{lemma}
\begin{proof}
\todo[color=red,inline]{This cuts down the $\Sigma^2$ space.}
\end{proof}
...
\end{lemma}
\begin{proof}[Proof sketch]
This requires calculation in a coordinate.  Since $\G$ is of height $d$, it admits a coordinate where the negation series takes the form \[[-1]_{\G}(x) = -x + cx^{2^d} + \cdots.\]  Substituting this into the definition of $\lambda_2$ and tracking the computation gives the result.
\end{proof}
...
\end{corollary}
\begin{proof}
\todo[inline]{The cubical structure had something to do with the Abel's Theorem method of constructing the elliptic curve multiplication.}
\end{proof}
...
more kitchloo-laures-wilson results---maybe just a remark?

remark: Compare with Neil's response at \texttt{http://mathoverflow.net/questions/123958/a-formal-group-law-over-oriented-bordism}.

remark: the $K$--theory results are supposed to entail results about general elliptic cohomology theories
...
\end{tikzcd}
\end{center}
or, equivalently, an equivalence \[M\String \simeq MU[6, \infty) \sm_{\Susp^\infty_+ \Spin/SU} \S.\]
\todo{This approach is \emph{not} independent of all the previous fuss with $B\Spin$.  We still need all of that to conclude that $B\Spin \to BU[6, \infty)$ induces an inclusion of formal schemes, which identifies the formal scheme associated to $\Spin/SU$ with $\ker \tau$.}
\todo{There's a gap here. I think that we learn that $\Spin/SU \to BU[6, \infty)$ is an inclusion, and showing that that includes to the right thing along the zero-section is an additional argument.}
This in turn gives a $\Tor$ spectral sequence of signature \[\Tor^{K_* \Spin/SU}_{*, *}(K_* MU[6, \infty), K_*) \Rightarrow K_* M\String.\]  So, under the flatness hypothesis above, there are no higher $\Tor$ terms so the spectral sequence collapses to give \[K_* M\String \cong K_* MU[6, \infty) \mmod K_* \Spin/SU.\]  So, what remains to be shown is that $K_* \Spin/SU$ picks out the correct extra relation for $\Sigma$--structures \todo{You have not introduced $\Sigma$--structures yet.}.  Then, we need a density argument to show that this handles all of the at-a-point cases of elliptic cohomology.
...
I've been looking at this again, and I think $B\String$ can be successfully analyzed at low heights using the $B\Spin$ analysis, which is height-independent, and the following diagram of fibrations:
...
KLW show that these fibrations induce short exact sequences of group schemes at heights bounded by $2$.  This identifies $\Spin/SU_K \to BSU_K$ with the inclusion of $\ker \tau$ and $\Spin/SU_K \to BU[6, \infty)_K$ with the inclusion of $\ker(\tau \delta) = \ker(1 + \xi)$, using the KLW facts that $B\Spin_K$ and $B\String_K$ themselves inject compatibly into $BU[6, \infty)_K$ (see e.g.\ 2.3.6 of KLW).  The trick is then to identify these kernel subgroups with image subgroups of different maps---we want to do this because kernel subgroups do not (seem to?) describe testable conditions for descending maps, whereas if we knew (for instance) $\ker(1 + \xi) = (\im (1 - \xi)) + (\im \tau)$, then we could show that a function in $C^3(\G; \Gm)$ descends to a function on $B\String_K$ if and only if it vanishes on points in the images of these maps, themselves easily described since $C_2$ and $C_3$ are fppf-locally generated by point-divisors.

Another fact from the diagram is that the inclusion of $\ker(1 + \xi)$ into $BU[6, \infty)_K$ remains an inclusion after projection along $\delta$.  Another fact is that there is a sequence of inclusions \[\im(1 - \xi) \subseteq \ker(1 + \xi) \supseteq 2 \cdot \ker(1 + \xi) \subseteq \im(1 - \xi).\]  This string is enough to show that the Hopkins condition is \emph{necessary} to get a $\String$--orientation, but perhaps not sufficient due to some $2$--torsion points in $C_3$.  Another fact KLW show is that $(\OS{BSO}{2})_K$ models $\im \tau$.  This doesn't seem compatible with the identification of $\Spin/SU$ as $\OS{kO}{6}$.  Another fact is that I \emph{think} that $\tau$ is not a $kU^0$--linear map.  I wonder if we can use divisibility to gain some traction: away from $2$ multiplication by $2$ is an isomorphism and at $2$ multiplication by $2$ is an fppf-surjection, so we can assume any given sum in $\ker(1 + \xi)$ admit componentwise square roots...  KLW also show that $B\Spin_K$ can be modeled by $\ker \tau$ (and this is the same place where they identify $BSO_2$ as $\im \tau$).  A routinely useful computational trick seems to be $[x] = -\<x\>[-x]$.
...
FPFP Example 12.10 claims applications to $BU[2k, \infty)$, but I can't tell what they are.

Mention all this work of Gerd.  He has a bunch of papers that are all clustered around these topics.
