...
\section{Thom sheaves and duality}

\begin{definition}
Let $E$ be a complex-orientable theory and let $V \to X$ be a complex vector bundle over a space $X$.  The cohomology of the Thom space $E^* T(V)$ forms a $1$--dimensional $E^* X$--module, and we denote the associated line bundle over $X_E$ by $\ThomSheaf{V}$.
\end{definition}

\todo{This construction is natural for pullbacks of vector bundles, and it takes sums of bundles to tensor products of sheaves.}
...
\begin{example}
\todo[inline]{The line bundle associated to the reduced tautological bundle on $\CP^\infty$ is the the sheaf of functions vanishing to first order at the origin.}
\end{example}

\todo{Relationship between $\ThomSheaf{V}$ and the divisor inspired by $V$.}

\todo{Euler classes and Thom classes}

------

\textbf{I'm going to retype a bunch of stuff from Section 8 of the $H_\infty$ AHS paper.  It's a pretty nice sequence of examples.  Not all of it (i.e., the level structure stuff at the end) belongs right here and right now.}
...
Here is a series of examples that lead to a fairly complete understanding of the functor $\mathbb L$:
\begin{enumerate}
...
\item Suppose that $V$ is a line bundle over $X$, classified by a map $b \co X \to \CP^\infty$.  In terms of algebraic geometry, the map $b$ defines an $X_E$--valued point $b = b_E$ of $\G$.  It follows that \[\ThomSheaf{V} \cong b^* \sheaf I(0) \cong 0^* \sheaf I(-b).\]
...
Is it possible to give an example of a complex-oriented theory which receives an $MSU$ orientation which \emph{does not} factor the complex orientation but \emph{does} (\emph{must}, really) factor the unit?  This must have to do with the $\tau$ map, in order to check that the map isn't $\delta$ of something...?
