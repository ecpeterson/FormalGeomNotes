...
\begin{theorem}\citeme{Theorem 3.8 of RW}
For $E$ and $G$ sufficiently nice, there is an equation \[b(s) +_{[G]} b(t) = b(s +_E t),\] where
\begin{align*}
b(s) & = \sum_i b_i x^i, \\
b(s +_E t) & = \sum_n b_n \left(\sum_{i, j} a_{ij}^E s^i t^j \right)^n, \\
b(s) +_{[G]} b(t) & = \bigast_{i, j} [a_{ij}^G] \circ \left( \sum_k b_k s^k \right)^{\circ i} \circ \left( \sum_\ell b_\ell t^\ell \right)^{\circ j}. \qed
\end{align*}
\end{theorem}

Calculation of $H\F_{p*} \OS{MU}{2*}$\citeme{Pages 266--270 of Ravenel--Wilson, especially the bottom of 268.}

Calculation of $H\Z_* \OS{MU}{2*}$: no torsion, since $H\F_{p*} \OS{MU}{2*}$ is even, and since the free map is an isomorphism for all primes it's an isomorphism integrally

Since $H\Z_* \OS{MU}{2*}$ is $\Z$--free and even, $MU_* \OS{MU}{2*}$ is $MU_*$--free and even.  It follows that since the free Hopf ring map to $H\Z_* \OS{MU}{2*}$ is an isomorphism, so is the one tensored up to $MU_*$.
