...
\end{tikzcd}
\right\}.\]
Its totalization gives the \index{completion!unstable}\textit{unstable \(E\)--nilpotent completion of \(X\)}, and the associated filtration spectral sequence on homotopy is the \index{Adams spectral sequence!unstable}\textit{unstable Adams spectral sequence}.\footnote{The curious reader can find unstable analogues of \Cref{HF2ASSFigure} in \cite{BPS}.}  The simplicial scheme \[\Ucontext{E} = \Spec \pi_0 \sheaf{UD}_E(S^0)\] forms the \index{context!unstable}\textit{unstable context} for \(E\), and a space \(X\) gives rise to a quasicoherent sheaf over \(\Ucontext{E}\) by: \[\Ucontext{E}(X) = (\pi_0 \sheaf{UD}_E(X))\widetilde{\quad}.\]
\end{definition}
...
Let \(E\) satisfy {\UFH}.  The associated Hopf ring \(E_0 \OS{E}{0}\) gives rise to a comonad \(G(M) = M \otimes_{E_0} E_0 \OS{E}{0}\), and comodules for the Hopf ring are defined to be \(G\)--coalgebras.\index{comodule!coalgebra}
\end{definition}
...
\end{corollary}
\begin{proof}
We approach via the system \[\HFtwo_* \HFtwo \cong \colim\left( \cdots \to \HFtwo_{*+j} \OS{\HFtwo}{j} \to \HFtwo_{*+(j+1)} \OS{\HFtwo}{j+1} \to \cdots \right).\]  Each suspension map factors through projection to \(\ast\)--indecomposables~\cite[Corollary 2.18]{BJW}, and hence we may replace this system with \[\HFtwo_* \HFtwo \cong \colim\left( \cdots \to Q^* \HFtwo_{*+j} \OS{\HFtwo}{j} \to Q^* \HFtwo_{*+(j+1)} \OS{\HFtwo}{j+1} \to \cdots \right).\]  \Cref{CalculationOfUnstableSteenrodHopfRing} gives us unabashed access to the \(\ast\)--indecomposables:
\begin{align*}
...
\end{align*}
where we have translated to our previous notation by writing \(\xi_j\) for \(a_{(j)}\) and juxtaposition for \(\circ\)--product.  Finally, the maps in this system correspond to iteratively taking the \(\circ\)--product with the homology suspension element \(\alpha_{(0)} = \xi_0\), which has the effect of inverting it.  The final assertion that suspension has no stable effect yields the claim.\footnote{As a reminder, the graded unstable element \(e \in H_1 S^1\) contributes to stable degree \(1 - 1 = 0\).}
\end{proof}
...
Towards the second, we have already described an interesting embedding of \(\CatOf{Algebras}_{k/}\) into \(\CatOf{HopfRings}\), \(R \mapsto k[R]\), analogous to the group-ring construction embedding groups into Hopf algebras.  Note that we can describe the arithmetic in its ring of \(\ast\)--indecomposables:
