...
Let $E_*^R \OS{G}{*}$ be the free Hopf ring under $E_*[G^*]$ generated by $C = E_* \CP^\infty$ together with the relation\todo{Talk about where the relation comes from: you use the orienting class $\CP^\infty \to \OS{E}{2}$ and use it to build a pushforward in $F$--homology.} \[b(s) +_{[G]} b(t) = b(s +_E t),\] where $b(s) = \sum_i b_i x^i$ and hence
\begin{align*}
