...
R[S] \otimes_R R[S] & \xrightarrow{\circ} & [s] \circ [s'] & = [s \cdot s'], \\
...
\beta(s) +_{[E]} \beta(t) & = \bigast_{i, j} [a_{ij}^E] \circ \left( \sum_k \beta_k s^k \right)^{\circ i} \circ \left( \sum_\ell \beta_\ell t^\ell \right)^{\circ j}.
\end{align*}
\end{theorem}
...
\end{lemma}
\begin{proof}
Modulo $\ast$--decomposables, we can write \[0 \equiv \<s\> \ast \<s'\> = [s + s'] - [s] - [s'] + [0] = \<s + s'\> - \<s\> - \<s'\>.\]  We can also directly calculate \[\<s\> \circ \<s'\> = [s s'] - [0] - [0] + [0] = \<s s'\>. \qedhere\]
\end{proof}
