...
\end{tikzcd}
\right\}.\]
Its totalization gives the \index{completion!unstable}\textit{unstable \(E\)--completion of \(X\)}.  The simplicial scheme \[\Ucontext{E} = \Spec \pi_0 \sheaf{UD}_E(S^0)\] forms the \index{context!unstable}\textit{unstable context} for \(E\), and a space \(X\) gives rise to a quasicoherent sheaf \[\Ucontext{E}(X) = (\pi_0 \sheaf{UD}_E(X))\widetilde{\quad}.\]
\end{definition}
...
\begin{definition}
In the setting of unstable homology cooperations, we define an \index{operations!unstable}\textit{additive unstable operation} to be one which lies in the \index{Hopf ring!star indecomposables@\(\ast\)--indecomposables}\(\ast\)--indecomposable quotient \(Q^* E_* \OS{E}{j}\).
\end{definition}
...
\begin{enumerate}
	\item Define the Hopf ring spectrum\index{Hopf ring!spectrum, \(\SpH\)} of a Hopf ring \(H\) by the formula \[(\SpH H)(T) = \CatOf{HopfRings}(H, T).\]  What recognizable functor \(\SpH \AA(F, E)\) does the Hopf ring \(\AA(F, E)\) corepresent?
...
\begin{definition}
The \index{Kahler differentials@K\"ahler differentials!cohomologically invariant}\textit{cohomologically translation invariant differentials} is the \(A\)--submodule \(PH^1_{dR}(\G/A) \subseteq H^1_{dR}(\G/A)\) defined as the kernel of \(\mu^* - \pi_1^* - \pi_2^*\).\footnote{The symbol ``$P$'' here denotes the primitives.  Using the identity $\mu^* = \Delta$, the equation $\Delta(x) = x \otimes 1 + 1 \otimes x$ holds if and only if $x$ lies in the kernel of this operator.}
\end{definition}
