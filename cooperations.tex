...
The odd--primary analogue of this result appears in Wilson's book, where again the bar spectral sequences are collapsing.  The end result is \[\HFp_* \OS{\HFp}{*} \cong \frac{\bigotimes_{I, J} \F_p[e_1 \circ \alpha_I \circ \beta_J, \alpha_I \circ \beta_J]}{(e_1 \circ \alpha_I \circ \beta_J)^{\ast 2} = 0, (\alpha_I \circ \beta_J)^{\ast p} = 0, e_1 \circ e_1 = \beta_1},\] where $e_1 \in (\HFp)_1 \OS{\HFp}{1}$ is the \index{homology suspension}homology suspension element, $\alpha_{(j)} \in (\HFp)_{2p^j} \OS{\HFp}{1}$ are the analogues of the elements considered above, and $\beta_{(j)} \in (\HFp)_{2p^j} \CP^\infty$ are the algebra generators of the Hopf algebra dual of the ring of functions on the formal group $\CP^\infty_{\HFp}$ associated to $\HFp$ by its natural complex orientation.  In particular, the Hopf ring is \emph{free}\index{Hopf ring!free} on these Hopf algebras, subject to the single interesting relation $e_1 \circ e_1 = \beta_{(0)}$, essentially stemming from the equivalence $S^1 \sm S^1 \simeq \CP^1$.
\end{remark}
...
\begin{definition}
In the setting of unstable homology cooperations, we define an \index{operation!unstable!additive}\textit{additive unstable operation} to be one which lies in the \index{Hopf ring!star indecomposables@$\ast$--indecomposables}$\ast$--indecomposable quotient $Q_* E_* \OS{E}{j}$.
\end{definition}
...
\end{align*}
and the computation above presents $\Spec \mathcal A P_0$ as the open subscheme of $\Spec \F_2[\xi_0, \xi_1, \ldots]$ determined by the invertibility of $\xi_0$.\footnote{Note that $Q^* \HFtwo_* \OS{\HFtwo}{*}$ does \emph{not} form a Hopf algebra, essentially because it is missing a version of $\chi$ that inverts $\circ$--multiplication.  It is remarkable that inverting the homology suspension element automatically produces such an antipode.}  Hence, we see that a compatible name for the inertia group in the unstable context for $\HFtwo$ is
\begin{align*}
...
As in the stable case, the Hopf ring associated to a ring spectrum satisfying {\UFH} splits off a factor of $B\N$ that tracks the grading\index{graded ring}, and the passage to the stable context in particular covers the localization $B\N \to B\Gm$.
\end{remark}
...
\begin{enumerate}
    ...
    \item Finally, assume the entire structure of a Hopf $R$--algebra on $A$ and of a Hopf $S$--algebra on $B$.  For each $a', a'' \in A$ and $b \in B$ with diagonal $\Delta b = \sum_j b'_j \otimes b''_j$, we can consider the homomorphism relation \[(a'a'')[b] = \bigast_j (a'[b'_j] \circ a''[b''_j]).\]
\end{enumerate}
...
\begin{definition}
For a ring $R$, the \index{Hopf ring!$ast$-square-zero extension}$\ast$--square--zero Hopf ring $iR$ has underlying abelian group $\Z \oplus R$.  For an element $r \in R$, we write $[r] = (0, r)$ for the corresponding element in $iR$, and in this notation the Hopf ring structure maps are set by the formulas
\begin{align*}
...
\begin{definition}
The \index{invariant differential!cohomologically invariant}\textit{cohomologically translation invariant differentials} is the $A$--submodule $PH^1_{dR}(\G/A) \subseteq H^1_{dR}(\G/A)$ defined as the kernel of $\mu^* - \pi_1^* - \pi_2^*$.
\end{definition}
...
0 \arrow{r} & \left\{\begin{array}{c}\text{integrals} \\ \text{with $A$} \\ \text{coefficients}\end{array}\right\} \arrow{r} \arrow[equal]{d} & \left\{\begin{array}{c}\text{all conceivable integrals} \\ \text{of differentials} \\ \text{defined over $A$}\end{array}\right\} \arrow{r} \arrow[equal]{d} & \left\{\begin{array}{c}\text{missing} \\ \text{integrals}\end{array}\right\} \arrow{r} \arrow[equal]{d} & 0 \\
...
Let $k$ be a perfect field of characteristic $p > 0$, and let $\G_0$ be a ($1$--dimensional) formal group over $k$.  Then, choose a lift $\G$ of $\G_0$ to $\W_p(k)$, and define the \index{Dieudonne module@Dieudonn\'e module!contravariant functor}\textit{(contravariant) Dieudonn\'e module} of $\G_0$ by $D^*(\G_0) := PH^1_{dR}(\G / \W_p(k))$.
\end{definition}
...
The functor $D^*$ determines a contravariant equivalence of categories between smooth $1$--dimensional formal groups $\G_0$ over $k$ of finite $p$--height and \index{Dieudonne module@Dieudonn\'e module}\textit{Dieudonn\'e modules}, which are modules $M$ over the Cartier--Dieudonn\'e ring \[\Cart_p = \left. \W_p(k)\<F,V\> \middle/ \left( \begin{array}{c} FV = VF = p, \\ Fw = w^\phi F, \\ wV = Vw^\phi \end{array} \right) \right.\] which are free and finite rank over $\W_p(k)$.
\end{theorem}
...
w_n(F_m a_*) & = \begin{cases} w_{n/m}(a_*) & \text{if $m \mid n$}, \\ 0 & \text{otherwise}. \end{cases}
\end{align*}
...
We now seek to compare our two Dieudonn\'e functors.  In some sense, the comparison is inspired by an integration pairing: $D_*(\G)$ is populated by curves $\gamma$ on $\G$ and $D^*(\G)$ is populated by $1$--forms $\omega$ on $\G$, which we would like to sew together to form \[D_*(\G) \times D^*(\G) \xrightarrow{(\gamma, \omega) \mapsto \text{``$\operatorname{res}_{z=0} \gamma^* \omega$''}} \W_p(k).\]
\todo[inline]{There is more to say here.  Can you give more intuition about how these two presentations are related, for example from Lie theory?  What curve does a cohomologically left-invariant form get sent to?  Is ``cohomologically-invariant'' analogous to ``$p$-typification'', perhaps along the lines of the ``crystalline''-ness of $H^1_{dR}$?  Can these primitives be furthermore related to the idea that taking primitive cooperations selects the additive ones? // Erick Knight suggested that there should be an alternative definition of covariant Dieudonn\'e theory also expressed by the cohomology of some sheaf, and that this will give rise to the desired duality pairing with values in some Tate-twisted ground object, which is precisely what we are looking for.  I wonder if the right thing to do is to form the bar complex on the de Rham sheaf...! // I talked to Mike about this, and his suggestion was to learn about Cartier's ``logarithmic modules'', which lift the $p$--typical curves on a formal group in positive characteristic to a preferred submodule of curves on a lift to mixed characteristic, and that once these two things ($1$--forms and logarithmic curves) are in the same place, the pairing I want is probably the residue map / the Serre duality pairing.  He half-heartedly suggested Lazard's book, especially VII.3.10 and V.7.3.  (Also on the table of ideas: the Cartier dual is witnessed by the existence of a Poincar\'e bundle, which is a biextension.  Maybe this leads somewhere?)}
...
\item There are three nontrivial points in $\G[2]$: $\alpha$, $\beta$, and $\alpha + \beta$.  Quotienting by them gives three points at order $1/(q+1)$, the first bunch of \index{canonical lift!quasicanonical}\textit{quasicanonical lifts}, which have partial formal $\mathcal O_A$--module structures.
...
\index{Dieudonn\'e module!graded}Let $\CatOf{GradedDMods}$ denote the category of graded abelian groups $M$ equipped with maps $V\co M_{pn} \to M_n$ and $F\co M_n \to M_{pn}$ (where $n$ is even if $p \ne 2$) satisfying
\begin{enumerate}
\item $M_{< 1} = 0$.
...
For $n > 1$ an integer satisfying $n \not\equiv \pm 1 \pmod{2p}$, the \index{Brown--Gitler spectrum}\textit{Brown--Gitler spectrum} $B(n)$ is the spectrum satisfying \[B(n)_n X \cong D_n H_* \Loops^\infty X.\] (As convention, when $n \equiv \pm 1 \pmod{2p}$ we set $B(n) := B(n-1)$, and $B(0) := \S^0$.) \qed
\end{corollary}
...
The naive tensor product $M \otimes N$ of Dieudonn\'e modules $M$ and $N$ receives the structure of a $\W(k)[V]$--module, where $V(x \otimes y) = V(x) \otimes V(y)$.  We define the \index{Dieudonne module@Dieudonn\'e module!tensor product}\textit{tensor product of Dieudonn\'e modules}\footnote{This definition is specialized to $\F_p$, where we don't have to worry about Frobenius semi-linearity.} by \[M \boxtimes N = \left.\frac{\W(k)[F, V]}{(VF = p)} \otimes_{\W(k)[V]} (M \otimes N) \middle/ \left( \begin{array}{c} 1 \otimes Fx \otimes y = F \otimes x \otimes Vy, \\ 1 \otimes x \otimes Fy = F \otimes Vx \otimes y \end{array} \right) \right. .\]
\end{definition}
...
\begin{theorem}[{\cite[Theorem 5.7]{RavenelWilsonKthyOfEMSpaces}, \cite[Proposition 2.4.4]{HopkinsLurie}, cf.\ \Cref{MUConvertsTorsionPointsToTorsionPoints}}]\label{KtheoryConvertsTorsionToTorsion}
There is an isomorphism \[BS^1[p^j]_K \cong BS^1_K[p^j].\]
\end{theorem}
\begin{proof}[Remarks on proof]
We have already proven a very similar theorem as \Cref{MUConvertsTorsionPointsToTorsionPoints}.  The crux of that argument was to show that multiplication by the $p$--series in $MU^* \CP^\infty$ was injective without knowing very much about the ground ring $MU^*$.  Here our task is even easier: any \index{formal group law!p series@$p$--series}$p$--series for $\Gamma$ takes the form $[p](x) = c x^{p^d} + \cdots$ for $c$ a unit.  Such an element is certainly not a zero-divisor.
\end{proof}
