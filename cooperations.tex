...
\end{corollary}
\begin{proof}
This follows from the equivalence \[\CatOf{Algebras}_{R/}(A, B) = \CatOf{HopfRings}_R(R[A], iB).\]  Distinct algebras \(B\) and \(B'\) determine distinct functors \(\CatOf{Algebras}_{R/}(-, B)\) and \(\CatOf{Algebras}_{R/}(-, B')\), and this distinction induces a difference in the functors \(\CatOf{HopfRings}_R(-, iB)\) and \(\CatOf{HopfRings}_R(-, iB')\).
\end{proof}
