...
Remarkably, these unstable contexts also admit algebro-geometric interpretations.  In finding the right language for this, we introduce different subclasses of cooperations (e.g., \textit{additive}), and we are also naturally led to consider \textit{mixed cooperations} (as we did stably in \Cref{OrientationsOnEAndMU}) of the form $E_* \OS{F}{*}$.  The running theme is that when $E$ and $F$ are complex-orientable, there is a natural approximation map\todo{Should this be $E_* \OS{F}{2*}$?  This seems tricky: $K_\Gamma{}_* \OS{H\Z}{q+1}$ shouldn't restrict to even things, but $H\F_p{}_* \OS{BP}{2*}$ probably should.} \[\Spec Q^* E_* \OS{F}{*} \to \InternalHom{FormalGroups}(\CP^\infty_E, \CP^\infty_F)\] which is an isomorphism in every situation of interest.  However, these isomorphisms do not appear to admit uniform proofs\footnote{The best uniform result I can find is due to Butowiez and Turner~\cite[Theorem 3.12]{ButowiezTurner}.}, so we instead investigate the following cases by hand:
\todo{You will probably have to re-reference this list.}
\begin{itemize}
\item (\Cref{UnstableSteenrodCoops}:) For $E = F = H\F_2$, we compute the full unstable dual Steenrod algebra $H\F_2{}_* \OS{H\F_2}{*}$ by means of iterated bar spectral sequences.  We then pass to the additive unstable cooperations, where we show by hand that this presents the homomorphism scheme $\InternalHom{FormalGroups}(\G_a, \G_a)$.  Finally, we pass to the stable additive cooperations, and we check that our results here are compatible with the isomorphism \[\Spec H\F_2{}_* H\F_2 \cong \InternalAut{\G_a}\] presented in \Cref{SteenrodAlgIdentifiedWithAutGa}.
\item (\Cref{COableCoopnsII}:) We next consider the case where $F = MU$ and where $E$ is any complex-orientable theory.  We begin with the case $E = H\F_p$, where we can again approach the problem using iterated bar spectral sequences.  The resulting computation is sufficiently nice that we can use this special case of $E = H\F_p$ to deduce the further case of $E = H\Z_{(p)}$, then $E = H\Z$, then $E = MU$, and then finally $E$ any complex-orientable theory.
\item (\Cref{LEFTCooperations}:) Having been able to vary $E$ as widely as possible in the previous case, we then turn to trying to vary $F$.  This is considerably harder, since the infinite loopspaces $\OS{F}{*}$ associated to $F$ are extremely complicated and vary wildly under even ``small'' changes in $F$.  However, in the special case of $E = H\F_p$, we have an incredibly powerful trick available to us: Dieudonn\'e theory, discussed in \Cref{SectionDieudonneModules}, gives an equivalence of categories \[D_*\co \CatOf{HopfAlgebras}^{> 0, \fin}_{\F_p/} \to \CatOf{GradedDMods},\] which postcomposes with \[\CatOf{Spectra} \xrightarrow{\Loops^\infty} \CatOf{Loopspaces} \xrightarrow{H\F_p{}_*} \CatOf{HopfAlgebra}^{>0, \fin}_{\F_p/} \xrightarrow{D_*} \CatOf{GradedDMods} \subseteq \CatOf{Modules}_{Cart}\] \todo{``Cart'' is a placeholder.} to give a homological functor.  This means that the Dieudonn\'e module associated to an infinite loopspace varies stably with the spectrum underlying the loopspace, which is enough leverage to the settle the case where $F$ is any Landweber--flat theory.
\item (\Cref{CoopnsForMoravaKandHA}:) Finally, we settle one further case not covered by any of our generic hypotheses above: $E = K_\Gamma$ and $F = H\Z/p^j$.  Neither $K_\Gamma$ nor $H\Z/p^j$ is Landweber--flat, but because $K_\Gamma$ is a field spectrum and because the additive group law associated to $H\Z/p^j$ is so simple, we can still perform the requisite iterated bar spectral sequence calculation by hand.
\end{itemize}
...
\begin{lemma}[{\cite[Proposition 2.94]{StricklandFSFG}}]
Let $X$ and $Y$ be schemes over $S = \Spec R$, such that $\sheaf O_X$ forms a finite and free $R$--module.  There is then a mapping scheme $M$, such that points $f \in M(A)$ naturally biject with maps $f\co X \times_S \Spec A \to Y \times_S \Spec A$. \qed
\end{lemma}
...
Given a coaugmented $R$--coalgebra $A$ and an augmented $S$--algebra $B$, we form the \textit{free relative Hopf $R[S]$--ring} $A_{R[S]}[B]$ generated under the Hopf ring operations by symbols $a[b]$ for $a \in A$, $b \in B$, according to the atomic rules
\begin{align*}
...
\Delta(a[b]) & = (\Delta a)([b] \otimes [b]).
\end{align*}
Now assume further that $A$ is a Hopf $R$--algebra and $B$ is a Hopf $S$--algebra, so that the multiplication on $A$ and the comultiplication on $B$ have not yet been used in the formation of $A_{R[S]}[B]$.  We define $A_{R[S]}^{\circ = \Delta} B$ to be the quotient Hopf ring where for each $a', a'' \in A$ and $b \in B$ with $\Delta b = \sum_j b'_j \otimes_{E^*} b''_j$ we enforce the additional relation \[\bigast_j (a'[b'_j] \circ a''[b''_j]) = (a'a'')[b].\]
\end{definition}

\begin{lemma}
There is a natural map \[(F_* \CP^\infty)_{F_*[E^*]}[E^* \CP^\infty] \to F_*(\OS{E}{*}),\] and it factors through the equivalence relation imposed in \Cref{DefnAlgebraicModelOfCoopns}.
\end{lemma}
\begin{proof}
We construct a Kronecker-type pairing \[\<-,-\>\co F_n \CP^\infty \times E^m \CP^\infty \to F_n(\OS{E}{m})\] as follows: given a class $f \in \pi_n F(\CP^\infty)$ and a class $e\co \CP^\infty \to \OS{E}{m}$, we can compose the two to produce an element $e_*(f) \in \pi_n F(\OS{E}{m})$.  Universality thus gives a map of Hopf rings $(F_* \CP^\infty)_{F_*[E^*]}[E^* \CP^\infty] \to F_*(\OS{E}{*})$.  The second assertion follows the duality property of (the above enhanced version of) the Kronecker pairing: the map $\mu\co \CP^\infty \times \CP^\infty \to \CP^\infty$ induces two maps
\begin{align*}
\mu_*\co & (F_* \CP^\infty)^{\otimes_{F_*} 2} \to F_* \CP^\infty, \\
\mu^*\co & E^* \CP^\infty \to (E^* \CP^\infty)^{\otimes_{E_*} 2},
\end{align*}
and we have the identity \[\<\mu_* (a' \otimes a''), b\> = \<a \otimes a'', \mu^* b\>.\]  Rewriting this in the above language reveals exactly the equation \[(a' a'')[b] = \bigast_j (a'[b'_j] \circ a''[b''_j]). \qedhere\]
\end{proof}

The main theme of this Case Study is that this induced map off of the quotient is very often an isomorphism (and, in turn, that the theory of formal group laws also captures everything about the theory of unstable cooperations).  Because we will be carrying this algebraic model around with us, we will pause to give it a name.

\begin{definition}
\todo{This notation is still bad.  So, possibly, is the ``${}^{\circ = \Delta}$'' notation used above.  That one at least suggests that it has something to do with homomorphisms (very vaguely), but this generic name doesn't do that at all.}
For $F$ and $E$ ring spectra, we define their \textit{algebraic Hopf ring} $A(F, E)$ by \[A(F, E) = (F_* \CP^\infty)_{F_*[E^*]}^{\circ = \Delta}[E^* \CP^\infty].\]
\end{definition}

\begin{lemma}[{\cite[Theorem 3.8]{RavenelWilsonHopfRingForMU}, \cite[Theorem 9.7]{Wilson}}]\label{UnstableRWRelation}
After choosing complex orientations of $E$ and $F$, there is a natural morphism of Hopf rings \[A(F, E) \cong \frac{F_*[E^*][F_* \CP^\infty]}{\beta(s +_F t) = \beta(s) +_{[E]} \beta(t)},\] where the equation is of power series and the equality is imposed term-by-term on the Hopf ring.  The formal sum $\beta(s)$ is given by $\beta(s) = \sum_j \beta_j x^j$, where $\beta_j$ is dual to the $j${\th} power of the chosen coordinate in $F^* \CP^\infty$, and the formal group law expressions expand to
\begin{align*}
...
\end{align*}
\end{lemma}
\todo[inline]{I think the stuff written above (i.e., the algebraic model presented here) is a little broken and so you can't actually prove this.  You are supposed to be imposing algebraic and coalgebraic behavior, one of which cuts down the size of $E^* \CP^\infty$ and the other of which imposes these $\beta$--relations.  You seem to be missing the algebraic relations.}
...
Finally, we are able to explain the phenomenon uncovered by computation in \Cref{UnstableContextsSection}, where we passed to the $\ast$--indecomposables to find the classical ring of functions on the endomorphism scheme of $\G_a$.  This arises out of two parallel questions:
\begin{enumerate}
	\item What does the Hopf ring $A(F, E)$ corepresent, considered as a functor on Hopf rings?
	\item How does this functor interact with classical rings?
\end{enumerate}

There is in fact an embedding of $\CatOf{Rings}$ into $\CatOf{HopfRings}$, analogous to the square-zero extension functor sending an abelian group $A$ to the augmented algebra $Z \oplus A$ with trivial multiplication on $A$.

\begin{definition}
For a ring $R$, the $\ast$--square--zero Hopf ring $iR$ has underlying abelian group $\Z \oplus R$, and for an element $r \in R$ we write $[r] = (0, r)$ for the corresponding element in $iR$.  The Hopf ring structure maps are then determined by the formulas
\begin{align*}
...
\begin{corollary}
For complex-orientable $F$ and $E$, there is a natural isomorphism \[\Spec Q^* A(F, E) \cong \InternalHom{FormalGroups}(\CP^\infty_F, \CP^\infty_E).\]
\end{corollary}
\begin{proof}
This is a matter of calculating $Q^* A(F, E)$, which is possible to do coordinate-freely, but it is at least as clear to pick coordinates.  Doing this and using \Cref{ArithmeticInQAst}, we have \[\bigast_{i, j} \left([a_{ij}^E] \circ \left( \sum_k \beta_k s^k \right)^{\circ i} \circ \left( \sum_\ell \beta_\ell t^\ell \right)^{\circ j} \right) \equiv \sum_{i, j} a_{ij}^E \left( \sum_k \beta_k s^k \right)^i \left( \sum_\ell \beta_\ell t^\ell \right)^j \;\text{(in $Q^*$)},\] from which it follows that \[Q^* A(F, E) = \left. (F_* \otimes E_*)[\beta_0, \beta_1, \beta_2, \ldots] \middle/ \left( \beta(s +_F t) = \beta(s) +_E \beta(t) \right) \right. . \qedhere\]
\end{proof}
...
\begin{remark}
In the unmixed case of $E = F$, as we saw in the computational example in \Cref{UnstableContextsSection}, the algebraic Hopf ring $A(E, E)$ picks up an extra diagonal corresponding to the composition of formal group endomorphisms of $\CP^\infty_E$, and the resulting pair $(\Spec E_*, \underline{\operatorname{End}}(\CP^\infty_E))$ forms a category scheme.  These schemes act by pre- and post-composition on the mixed algebraic Hopf ring $\Spec Q^* A(F, E)$, and these actions are compatible with the structure maps in the unstable context $\Ucontext{E \vee F}$ described at the beginning of this Lecture.  This description is also compatible with passing to the stable context $\context{E \vee F}$: it is exactly the inclusion of the simplicial subobject consisting of the formal group isomorphisms and automorphisms.
\end{remark}
...
\section{Mixed and unmixed unstable cooperations in complex bordism}\label{COableCoopnsII}

\todo[inline]{\textbf{\textit{BELOW THIS LINE THERE MAY STILL BE SOME PLACES WHERE $F_*^R \OS{E}{*}$ IS WRITTEN IN PLACE OF $A(F, E)$.  READER BEWARE!}}}
...
\todo{Ravenel and Wilson also tell you how to get a basis of $\ast$--indecomposables, which they call ``allowable'' monomials.  Might be worth mentioning.}
\citeme{Pages 266--270 of Ravenel--Wilson, especially the bottom of 268.}
\todo{This section proves Wilson's Thesis Pt 1.  Maybe we should include a remark talking about the consequences he draws from it in Pt 2, including that all nice $p$--local $H$--spaces are actually infinite loopspaces.}
...
\todo{It is also the case that the $\Z_{(p)}$-- and $\F_p$--cohomology of $\OS{BP}{2k}$ is polynomial, and it's proved in basically the same way.  This is the first half of Wilson's thesis: Theorem 3.3.  As a consequence, you know that the co/homology is \emph{bipolynomial}, which means that it splits as a tensor product of the usual ``singly generated'' bipolynomial Hopf algebras.  This leads into the discussion of the Witt Hopf algebras in the next section.}
\todo{Homology of truncated Brown--Peterson spectra: Wilson's thesis, part 2, Prop 1.7.  The goal of Part 2 is to gain access to the cohomologies of the loopspaces in the truncated Brown--Peterson spectra as modules over the Steenrod algebra, so that they can be split apart into prime factors using Adams spectral sequence arguments.  One of the main consequences of the calculation is that some of the bottommost loopspaces have such small cohomology modules that they themselves must be exactly the prime factors we are looking for.}
...
\todo{We know of a connection between $H_* BU$ and the Witt scheme.  Is there a connection between $E_* MU$ and curves, or $E_* BP$ and $p$--typical curves, which is visible from this perspectve?  Almost definitely!  Also, a connection between curves and divisors: the zero locus of a given curve...  ---  Actually, Remark 18.1 in FPFP says that this is an unhealthy way to think about $H_* BU$.  Very interesting.}
...
\todo{Bousfield's paper \textit{On $\lambda$--rings and the $K$--theory of infinite loopspaces} gives some analogues of the Goerss--Lannes--Morel results but for $p$--adic $K$--theory of connective spectra instead.}
