...
For $F$ and $E$ ring spectra, we define their \textit{algebraic Hopf ring} $\AA(F, E)$ (or \textit{algebraic approximation}) by \[\AA(F, E) = (F_* \CP^\infty)_{F_*[E^*]}^{\circ = \Delta}[E^* \CP^\infty].\]
\end{definition}

\begin{lemma}[{\cite[Theorem 3.8]{RavenelWilsonHopfRingForMU}, \cite[Theorem 9.7]{Wilson}}]\label{UnstableRWRelation}
After choosing complex orientations of $E$ and $F$, there is a natural morphism of Hopf rings \[\AA(F, E) \cong \frac{F_*[E^*][F_* \CP^\infty]}{\beta(s +_F t) = \beta(s) +_{[E]} \beta(t)},\] where the equation is of power series and the equality is imposed term-by-term on the Hopf ring.  The formal sum $\beta(s)$ is given by $\beta(s) = \sum_j \beta_j x^j$, where $\beta_j$ is dual to the $j${\th} power of the chosen coordinate in $F^* \CP^\infty$, and the formal group law expressions expand to
\begin{align*}
...
\begin{enumerate}
	\item What does the Hopf ring $\AA(F, E)$ corepresent, considered as a functor on Hopf rings?
...
\todo[inline]{\textbf{\textit{BELOW THIS LINE THERE MAY STILL BE SOME PLACES WHERE $F_*^R \OS{E}{*}$ IS WRITTEN IN PLACE OF $\AA(F, E)$.  READER BEWARE!}}}
