...
As a consequence, we deduce that there is a special class of Hopf rings where the Hopf ring spectrum behaves like an object in ordinary algebraic geometry: \[(\SpH H)(iA) = \CatOf{HopfRings}(H, iA) = \CatOf{Rings}(Q^* H, A) = (\Spec Q^* H)(A).\]  On this locus, we have a real shot of understanding the behavior of \(\SpH \AA(F, E)\) by analyzing the quotient \(Q^* \AA(F, E)\).  We begin with two helpful results:
...
Finally, we are able to explain the phenomenon uncovered by computation in \Cref{UnstableContextsSection}, where we passed to the \(\ast\)--indecomposables to find the classical ring of functions on the endomorphism scheme of \(\G_a\).
