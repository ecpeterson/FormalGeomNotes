...
If an even-periodic \(E\) satisfies {\UFH} and \(X\) is a space with \(E_* X\) a free \(E_*\)--module, then \(E_0 X\) forms a \(G\)--coalgebra.
\end{lemma}
\begin{proof}
...
In the above case, the cosimplicial object \(\pi_0 \sheaf{UD}_E(X)\) is the cobar resolution for \(G\), and the \(E_2\)--page of the \index{Adams spectral sequence!unstable}unstable descent spectral sequence is presented as
\[\pushQED{\qed}
