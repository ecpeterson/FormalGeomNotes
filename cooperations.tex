...
\todo[inline]{
What all needs to be mentioned in this Lecture:
the mixed unstable {\FH} condition;
the complex-oriented algebraic approximation.
}
...
\begin{definition}\label{DefnAlgebraicModelOfCoopns}
\todo[inline]{We want to define a variation of the free Hopf ring on a coalgebra construction~\cite[Equation 1.17]{RavenelWilsonHopfRingForMU} where two Hopf algebras (over $F_*$ and over $E^*$) are permitted.  Then, we want to impose the relations required to get a multiplicative and diagonal-preserving map.}
\todo[inline]{Come up with a good name for this guy.  He's some kind of mapping scheme.  There's a better name than $F_*^R \OS{E}{*}$!!}
\end{definition}

\begin{lemma}
There is a natural map \[F_*[E^*][F_* \CP^\infty, E^* \CP^\infty] \to F_*(\OS{E}{*}),\] and it factors through the equivalence relation imposed in \Cref{DefnAlgebraicModelOfCoopns}.
\end{lemma}
\begin{proof}
The natural map arises as follows: for a class $f \in \pi_n F(\CP^\infty)$ and a class $e\co \CP^\infty \to \OS{E}{m}$, we can compose the two to produce an element $e_*(f) \in \pi_n F(\OS{E}{m})$.  Universality thus gives a map of Hopf rings $F_*[E^*][F_* \CP^\infty, E^* \CP^\infty] \to F_*(\OS{E}{*})$.

\todo[inline]{Finish the second half of this proof.}
\end{proof}
...
\todo[inline]{This following Lemma could use revision: somehow the real thing we want to do is check that this $\beta$--relation is the only thing left over after performing the algebra-map (but not yet coalgebra-map) relations from above.}
\begin{lemma}[{\cite[Theorem 3.8]{RavenelWilsonHopfRingForMU}, \cite[Theorem 9.7]{Wilson}}]\label{UnstableRWRelation}
After choosing complex orientations of $E$ and $F$, there is a natural morphism of Hopf rings
\todo{This equation is missing a left-hand side because we haven't settled on notation yet.}
\[F_*[E^*][F_* \CP^\infty] / (\beta(s +_F t) = \beta(s) +_{[E]} \beta(t)) \to F_*(\OS{E}{*}),\] where the equation is of power series and the equality is imposed term-by-term on the Hopf ring.  The formal sum $\beta(s)$ is given by $\beta(s) = \sum_j \beta_j x^j$, where $\beta_j$ is dual to the $j${\th} power of the coordinate in $F^* \CP^\infty$, and the formal group law expressions expand to
\begin{align*}
...
\begin{enumerate}
	\item What does the Hopf ring above corepresent, considered as a functor on Hopf rings?
...
\begin{lemma}
There is an adjunction \[\CatOf{Rings}(Q^* S, R) = \CatOf{HopfRings}(S, iR). \qed\]
\end{lemma}

\begin{corollary}
Using $i$ to embed $\CatOf{Rings}$ into $\CatOf{HopfRings}$, the functor corepresented by the algebraic Hopf ring evaluates on classical rings to give \[\CatOf{HopfRings}(???, iR) = \CatOf{Rings}(Q^* ???, R) = \InternalHom{FormalGroups}(\CP^\infty_F, \CP^\infty_E)(R). \qed\]
\end{corollary}
...
Note that the Hopf ring-ring $R[S]$ has a natural augmentation given by $[s] \mapsto 1$, so that $\<s\> = [s] - [0]$ form a generating set of the augmentation ideal.
