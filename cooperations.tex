...
\section{Free Hopf rings and the Ravenel--Wilson relation}
\todo{Change this title.}
\todo{This section is all out of order.  The free things are not described well, and they should be.  The technique used in the course of the Lemma at the end is useful earlier, for describing what happens to the various products on $R[S]$ when passing to the indecomposable quotient --- and hence what the Ravenel--Wilson relation imposes on the indecomposable quotient of the free Hopf ring.  That should be the real thrust of this section, plus a few oddities about the example case we're beginning to work toward.}
...
\begin{definition}\todo{I don't like the upper-$R$ notation.  Having a scheme theoretic description of this object should let us pick a better name.  I'm also unhappy that ``mixed unstable cooperations'' is an achiral name, meaning it doesn't indicate which object is the spectrum and which is the infinite loopspace.}
Let $E_*^R \OS{G}{*}$ be the free Hopf ring under $E_*[G_*]$ generated by $C = E_* \CP^\infty$ together with the relation\todo{Talk about where the relation comes from: you use the orienting class $\CP^\infty \to \OS{E}{2}$ and use it to build a pushforward in $F$--homology.} \[b(s) +_{[G]} b(t) = b(s +_E t),\] where $b(s) = \sum_i b_i x^i$ \todo{What are the $b_i$'s?} and hence
\begin{align*}
...
\end{align*}
The scheme $\Spec Q^* E_*^R \OS{G}{*}$ models \[\Spec Q^* E_*^R \OS{G}{*} \cong \InternalHom{FormalGroups}(\CP^\infty_E, \CP^\infty_G).\]\todo{This part of the ``definition'' should be worked separately.  It requires understanding what $\ast$--indecomposables look like, moving this funny formula into the augmentation ideal, checking that $\ast$ gets converted to $+$, $\circ$ to $\cdot$, and showing that the resulting thing really looks like the scheme of formal group homomorphisms.  None of this is totally trivial; Hood, Allen, and I spent a while trying to pull it apart one afternoon.}
\end{definition}

\begin{lemma}[{\cite[Theorem 3.8]{RavenelWilsonHopfRingForMU}, \cite[Theorem 9.7]{Wilson}}]\label{ComparisonMapInCOUnstableCoopns}
For complex-orientable ring spectra $E$ and $G$, there is a natural map $E_*^R \OS{G}{*} \to E_* \OS{G}{*}$. \qed
\end{lemma}
\todo{It might be nice to describe this map more explicitly as you did in class -- how does $G_*$ and $E_* \mathbb{C}P^\infty$ map into $E_* \OS{G}{*}$?
...
To begin, we will investigate the more modest setting of $E = H\F_p$ and $G = BP$ by calculating $H\F_{p*} \OS{BP}{2*}$.  One might think that this is merely a first guess at a task that seems accomplishable, but we will quickly show that it plays the role of a universal example of this calculation.  In this case, we will show that the comparison map $H\F_p{}_*^R \OS{BP}{*} \to H\F_p{}_* \OS{BP}{2*}$ is an isomorphism.  This is proved by a fairly elaborate counting argument, and as such our first move will be to produce an upper bound for the size of the source Hopf ring.
...
We are aiming for a proof of the following Theorem:
...
This is proved by a fairly elaborate counting argument, and as such our first move will be to produce an upper bound for the size of the source Hopf ring.  (To abbreviate notation, we will write $H$ for $H\F_p$ for the remainder of the lecture.)

\begin{lemma}
As a $\circ$--algebra, \[Q^* H_0^R \OS{BP}{2*} \cong \F_p[[v_n] - [0_{-|v_n|}] \mid n \ge 1],\] where $0_{-|v_n|}$ denotes the null element of $BP^{|v_n|}(*)$.\todo{We used the augmentation sending $[s]$ to $[0] = 1$ here.}
\end{lemma}
\begin{proof}
...
\begin{lemma}[{\cite[Lemma 3.14]{RavenelWilsonHopfRingForMU}}]\todo{Look at Theorem 7.32 in the BP Primer.}
Write $I = ([p], [v_1], [v_2], \ldots)$, and consider $Q^* H_* \OS{BP}{2} / I^{\circ 2} \circ Q^* H_* \OS{BP}{2}$.  In this quotient, for any $n$ we have \[\sum_{i=1}^n [v_i] \circ b_{(n-i)}^{\circ p^i} \equiv 0. \qed\]
\end{lemma}
...
\end{corollary}
\todo{I don't see the point of this corollary?}
...
\todo[inline]{Work Erick's examples here?}
...
Today we will put Dieudonn\'e modules to work for us in algebraic topology.  Writing $H = H\F_p$, the following result of Goerss, Lannes, and Morel will be central for us:
...
\todo[inline]{Remark 2.9 has a helpful discussion of how to extend to the case $B(0)$.}
...
\begin{lemma}\citeme{Find a citation}
...
\begin{lemma}[{\cite[Proposition 10.2]{GoerssDieudonne}}]
...
\begin{lemma}[{\cite[Proposition 11.6]{GoerssDieudonne}}]
Let $E$ be an even ring spectrum with $E_*$ torsion-free, and suppose that $E_* B(n)$ is even for all $n$.  Then the purely algebraic version of this statement is true: writing $D_E = \{D_{2m} H_* \OS{E}{2n}\}$, the map \[D_E[e] / (e^2 - b_1) \to D_* H_* \OS{E}{*}\] is an isomorphism.
\end{lemma}
\begin{proof}
Since $0 = E_{2n-2k-1} B(2n) \to D_{2n} H_* \OS{E}{2k+1}$ is onto, the latter group is zero, and a bar spectral sequence argument shows the same is true for $D_{2n+1} H_* \OS{E}{2k+2}$~\cite[Lemma 11.5.1]{GoerssDieudonne}.  Hence, the map \[(D_E[e] / (e^2 - b_1))_{*, 2n} \to (D_* H_* \OS{E}{*})_{*, 2n}\] is an isomorphism, and we need only show that \[e D_* H_* \OS{E}{2n} \to D_* H_* \OS{E}{2n+1}\] is an isomorphism as well.  Since $e(Fx) = F(Ve \circ x) = 0$ and $D_* / FD_*H \cong QH$, we see that $e$ kills decomposables and suspends indecomposables: \[e D_* H_* \OS{E}{2n} = \Susp QH_* \OS{E}{2n}.\]  The result then follows from the same bar spectral sequence analysis as above.
\end{proof}

\begin{theorem}[{\cite[Theorem 11.7]{GoerssDieudonne}}]
Restricting attention to the even parts, the maps \[E_* B(2*) \to D_E \from R_E\] are isomorphisms for $E$ Landweber flat.
\end{theorem}
\begin{proof}
In \Cref{HopfRingForEBP}, we showed that these maps are isomorphisms for $E = BP$.  If $E$ is Landweber flat, then the left term (and hence middle term) is determined by change-of-base from the $BP$ left term.  The right term commutes with change-of-base by definition, and the theorem follows.
\end{proof}
% Goerss's original proof of this didn't want to assume the Ravenel--Wilson result, which he reproves by identifying the spectra $B(n)$ as Brown--Gitler spectra and using a different (easier) computation of Ravenel's about $BP_* \Loops^2 S^3$.  Since we've already done the Ravenel--Wilson computation, we can jump straight to it.


\todo[inline]{Compare also with the main result of {\cite{HopkinsHunton}}.}
