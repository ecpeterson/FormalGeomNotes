...
Remarkably, these unstable contexts also admit algebro-geometric interpretations.  In finding the right language for this, we introduce different subclasses of cooperations, and we are also naturally led to consider \textit{mixed cooperations} (as we did stably in \Cref{OrientationsOnEAndMU}) of the form \(F_* \OS{E}{*}\).  The running theme is that when \(E\) and \(F\) are complex-orientable, there is a natural approximation map \[\Spec Q^* F_* \OS{E}{*} \to \InternalHom{FormalGroups}(\CP^\infty_F, \CP^\infty_E)\] which is an isomorphism in every situation of interest.  However, these isomorphisms do not appear to admit uniform proofs\footnote{The best uniform result I can find is due to Butowiez and Turner~\cite[Theorem 3.12]{ButowiezTurner}.}, so we instead investigate the following cases by hand:
\begin{description}
...
\begin{definition}
For a Hopf ring \(H\) with augmentation \(\eps\), we define the \textit{\(\ast\)--indecomposable quotient} by \(Q^* H = \ker \eps / (\ker \eps)^{\ast 2}\) (cf.\ \Cref{DefnOfCoTangentSpaces}).  In the setting of unstable homology cooperations, the collection of \index{operations!unstable}\textit{additive unstable cooperations} is defined as the \index{Hopf ring!star indecomposables@\(\ast\)--indecomposables}\(\ast\)--indecomposable quotient \(Q^* E_* \OS{E}{j}\).
\end{definition}
...
\end{align*}
The even-periodic analogue of \Cref{CalculationOfUnstableSteenrodHopfRing} presents \(\Spec \mathcal A P_0\) as the open subscheme of \(\Spec \F_2[\xi_0, \xi_1, \ldots]\) with \(\xi_0\) inverted.\footnote{Note that \(Q^* \HFtwo_* \OS{\HFtwo}{*}\) does \emph{not} form a Hopf algebra, essentially because it is missing a version of \(\chi\) that inverts \(\circ\)--multiplication.  It is remarkable that inverting the homology suspension element automatically produces an antipode.}  Hence, we see that a compatible name for the inertia group in the unstable context for \(\HFtwo P\) is\index{formal group!homomorphism}
\begin{align*}
...
We thus set about trying to understand the Hopf rings \(F_*(\OS{E}{*})\) in general.  In our computational example in \Cref{UnstableContextsSection}, we found that the topological Hopf ring \(\HFtwo_*(\OS{\HFtwo}{*})\) modeled endomorphisms of the additive formal group after passing to a suitable quotient, and we will take this as inspiration to construct an algebraic model which approximates the topological Hopf ring.
...
\enumeratext{Noting the similarity of the second and fourth relations to those imposed in \Cref{MappingSchemeStatement}, there are two additional families of relations we might consider:}
    \item The dual to the fourth relation is \[(\eta(1))[b] = [\eta\eps(b)].\]
    \item The dual to the second relation requires the presence of Hopf algebra structures on \(A\) and \(B\).  In this setting, for \(a', a'' \in A\) and for \(b \in B\) with diagonal given by \(\Delta(b) = \sum_j b'_j \otimes b''_j\), the dual relation is then \[(a'a'')[b] = \bigast_j \sum_k a_{jk}'[b'_j] \circ a_{jk}''[b''_j].\]
\end{enumerate}
...
\end{lemma}
\begin{proof}
For any space \(X\), we construct a \index{Kronecker pairing}Kronecker-type pairing \[\<-,-\>\co F_n Z \times E^m Z \to F_n(\OS{E}{m})\] as follows: given a class \(f \in \pi_n F(X)\) and a class \(e\co X \to \OS{E}{m}\), we can compose the two to produce an element \(e_*(f) \in \pi_n F(\OS{E}{m})\).\footnote{This map was considered by Goerss~\cite[Proposition 10.2]{GoerssDieudonne}, who cites Strickland as inspiration.}  This pairing is ``bilinear'' in the following senses:
\begin{align*}
\<a' + a'', b\> & = \<a', b\> + \<a'', b\>, &
...
The orientations of \(E\) and \(F\) give rise to classes \(x^j \in E^{2j} \CP^\infty\) and \(\beta_k \in F_{2k} \CP^\infty\), and hence classes \(\beta_k[x^j] \in \AA(F, E)\).  The four core relations imposed on this Hopf ring give us three useful identities:
\begin{enumerate}
...
\begin{enumerate}
	\item Define the \textit{Hopf ring spectrum}\index{Hopf ring!spectrum, \(\SpH\)} of a Hopf ring \(H\) by the formula \[(\SpH H)(T) = \CatOf{HopfRings}(H, T).\]  What functor \(\SpH \AA(F, E)\) does the Hopf ring \(\AA(F, E)\) corepresent?
	\item How does this functor interact with classical rings?
\end{enumerate}

Towards the second, we have already described an interesting bifunctorial embedding \((R, S) \mapsto R[S]\) analogous to the group-ring construction embedding groups into Hopf algebras.  THe following observation describes the arithmetic in its ring of \(\ast\)--indecomposables:
...
Specializing to \(R = \Z\) and varying \(S\), the map \(c\) looks as though it might play the role of a counit for an adjunction.  However, when trying to construct a corresponding unit map, one finds that the definition of the \(\ast\)--product in the ring-ring construction is too free to easily map into.  To ameliorate this, we might destroy the \(\ast\)--product instead by quotienting by \((\ker \eps)^{\ast 2}\).\footnote{One should compare this with an analogous construction in classical rings: the computation in \Cref{ConstructionTangentAffineScheme} shows \(A / I^2 = R \oplus (I / I^2) \cong \Sym(T_0^* A) / (T_0^* A)^2\).}  One quickly checks that this defines a Hopf ring ideal, as in the following steps:
\begin{align*}
\intertext{Closure under \(\chi\):}
\chi(\<r\> \ast \<s\>) & = \<-r\> \ast \<-s\>. \\
\intertext{Closure under \(\ast\):}
(\<r\> \ast \<s\>) \ast (\<r'\> \ast \<s'\>) & = (\<r+s\> - \<r\> \eps(s) - \eps(r)\<s\>) \\
& \quad \ast (\<r' + s'\> - \<r'\> \eps(s') - \eps(r') \<s'\>). \\
...
\begin{center}
\begin{tikzcd}
\CatOf{HopfRings} \arrow[shift left=0.2\baselineskip, "Q^*"]{r} & \CatOf{Rings} \arrow[shift left=0.2\baselineskip, "i"]{l}.
\end{tikzcd}
\end{center}
...
0 \arrow{r} & x A\ps{x} \arrow{r} & \{f \in x K\ps{x} \mid \mathrm{d}f \in A\ps{x}\dx\} \arrow{r}{\mathrm{d}} & H^1_{dR}(\G/A) \arrow{r} & 0 \\
0 \arrow{r} & x A\ps{x} \arrow{r} \arrow[equal]{u} & \left\{ f \in x K\ps{x} \middle| \begin{array}{c} \mathrm{d}f \in A\ps{x}\dx, \\ \delta f \in A\ps{x,y} \end{array} \right\} \arrow{u} \arrow{r}{\mathrm{d}} & PH^1_{dR}(\G/A) \arrow{r} \arrow{u} & 0,
\end{tikzcd}
\end{center}
...
\item At each quasicanonical point, you can form three quotients: two of them make the situation ``worse'' (i.e., raise the order of the quasicanonical point), and one of them makes the situation ``better'' (i.e., lowers the order), which is a kind of witness to the identification \(\G / \G[2] \cong \G\).
...
\item The group \(\F_{2^2}^\times \le \Aut \G\) acts by rotation on \(\mathbb P^1\).
...
Let \(G(n)\) be the free \index{Steenrod algebra!unstable module}unstable \(\mathcal A^*\)--module on one generator of degree \(n\),\footnote{This module admits a presentation as \[G(n) = \begin{cases} \Susp^n \mathcal A / \{\beta^\eps P^i \mid 2pi + 2\eps > n\}\mathcal A & \text{if \(p > 2\)}, \\ \Susp^n \mathcal A / \{\Sq^i \mid 2i > n\}\mathcal A & \text{if \(p = 2\)}. \end{cases}\]  The Spanier--Whitehead dual of this right-module, \(DG(n)\), is given by \[\Susp^n (D G(n))^* = \begin{cases}\mathcal A / \mathcal A \{\chi(\beta^\eps P^i) \mid 2pi + 2\eps > n\} & \text{if \(p > 2\)}, \\ \mathcal A / \mathcal A \{\chi{\Sq^i} \mid 2i > n\} & \text{if \(p = 2\)}. \end{cases}\]} so that if \(M\) is an unstable \(\mathcal A^*\)--module then \[\CatOf{Modules}_{\mathcal A^*}(G(n), M) = M_n.\]
\end{definition}
...
We learn immediately that \(H_* B(n)\) is finite, so that a unique Spanier--Whitehead predual exists and agrees with the Spanier--Whitehead dual.  The statement then follows from the previous equality.
\end{proof}
...
These properties actually uniquely characterize the spectra \(B(n)\) as \textit{Brown--Gitler spectra}\index{Brown--Gitler spectrum}, which arise in many other settings in stable homotopy theory.  For instance, the stable James splitting for \(\Loops^2 S^3\) is given by \[\Susp^\infty_+ \Loops^2 \Susp^2 S^1 \simeq \bigvee_{j=0}^\infty \Susp^j B(\lfloor j/2 \rfloor).\]
\end{remark}
