...
\end{tikzcd}
\end{center}
The effect of killing the transfer ideal in $(BC_p)_E$ is to force the image divisor to be a subdivisor of $\CP^\infty_E[p]$.\todo{Explain why.}  Finally, passing to $B\Sigma_p$ from $BC_p$ exactly destroys the choice of generator of $\Z/p$, i.e., it encodes passing from the homomorphism to the image divisor.  This winds up giving an isomorphism \[(B\Sigma_p)_E^{/\Tr} \cong \Sub_p \CP^\infty_E,\] where $\Sub_p$ denotes the scheme of subgroup divisors of rank $p$.
\end{example}
...
% Describe the topological effect of a power operation on $\CP^\infty_E$
% Isogenies of formal curves
% Weierstrass preparation
% Quotient formal groups and factorization
% Isogenies versus subgroups (+ isomorphisms), the (F-)isogenies pile
% The regular representation, Strickland’s theorems
% The Drinfel’d ring and Galois descent
...
% McClure’s theorems
% Norm-coherence
% Matt’s theorem about norm-coherent sections of line bundles
...
% Identification of H_∞ orientations of E-theory by MU, MSU, MU[6, ∞)
% Projective geometry to check that the $\sigma$--orientation satisfies this. ($E_2$. $K^\Tate$? $Ell$?)
% Indication about H_∞ orientations of Tate K-theory (need power operations internal to it)
