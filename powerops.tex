...
\begin{align*}
b_0 & = \lim_{r \to \infty} b_{(p-1) p^r} \tag{interpolation of homotopy groups} \\
& = \int_{\Z_p^\times} x^{(p-1) p^r} d\mu_c \tag{interpretation as moments} \\
& = \int_{\Z_p^\times} d\mu_c \tag{interpolation of moments} \\
& = \frac{1}{2p} \log c^{p-1}, \tag{\Cref{MazurMeasuresThm}}
\end{align*}
from which it follows that \(B\) belongs to a filler \(A\) if and only if \[\lim_{r \to \infty} b''_{(p-1)p^r} = \frac{1}{2p} \log c^{p-1}.\]
