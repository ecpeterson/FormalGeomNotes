...
\begin{center}
\begin{tikzcd}
\Gamma \arrow{d} & (\phi^j)^* \Gamma \arrow{l} \arrow{rd} \arrow[equal, "g_{p^j[0]}"]{r} & (j^* G)/p^j\cdot[0] \arrow{r} \arrow{d} & G/K \arrow{d} \\
...
A ring map $x\co MUP \to E_\Gamma$ is $H_\infty$ if and only if the internal power operations commute: \[x \circ \mu^{C_p}_{MUP} \circ \Delta = \mu^{C_p}_{E_\Gamma} \circ x^{\sm p}_{hC_p} \circ \Delta \in E_\Gamma^0 T(\L \otimes \rho \downarrow \CP^\infty \times BC_p). \qed\]
\end{corollary}
...
\begin{theorem}[{Goerss--Hopkins--Miller~\cite{GoerssHopkins}; Behrens; Lurie~\cite{LurieSurveyOfEll}}]
...
\begin{lemma}
The $p$--divisible group of an elliptic curve is either formal of height $2$ (called the \textit{supersingular} case) or an extension of an \'etale $p$--divisible group of height $1$ by a formal group of height $1$ (called the \text{ordinary} case).
\end{lemma}
\begin{proof}
...
\psi^k(x) \cdot \psi^{k'}(x) & = \psi^{k + k'}(x). \qed
\end{align*}
\end{theorem}
...
\begin{lemma}[Igusa]\citeme{Igusa}
...
\begin{lemma}
For $\sheaf O$ a topological enrichment of an appropriate map $\stack N \to \moduli{fg}$, there is a spectral sequence \[E_2^{s, t} = H^s(\stack{N}; \pi_t \circ \sheaf O) \Rightarrow \pi_{t-s} \sheaf{O}(\stack{N}). \qed\]
\end{lemma}
...
\begin{center}
\begin{tikzcd}
M\Spin \arrow[densely dotted]{r} \arrow[bend left=15]{rr} \arrow{rrd} \arrow{rd} & KO_{(p)} \arrow{r} \arrow[crossing over]{d} & KO_p \arrow{d} \\
