...
We thus set out to understand the cohomological formal schemes $(B\Sigma_j)_E$ in the case where $E$ is a Morava $E$--theory.

\begin{example}
It is helpful to consider a low-index example to begin.
\end{example}

We also remark that an immediate effect of the $d^1$--differential in the descent spectral sequence is to pass to ``$\ast$--decomposables'', which in this setting amounts to killing all of the proper transfers.  We can therefore restrict attention to the following closed subschemes: \[(B\Sigma_j)_E^{/\tr} := \Spf E^0 B\Sigma_j / I_{\tr} := \Spf \left( E^0 B\Sigma_j / \im(\tr\co E^0 B\Sigma_i \times E^0 B\Sigma_{j-i} \to E^0 B\Sigma_j)_{i=1}^{j-1} \right).\]
...
\subsection*{Norm-coherence and the Lubin--Tate moduli}
...
% E_∞ mapping spectral sequence
% The natural E_∞ context
% The additive E_∞ context (as a semisimplicial object, or as C/B, or…?)

% Isogenies of formal curves
% Weierstrass preparation

% Subgroup divisors, the scheme of subgroups
...
% The abelian E_∞ context
% Level structures
% The Drinfel’d ring and Galois descent
% (Observation: subgroup divisors of the Lubin-Tate formal group have weight 1 everywhere)
