...
Amazingly, this is exactly the behavior of any theory of \index{power operation}power operations for \(E\)--theory when applied to \(\CP^\infty\).  An \(H_\infty\) structure on Morava \(E\)--theory has a \(\Sigma_{p^k}\)--power operation map \[E^0 \CP^\infty \otimes E^0 B\Sigma_{p^k} \from E^0 \CP^\infty,\] which becomes the following map of formal schemes:
\begin{center}
\begin{tikzcd}
...
Having worked through enough of the underlying algebra, we now return to our intended topological application of studying \(H_\infty\)--orientations of Morava \(E\)--theory by \(MUP\).  Since we have avoided giving an \(H_\infty\)--structure on \(E\)--theory so far, there are some questions about what this might mean.  To resolve this ambiguity, recall first from \Cref{OmnibusEinftyTheorem} that there exists at least one \(H_\infty\)--ring structure on Morava \(E\)--theory.  Next, notice that the set of \(H_\infty\)--ring maps is a subset of the homotopy classes of maps: those which commute with the \(\Sigma_n\)--power operations on the source and target.  In the case of \(MUP\)--orientations, this has a surprising effect: since an ordinary orientation of \(E\)--theory corresponds to a coordinate on its formal group, the theory of \(H_\infty\)---orientations is therefore insensitive to the behavior of the power operation on spaces other than \(\CP^\infty\).  Additionally, the behavior of any putative theory of power operations on this particular space was totally determined in the previous section.  We therefore need not be more specific.
