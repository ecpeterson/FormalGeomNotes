...
\begin{enumerate}
    \item Except for a real \index{zeta function@$\zeta$--function!Euler factor}Euler factor, $\zeta$ is basically the \index{Mellin transform}Mellin transform of the measure $\frac{dx}{e^x - 1}$ (i.e., its sequence of moments): \[\zeta(s) = \frac{1}{\Gamma(s)} \int_0^\infty x^{s-1} \frac{dx}{e^x - 1}.\]
