...
\todo[inline]{Cross-references into the books
Make sure there are no double-hats in A.1
Make sure *all* Homs got changed to CatOf and InternalHom in A.1. (TopologicalGroupoids should be internal.)
Make sure that the introduction says all of this is in HKR
Morava's \textit{Local fields} paper; 
Theorem 2.6 of Greenlees--Strickland;

Insert Schlank-Stapleton reference into Strickland’s Theorem subsection
Kuhn citation
Equation labels?
\(p\)--divisible group notation}
...
\begin{center}
\begin{tikzcd}
E^0 BU(j) \arrow{r} \arrow["\simeq"]{d} & E^0 B\Sigma_j / I_{\Tr} \arrow["\mathrm{inj.}"]{r} & C_0 \otimes E^0 B\Sigma_j \arrow["\simeq"]{d} / I_{\Tr} \\
...
\end{tikzcd}
\end{center}
The left-hand isomorphism is stated in \Cref{IdentificationOfBUnWithDivn}; the right-hand isomorphism is the content of the previous paragraph; the top-left horizontal map is surjective because subgroup divisors form a closed subscheme of all divisors; and \Cref{CZeroIsFaithfullyFlat} shows that the two right-hand horizontal maps are injections.  Altogether, these can be used to define the middle vertical map: any element can be preimaged along the surjection, pulled upward along the isomorphism, and pushed back down along the map induced by the regular representation, and one quickly checks that this gives an injective $E^0$--algebra homomorphism.

What remains to show, then, are that the source and target are finite and free of the same rank, and that the resulting determinant is a unit.  These are formidible tasks in both algebraic geometry and algebraic topology.
\end{proof}
...
\end{tabular}
\end{center}
Finally, given such a map \(f\), there is a \index{Bousfield--Kan spectral sequence}spectral sequence computing the homotopy groups of the \(E_\infty\) mapping space: \[E_2^{s, t} = H^s_\theta(A_*, \Loops^{-t} B_*) \Rightarrow \pi_{-s-t} \EinftyRings(E, F), f). \qed\]
\end{theorem}
...
\begin{theorem}[Mazur]
For any auxiliary \(c \in \Z_p^\times\), there is a functional \(f_c\) satisfying\footnote{\[\text{Explicitly,\;} f_c(h) = \int_{\Z_p^\times} h(x) \mathrm d\mu_c = \lim_{r \to \infty} \frac{1}{p^r} \sum_{\substack{0 \le i < p^r \\ p \nmid i}} \int_i^{ci} \frac{h(t)}{t} \mathrm dt.\]  It also satisfies the normalizing property \(\int_{\Z_p^\times} \mathrm d\mu_c = \frac{1}{p} \log(c^{p-1})\).}\footnote{With considerable effort, this output can be halved~\cite[Section 10.3]{AHR}.} \[f_c(x^{k \ge 1}) = \frac{-B_k}{k}(1 - p^{k-1})(1 - c^k).\]
\end{theorem}
