...
\begin{remark}[{\cite[Definition 5.17]{StricklandFSFG}}]
Take $C$ and $D$ to be formal curves over $X$.  A map $f\co C \to C'$ is an \textit{isogeny} when the induced map $C \to C \times_X C'$ exhibits $C$ as a divisor on $C \times_X C'$ as $C'$--schemes.  In this case, a divisor $D$ on $C'$ gives rise to a divisor $f^* D$ on $C$ by scheme-theoretic pullback.  Isogenies of formal groups are maps with finite kernels.  Every nonzero map of formal groups in perfect positive characteristic can be factored as a coordinate change and a Frobenius iterate.  In general, Weierstrass preparation says that any isogeny of curves can be factored as a coordinate change and a standard isogeny: a polynomial.
\end{remark}

Given a level structure $\ell\co A \to \G$, there is a function $N_\ell\co \sheaf O_{\G} \to \sheaf O_{\G}$ given by \[N_\ell(x) = \sum_{a \in A}{}_{\G} (x +_{\G} \ell(a)).\]  Also, given an isogeny $q\co C \to C'$, there is a construction $N_q\co \sheaf O_C \to \sheaf O_{C'}$ given by determinant.  The formal group $\G/A$ defined by coequalizer is set up so that these constructions agree.
...
\begin{enumerate}
    ...
    \item At the special fiber, $q_{p^k}$ reduces to the $k${\th} Frobenius iterate.  (There is \emph{one} of these in each star-isomorphism class.)
    \qedhere
\end{enumerate}
\end{lemma}

The power operation for $E$--theory has this description.

This construction gives a kind of section to the map from isogenies to subgroup divisors given by kernel.

The coequalizer definition comes equipped with a third isomorphism theorem: if $\G \to \H$ has kernel $K$, then there is an isomorphism $\G/K \xrightarrow{\simeq} \H$.  In particular, this gives isomorphisms $\G/K \to \G$ in the case of the maps $q_{p^k}$, witnessing the quotients of the Lubin--Tate formal group as, again, universal deformations.  Draw the norm-coherence triangle.
...
\begin{theorem}[{\cite[Proposition VIII.7.3]{BMMS}}]
Let $F$ be a $p$--local spectrum such that $\pi_* F$ is free over $\Z_{(p)}$ in even dimensions and zero in odd dimensions.  Let $X$ be a space such that $H_*(X; \Z)$ is free abelian in even dimensions and zero in odd dimensions.  If $X$ and $F$ are additionally finite type, then the map \[\iota^* \otimes \Delta^*\co \widetilde F^* X^{\sm G}_{hG} \to \widetilde F^* X^{\sm G} \oplus \widetilde F^*(X \sm BG_+)\] is injective. \qed
\end{theorem}




Do the calculation in the case of $MUP \to E$.  Conclude that $H_\infty$ maps are exactly norm-coherent coordinates.
...
\begin{theorem}
$H_\infty$ orientations $MU[6, \infty) \to E_\Gamma$ are norm-coherent cubical structures.
\end{theorem}
