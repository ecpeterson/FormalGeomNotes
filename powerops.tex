...
\end{tikzcd}
\end{center}
The first functor sends a space $X$ to its spectrum of $E$--cochains $E^X$, and the other two functors form a free/forgetful monad approximating $\EinftyRings_E$ by $\CatOf{Modules}_E$.  These kinds of functors are familiar to us from the discussion of contexts in \Cref{StableContextLecture} and \Cref{UnstableContextsSection}, and the recipe applied in those situations gives an analogous story here.  First, there is a natural map \[\CatOf{Spaces}(*, X) \to \EinftyRings_E(E^X, E),\] which one hopes is an equivalence under (often very strong) hypotheses on $E$ and on $X$.\footnote{It is an unpublished theorem of Hopkins and Lurie that if $X$ is a space admitting a finite Postnikov system with at most $\height \Gamma$ stages and involving only finite groups, then the natural map $F(*, X) \to \EinftyRings_{E_\Gamma/}(E_\Gamma^X, E_\Gamma)$ is an equivalence.}  Second, the adjunction gives a mechanism for resolving $E^X$, which feeds into a spectral sequence computing this mapping space.  The functors $\P$ and $\P_E$ can be given by explicit formula:
\begin{align*}
\P(X) & = \bigvee_{j=0}^\infty X^{\sm j}_{h\Sigma_j}, &
\P_E(M) & = \bigvee_{j=0}^\infty M^{\sm_E j}_{h\Sigma_j}.
\end{align*}
Finally, we expect the homotopy groups of this resolution to form a quasicoherent sheaf over a suitable \emph{$E_\infty$ context}, which forms the case where $X$ is a point.

In the case where we are resolving a point, we can explicitly name some of the terms in this resolution, which will help us better understand what we are studying.  To begin, the bottom two stages of the resolution take the form
\begin{center}
\begin{tikzcd}
\EinftyRings_E(E, E) \\
...
\end{tikzcd}
\end{center}
The available adjunctions give us access to these in more explicit terms:
\begin{align*}
\EinftyRings_E(\P_E(E), E) & \simeq \CatOf{Modules}_E(E, E) \\
& \simeq \CatOf{Spectra}(\S, E),
\intertext{which on homotopy groups computes the coefficient ring of $E$.}
...
We thus set out to understand the cohomological formal schemes $(B\Sigma_j)_E$ in the case where $E$ is a Morava $E$--theory.  We first remark that an immediate effect of the $d^1$--differential in the descent spectral sequence is to pass to ``$\ast$--decomposables'', which in this setting amounts to killing all of the proper transfers.  We can therefore restrict attention to the following closed subschemes: \[(B\Sigma_j)_E^{/\Tr} := \Spf E^0 B\Sigma_j / I_{\Tr} := \Spf \left( \frac{E^0 B\Sigma_j}{\im(\Tr_{\Sigma_i \times \Sigma_j-i}^{\Sigma_j}\co E^0 B\Sigma_i \times E^0 B\Sigma_{j-i} \to E^0 B\Sigma_j)_{i=1}^{j-1}} \right).\]

\begin{example}
It is helpful to consider a low-index example to begin: $(B\Sigma_p)_E$.  In light of the results of \Cref{CharacterTheorySection}, we might begin by analyzing the (maximal) abelian subgroups of $\Sigma_p$.  There is only one: the transitive subgroup $C_p \subseteq \Sigma_p$.  In \Cref{KtheoryConvertsTorsionToTorsion}, we calculated $BS^1[p]_E = \CP^\infty_E[p]$, and we make the further observation that the regular representation map $BS^1[p] \to BU(p)$ induces the following map on cohomological formal schemes:
\begin{center}
\begin{tikzcd}
BS^1[p]_E \arrow{r} & BU(p)_E \\
\InternalHom{FormalGroups}(\Z/p, \CP^\infty_E) \arrow{r} \arrow[equal]{u} & \Div_n^+ \CP^\infty_E \arrow[equal]{u},
\end{tikzcd}
\end{center}
where the bottom arrow sends such a homomorphism to its image divisor.  This map belongs to a larger diagram of schemes:
\begin{center}
\begin{tikzcd}
(B\Sigma_p)_E^{/\Tr} \arrow{r} & (B\Sigma_p)_E \arrow{r} & BU(p) \\
(BC_p)_E^{/\Tr} \arrow{r} \arrow{u} & (BC_p)_E \arrow{u} \arrow{ru}.
\end{tikzcd}
\end{center}
The effect of killing the transfer ideal in $(BC_p)_E$ is to force the image divisor to be a subdivisor of $\CP^\infty_E[p]$\todo{Explain why.}, and this subscheme of homomorphisms is written $\Level(\Z/p, \CP^\infty_E)$.  Finally, passing to $B\Sigma_p$ from $BC_p$ exactly destroys the choice of generator of $\Z/p$, i.e., it encodes passing from the homomorphism to the image divisor.  This winds up giving an isomorphism \[(B\Sigma_p)_E^{/\Tr} \cong \Sub_p \CP^\infty_E,\] where $\Sub_p$ denotes the scheme of subgroup divisors of rank $p$.
\end{example}

In light of this example, there are several further variants of the $E_\infty$ context which one might consider:
\begin{itemize}
    \item The natural $E_\infty$ context: By this, we mean the context constructed in the previous subsection, whose $1$--simplices are given by $\prod_j (B\Sigma_j)_E$.
    \item The abelian $E_\infty$ context: A consequence of \Cref{CharacterTheorySection} is that we can fully understand the behavior of the natural context by embedding the rings $E^0 B\Sigma_j$ into the cohomology of the maximal abelian subgroups of $\Sigma_j$.  A benefit to this context is that it has predictable behavior in Morava $E$--theory: the cohomological formal scheme associated to an abelian group can be presented as an internal scheme of group homomorphisms.
    \item The unstable context: Every power operation can be considered as a family of unstable operations, giving a natural map to the unstable context considered in \Cref{UnstableContextsSection}.
\end{itemize}
We also saw in the example that it was profitable, from the perspective of cohomologial formal schemes, to preempt the $d^1$--differential and pass to the $\ast$--indecomposables.  In order,\footnote{\ldots i.e., in the order listed above, but not at all in logical order of proof.} Strickland has proven the following results about each of these reduced contexts:

\begin{theorem}[{\cite[Theorem 1.1]{StricklandEthyOfBSigma}}]
There is an isomorphism \[(B\Sigma_j)_E^{/\Tr} \cong \Sub_j \CP^\infty_E.\]  If $j$ is not a power of $p$, this is the terminal scheme. \qed
\end{theorem}

\begin{theorem}\citeme{Strickland somewhere}
There is an isomorphism \[BA_E^{/\Tr} \cong \Level(A^*, \CP^\infty_E),\] where $\Level(A^*, \CP^\infty_E)$ denotes the subscheme of $\InternalHom{FormalGroups}(A^*, \CP^\infty_E)$ subject to the condition that $A^*[n]$ forms a subdivisor of $\CP^\infty_E[n]$. \qed
\end{theorem}
...
In this subsection, we seek an algebraic comparison of the different sorts of contexts previously considered.  It will be useful to phrase our ideas in the language of \emph{isogenies}.
...
Let $R$ be a complete local ring.  Every isogeny $f\co \A^1 \to \A^1$ can be factored uniquely as a coordinate change and a monic polynomial. \qed
\end{lemma}

In the case of formal groups over a perfect field $k$ of positive characteristic, this reduces to two familiar structural results:

\begin{corollary}
Every nonzero map of formal groups over $k$ can be factored as an iterate of Frobenius and a coordinate change. \qed
\end{corollary}

\begin{corollary}
A map of ($1$--dimensional, smooth) formal groups over a complete local ring with residue field $k$ is an isogeny if and only if the kernel subscheme of the map is a divisor. \qed
\end{corollary}

\begin{remark}
The Frobenius iterate appearing in the Weierstrass factorization of the multiplication-by-$p$ isogeny $p\co \G \to \G$ is another definition of the height of $\G$.
\end{remark}

Isogenies of formal groups are also automatically (fppf-)surjective, and accordingly it is a good idea to address quotients of formal groups as well as the basic isomorphism theorems.

\begin{definition}
For $i\co K \subseteq \G$ be a subgroup divisor, we define the quotient group $\G/K$ to be the formal scheme whose ring of functions appears as the coequalizer
\begin{center}
\begin{tikzcd}
\sheaf O_{\G/K} \arrow{r} & \sheaf O_{\G} \arrow[shift left=0.4em, "\mu^*"]{r} \arrow[shift right=0.4em, "1 \otimes i^*"']{r} & \sheaf O_{\G} \otimes \sheaf O_K.
\end{tikzcd}
\end{center}
\end{definition}
...
\begin{lemma}
Pullback along the isogeny $q\co \G \to \G/K$ is computed by \[q^* \co D \mapsto D * K,\] where $*\co \Div_n^+ \G \times \Div_d^+ \G \to \Div_{nd}^+ \G$ is the convolution product of divisors on formal groups described in \Cref{ProductMapOfDivisorSchemes}. \qed
\end{lemma}
...
\begin{remark}
This second construction can be upgraded to an assignment from \emph{functions} on $\G$ to \emph{functions} on $\G/K$, rather than just the ideals that they generate.  Specifically, we define the \textit{norm} of $\phi \in \sheaf O_{\G}$ along a level structure $\ell\co A \to \G$ by the formula \[N_\ell \phi = \prod_{a \in A} (\phi \circ \tau_{\ell(a)}).\]  An often useful special case of this norm construction is that if $\phi$ is a coordinate on $\G$, then $N_\ell \phi$ is a coordinate on $\G/K$.
\end{remark}

\begin{remark}
In general, the pullback map $q^*$ admits the following description: an isogeny $q\co C \to C'$ gives a presentation of $\sheaf O_C$ as a finite free $\sheaf O_{C'}$--module.  A function $\phi \in \sheaf O_C$ therefore begets an element $\phi \cdot (-) \in \operatorname{GL}_{\sheaf O_{C'}}(\sheaf O_C)$, and the determinant of this element gives an element of $q^* \phi \in \sheaf O_{C'}$.  Letting $\phi$ range, this gives a multiplicative (but not typically additive) map $\sheaf O_C \to \sheaf O_{C'}$.  If a divisor $D$ is specified as the zero-locus of a function $\phi_D$, the divisor $q^* D$ is specified as the zero-locus of $q^* \phi_D$.
\end{remark}

\begin{lemma}[Third isomorphism theorem for formal groups]
...
\G/K \arrow["{g \quad \simeq}"]{rr} & & \mathbb H.
\end{tikzcd}
\end{center}
\end{lemma}
...
Finally, we note that these constructions interact with the Lubin--Tate moduli problem in an exceptionally interesting way.  This begins with the observation that multiplication by $p$ is an endo-isogeny of any Lubin--Tate group, so that the Lemma above gives rise to an isomorphism
\begin{center}
\begin{tikzcd}
& \G \arrow["q"']{ld} \arrow["p"]{rd} \\
\G/\G[p] \arrow["{g \quad \simeq}"]{rr} & & \G.
\end{tikzcd}
\end{center}
This diagram expresses the observation that $\G / \G[p]$ is again a universal deformation, as witnessed by a preferred isomorphism to $\G$.
\todo{You're missing something here: you need to suck the Frobenius into the deformation data to get the map $\psi$.}
In turn, this observation shows that there is some automorphism of Lubin--Tate space\footnote{An obvious but common point of confusion is that the map $\psi_{\G[p]}$ is \emph{not} an $LT_\Gamma$--algebra map.} \[\psi_{\G[p]}\co (\moduli{fg})^\wedge_\Gamma \to (\moduli{fg})^\wedge_\Gamma\] over which $g\co \G[p] \cong \psi_{\G[p]}^* \G$ covers the identity.
