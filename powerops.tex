...
For any auxiliary \(c \in \Z_p^\times\), there is a functional \(f_c\) given by
\begin{align*}
...
\end{align*}
which satisfies\footnote{With considerable effort, this output can be halved~\cite[Section 10.3]{AHR}.} \[f_c(x^{k \ge 1}) = -\frac{B_k}{k}(1 - p^{k-1})(1 - c^k) \quad \text{and} \quad \int_{\Z_p^\times} \mathrm d\mu_c = \frac{1}{p} \log(c^{p-1}). \pushQED\qed \qedhere \popQED\]
\end{theorem}
