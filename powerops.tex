...
There exist $E_\infty$ maps $Mj \to R$ if and only if $\gl_1 \eta_R \circ j$ is null-homotopic.  If this is the case, then $E_\infty(Mj, R)$ is a torsor for $[\Susp g, \gl_1 R]$. \qed
\end{corollary}
...
This accounts for both $E_\infty(M\Spin, \Q \otimes KO)$ and $E_\infty(M\Spin, \Q \otimes KO_p)$: the set of rational characteristic series includes into the set of ad\`elic characteristic series as the subset with rational coefficients.
...
We want to understand $E_\infty(M\Spin, KO_p)$ and its map to $E_\infty(M\Spin, \Q \otimes KO_p)$.  Here's the initial set-up:
\begin{center}
\begin{tikzcd}[
...
\begin{align*}
[\Susp^{-1} KO, KO] & = 0, &
[KO, KO] & = \Hom(\CatOf{Spaces}(\Z_p^\times / C_2, \Z_p), \Z_p)
\end{align*}
...
\begin{remark}
Some more open questions about this which emphasize the connection to number theory can be found at the end of one of Hopkins's Talbot talks~\cite{HopkinsTheStringOrientation}.
\end{remark}
