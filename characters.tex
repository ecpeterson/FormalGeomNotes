...
\todo[inline]{Ideas for introducing this appendix in the intro to the appendices: Probably the best thing is the desire to understand the $E$-cohomology of the symmetric group and extended powers motivated by understanding power operations. Reference the relevant sections of the book: 2.6.1, 3.5, 2.4}
...
\end{proposition}
\begin{proof}
This is Proposition 5.2 together with Lemma 5.7 in \cite{HKR}.
\end{proof}
...
\]
be a finite abelian group, then there is an isomorphism of $E^*$-algebras
\[
E^*(BA) \cong E^*\ps{x_1, \ldots, x_{i}}/([m_1]_{\G}(x_1), \ldots, [m_i]_{\G}(x_i)).
\]
\end{proposition}
\begin{proof}
This is Corollary 5.10 of \cite{HKR}.
\end{proof}
...
\begin{theorem} \label{app:CWapprox} (Equivariant CW Approximation, Chapter I.1 \cite{MayAlaska})
...
There are $p$-divisible groups that are not built from formal groups. The simplest of these plays a role in this story. Let 
\[
...
\end{proposition}
\begin{proof}
This is Proposition 5.12 of \cite{HKR}.
\end{proof}
...
\]
This choice of $\Lambda$ is the only choice made in this appendix. The last subsection of this appendix addresses how natural the character map is in automorphisms of $\Lambda$. From a certain perspective, \cite{BarthelStapleton} can be viewed as result concerning how natural the character map is in finite index maps $\Lambda \hookrightarrow \Lambda$. 
...
and
\[
\Spec(R) \times_{\Spec(E^0)} \InternalHom{FormalGroups}(A^*, \G[p^k]) \cong \InternalHom{FormalGroups}(A^*, \Lambda_{k}^*) \cong A^{\times n}.
\]
...
The idea of the construction of $C_0$ is quite simple. Proposition \ref{app:abeliangroupdualhom} implies that $E^0(B\Lk) \cong \sheaf O(\InternalHom{FormalGroups}(\Lk^*,\G[p^k]))$. To construct $C_0$, it suffices to understand the open subscheme of $\InternalHom{FormalGroups}(\Lk^*,\G[p^k])$ consisting of the homomorphisms that are isomorphisms. 
...
\]
Given $\alpha \in \Lk^*$, the naturality of Proposition \ref{app:abeliangroupdualhom} implies that the ring of functions on the corresponding map
\[
...
Let $C_{0}^{*} = \pi_{-*}HC_0P \cong C_0[u,u^{-1}]$ where $|u|=-2$. When $X = *$, Example \ref{xapoint2} gives an isomorphism
\begin{align*}
...
Let $X$ be a space and let $\Flag(V) \rightarrow X$ be the bundle of complete flags of a complex vector bundle $V$ over $X$, then
\[
E^*(\Flag(V))
\]
...
\end{proposition}
\begin{proof}
This is Proposition 2.4(2) in \cite{HKR}.
\end{proof}


Since $f \colon V \rightarrow X$ is an $m$-dimensional complex vector bundle, it follows that $EG \times V \rightarrow EG \times X$ is an $m$-dimensional complex vector bundle (this is the external product with a $0$-dimensional complex vector bundle). Since $G$ acts freely on $EG \times X$, the quotient $EG \times_G V \rightarrow EG \times_G X$ is also an $m$-dimensional complex vector bundle.

We proceed with two propositions the proofs of which can both be found in Proposition 2.6 in \cite{HKR}. The first proposition explains the relationship between flag bundles and the Borel construction, the second proposition gives the relationship between the inertia groupoid construction and flag bundles.
...
\end{tikzcd}
\right\}\]
produces the descent object for the canonical map $E^*(X) \rightarrow E^*(\Flag(V))$. Since a finitely-generated free extension is faithfully flat, Proposition \ref{finitefreeflags} implies that
...
\end{proposition}
\begin{proof}
By \ref{app:CWapprox}, since $U(n)/\mathbb{T}$ is compact it is equivalent to a finite $G$-CW complex. Let $A \subset G$ be abelian, then $\rho(A) \subset u\mathbb{T}u^{-1}$ for some $u \in U(n)$. Thus for $a \in A$, $a = utu^{-1}$ for some $t \in \mathbb{T}$, so $A$ fixes the coset $u\mathbb{T}$.
...
When $X = \ast$, Example \ref{app:classfncs} shows that the codomain of the character map is the ring of generalized class functions 
\[
...
\end{proposition}
\begin{proof}
For $E^*(EG \times_G -)$, this follows immediately from the discussion in the previous section. The key ingredient is Proposition \ref{finitefreeflags} along with Proposition \ref{app:orbitflags}, which say that $E^*(EG \times_G \Flag(V))$ is a finitely generated free $E^*(EG \times_G X)$-module. 
...
\[
0 \to HC_0P^*(\Fix_d(X)) \to HC_0P^*(\Fix_d(\Flag(V))) \to HC_0P^*(\Fix_d(\Flag(V)) \times_{\Fix_d(X)} \Fix_d(\Flag(V)))
\]
gives an exact sequence
\[
0 \to HC_0P^*(\Fix_d(X))^G \to HC_0P^*(\Fix_d(\Flag(V)))^G \to HC_0P^*(\Fix_d(\Flag(V)) \times_{\Fix_d(X)} \Fix_d(\Flag(V)))^G
\]
...
\begin{proof}[proof of Theorem \ref{app:mainthm1}]
By \ref{app:codescent}, it suffices to prove that the map is an isomorphism for $X = G/A$. Since 
\[
(G/A) \mmod G \simeq * \mmod A
\]
we have an equivalence
\[
\hom_{TopGpd}(* \mmod \Lambda, (G/A) \mmod G) \simeq \hom_{TopGpd}(* \mmod \Lambda, * \mmod A).
\]
This reduces us to the case $G = A$ and $X =*$. Since both sides of the character map have Kunneth isomorphisms for products of finite abelian groups, we are reduced to cyclic groups. We want to show that 
\[
...
\]
where $C_0$ is the maximal ramified extension of $\Q_p$ discussed in Example \ref{padicktheory}. This map is injective. Above height $1$, though, this is not generally true \cite{Kriz}.
\end{example}
...
There is also an action of $\mathrm{Aut}(\Lambda)$ on $\hom_{TopGpd}(* \mmod \Lambda, X \mmod G)$ given by precomposition. These actions combine to give a conjugation action on $HC_{0}P^{*}(EG \times_G \Fix_d(X))$. Explictly, for $\phi \in \mathrm{Aut}(\Lambda)$ this action is induced by the action on 
\[
* \mmod \Lambda \times \hom_{TopGpd}(* \mmod \Lambda, X \mmod G)
\]
...
\[
\mathrm{ev} \colon * \mmod \Lambda \times \hom_{TopGpd}(* \mmod \Lambda, X \mmod G) \to X \mmod G
\]
