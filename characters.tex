...
There are \(p\)--divisible groups that are \emph{not} built from formal groups, and the simplest of these plays a role in this story. \todo{Mention that the $p$--divisible group base-changed to a rational algebra is automatically \'etale, and that the constant $p$--divisible group is the only canonical \'etale group we know.} We define the \textit{constant \(p\)--divisible group of height \(1\)} to be 
\[
...
Let us work out \(C_0\) in the case of $p$-complete $K$-theory \(E_{\G_m} = \(KU\)_p\). In this case 
\[
\G = \G_m,
\]
...
\[
E^0(B\Lk) = \(KU\)_{p}^0(B\Z/p^k) \cong \Z_p[x]/([p^k]_{\G_m}(x)) \cong \Z_p[x]/(x^{p^k}-1).
\]
We have
\[
...
\]
and this is one of the elements in \(S_k\). Clearly it is a zero-divisor in \(\(KU\)_{p}^0(B\Z/p^k)\), thus
\[
S_{k}^{-1}\(KU\)_{p}^0(B\Z/p^k) \cong S_{k}^{-1}\Z_p[x]/(\Phi_{p^k}(x)).
\]
...
\[
\(KU\)_{p}^{0}(BG) \xrightarrow{} \mathrm{Cl}(G_{p}^{1}, C_0),
\]
where \(C_0\) is the maximal ramified extension of \(\Q_p\) discussed in \Cref{padicktheory}.  This map is injective.  Above height \(1\), though, this is not generally true~\cite{Kriz}, but it \emph{is} true for $G = \Sigma_n$~\cite[Theorem 7.3]{HKR}, which is enough to conclude that $E^0 B\Sigma_n$ is free of finite rank.\todo{Did I get this right?}
\end{example}
