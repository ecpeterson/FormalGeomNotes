...
\todo{This can definitely be rewritten some. The classical perspective is that Cartier divisors (specified as line bundles) are the sheafification of Weil divisors (specified as zero-loci of polynomials), and there isn't much sheafification to be done on a formal curve.}
...
\begin{definition}
The classes $c_j(V)$ of \Cref{CohomologyOfProjectivization} are called the \textit{Chern classes} of $V$ (with respect to the complex-orientation $t$ of $E$), and the polynomial $c(V) = \sum_{j=0}^n (-1)^{n-j} t^j c_{n-j}(V)$ is called the \textit{Chern polynomial}.
\end{definition}
...
\noindent In turn, a cohomology class $f\co \Susp^\infty_+ X \to E$ \todo{If you want to deal with positive degree elements, you have to start here.} gives rise to a morphism of $\Sigma_n$--spectra \[f^{\sm n}\co (\Susp^\infty_+ X)^{\sm n} \to E^{\sm n}.\]  The homotopy colimit of such a diagram is called the \textit{homotopy orbits} of the spectrum, and this gives a natural diagram
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
A ring spectrum $E$ equipped with a suite of factorizations $\mu_n$ satisfying compatibility laws embodying term collection (i.e., $x^a \cdot x^b = x^{a+b}$) and iterated exponentiation (i.e., $(x^a)^b = x^{ab}$) is called an \textit{$H_\infty$--ring spectrum}~\cite[Definition I.3.1]{BMMS}.
\todo[inline]{This is pretty much the definition of an $H_\infty$ ring spectrum, but this is not enough to extract power operations on positive-degree classes.  There, you want compatible systems of maps like $(\Susp^j E)^{\sm n}_{h\Sigma_n} \to \Susp^{nj} E$.  In general, you get such systems only for all $j$ which are divisible by some fixed $d$, and the resulting mess is called an $H_\infty^d$--structure.  For example, $KU$ has an $H_\infty^2$--structure and $KO$ has an $H_\infty^8$--structure.  My suspicion with Nat is that power operation maps come from applying the power operation functor to the identity class in $[E, E]$, and that the $H_\infty^d$--structures on these two spectra come from applying the power operation functor to the autoequivalence supplied by the relevant Bott element.  Meanwhile, Jeremy keeps telling us that this really has to do with representation spheres: the $n$--fold smash power of $S^j$ ought to be called $S^{j\rho}$ for $\rho$ the permutation representation of $\Sigma_n$.  Being able to identify $S^{j\rho}_{h\Sigma_n}$ with $S^{jn}$ is somehow the main input to an $H_\infty^n$--structure.  Justin says that this is definitely the right way to think about these things and that he has a result with Niles that $H_\infty^n$ equals $H_\infty$ + orientations for $n$--dimensional representation spheres.}
...
\begin{center}
\begin{tikzcd}
P \arrow{r} \arrow{d} & * \mmod G_1 \arrow{d} \\
...
\todo[inline]{Justin wrote me this helpful thing.

In 2.4.9 you indicate that you don't follow the argument for the additivity of the Tate power operation. I don't think that argument can work unless your group is $C_p$:

The Tate construction you are considering is the composite of Borel completion $(E\in Sp \mapsto \underline{E}\in Sp_G$ (where $\underline{E}^H=E^{hH}$), a smashing localization that kills free orbits, and then taking fixed points. Since transfers $Tr_H^G$ are defined using a map that factors through a $G$--spectrum of the form $G/H_+\wedge (something)$ the Tate construction you have considered will kill classes of the form $Tr_e^G$. Unless the group is $C_p$, you are going to have other transferred classes that you need to kill. 

Here is a construction of the generalized Tate spectrum that will have the desired property: Consider the standard permutation representation of $\Sigma_n$ acting on a set $S$ of $n$-elements. Define a family of subgroups $F$ of $\Sigma_n$ as follows: take any non-trivial partition of $S$ into disjoint subsets (so we exclude the partition with one subset $[S]$) and then consider the subgroup of $\Sigma_n$ which leaves this partition invariant (e.g. $\Sigma_i \times \Sigma_{n-i}$). These are the the subgroups which are the origins of our bad transfers. Of course, a transfer from a smaller subgroup such as $C_i\times C_{n-i}$ will factor through these other transfers, so we will define $F$ to be the smallest collection of subgroups containing the above bad groups and is closed under taking subgroups (this makes it into a Family). 

Let $O(\Sigma_n)_F\subseteq O(\Sigma_n)$ be the full subcategory of the orbit category spanned by the orbits whose isotropy is in $F$. 
There is a generalized induction/norm map:
$\colim_{\Sigma_n/H\in O(\Sigma_n)_F} E^{hH}--> E^{h\Sigma_n}$ whose cofiber we will denote $E^{t_F \Sigma_n}$  (and yes, I mean homotopy fixed points in the source)
this will be $E_\infty$ if $E$ and the map $E^{h\Sigma_n}-->E^{t_F \Sigma_n}$ will be a map of $E_\infty$ rings. Moreover we have done the minimal localization to the Borel completion of $E$ which kills these transfers. 

Changing your map to land in this Tate construction will give you an additive map. The straightforward augmentation of this construction to the $C_p$-case will yield the same map as the one you have considered.}

\begin{definition}[{\cite[Construction 6.1.6.4, Definition 6.1.6.24]{LurieHA}}]
\todo{This is confusing. Is it even right? Hm.  ---  Justin writes: The norm map factors as $F_{hH} \to F^H \to F^{hH}$.  Actually the equivariant homotopy theory is pretty rough in this section.}
...
\end{lemma}
\begin{proof}
The image of the map $\pi_0 (E^{\Susp^\infty_+ X})_{hG} \to \pi_0 (E^{\Susp^\infty_+ X})^{hG}$ is the kernel of the projection to the Tate object, and this image contains all transfers.\todo{This isn't super clear to me.}
\end{proof}
...
\todo{Is this Tate construction performed in $E$--modules?  Can you rewrite it so that it doesn't have to be?}
The assignment $X \mapsto \pi_0 (E^{(\Susp^\infty_+ X)^{\sm n}})^{tC_n}$ is a cohomology theory.
\end{lemma}
\begin{proof}
\citeme{I learned this proof from Tyler Lawson. He suggested looking at Greenlees--May or Lun{\o}e-Nielsen--Rognes.  This is Prop 2.2.3 of DAG XIII.  He also said that Charles has referred him to this point of view before}
...
F_0 \arrow{r} & F_1 \arrow{r} & \cdots \arrow{r} & F_{n-1} \arrow{r} & Y^{\sm n} \\
X^{\sm n} \arrow[equal]{u} & \bigvee X^{\sm (n-1)} \sm (Y/X) \arrow[leftarrow]{u} & \cdots & \bigvee X \sm (Y/X)^{\sm (n-1)} \arrow[leftarrow]{u} & (Y/X)^{\sm n} \arrow[leftarrow]{u}.
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
F_0^{tC_n} \arrow[equiv]{r} & F_1^{tC_n} \arrow[equiv]{r} & \cdots \arrow[equiv]{r} & F_{n-1}^{tC_n} \arrow{r} & (Y^{\sm n})^{tC_n} \\
(X^{\sm n})^{tC_n} \arrow[equal]{u} & * \arrow[leftarrow]{u} & \cdots & * \arrow[leftarrow]{u} & ((Y/X)^{\sm n})^{tC_n}. \arrow[leftarrow]{u}
\end{tikzcd}
\end{center}
...
To apply this machinery in our case, we need only equip $MU$ with the factorized multiplication maps appearing in the definition of an $H_\infty$ ring spectrum.

\begin{definition}[{\cite[Definition VII.7.4]{Rudyak}}]
Suppose that $\xi\co X \to BU(k)$ presents a complex vector bundle of rank $k$ on $X$.  The $n$--fold direct sum of this bundle gives a new bundle \[X^{\times n} \xrightarrow{\xi^{\oplus n}} BU(k)^{\times n} \xrightarrow{\oplus} BU(n \cdot k)\] of rank $nk$ on which the cyclic group $C_n$ acts.  By taking the (homotopy) $C_n$--quotient\todo{Make it clear that you're also taking the quotient of the total space.}, we produce a vector bundle $\xi(n)$ on $X^{\times n}_{hC_n}$ participating in the diagram
\begin{center}
\begin{tikzcd}
X^{\times n} \arrow["\xi^{\oplus n}"]{r} \arrow{d} & BU(k)^{\times n} \arrow["\oplus"]{r} \arrow{d} & BU(nk) \\
X^{\times n}_{hC_n} \arrow["\xi(n)"]{r} & BU(k)^{\times n}_{hC_n}. \arrow[densely dotted, "\widetilde \mu_n"]{ru}
\end{tikzcd}
\end{center}
...
\todo{Cite Christian's undergrad thesis?}
The picture Quillen paints of all this is considerably different from ours.  He begins by giving a different presentation of the complex cobordism groups of a manifold $M$: a complex orientation of a smooth map $Z \to M$ is a factorization \[Z \xrightarrow{i} E \xrightarrow{\pi} M\] through a complex vector bundle $\pi\co E \to M$ by an embedding $i$, as well as a complex structure on the normal bundle $\nu_i$.  Up to suitable notions of stability (in the dimension of $E$) and homotopy equivalence (involving, in particular, isotopies of different embeddings $i$), these quotient to give cobordism classes of maps complex-oriented maps $Z \to M$.  The collection of cobordism classes over $M$ of codimension $q$ over is isomorphic to $MU^q(M)$~\cite[Proposition 1.2]{Quillen}.  Quillen's definition of the power operations is then given in terms of this geometric model: a representative $f\co Z \to M$ of a cobordism class gives rise to another complex-oriented map $f^{\times n}\co Z^{\times n} \to M^{\times n}$, and he defines $P^{C_n}_{\mathrm{ext}}(f)$ to be the postcomposition with $M^{\times n} \to M^{\times n}_{hC_n}$.\todo{This target isn't a manifold, so be careful.}  All the properties of his construction must therefore be explored through the lens of groups acting on manifolds.
\end{remark}

\begin{example}
\citeme{Where can you find Steenrod squares defined like this?  The VFoS notes, but that's not a great reference.}
...
Having now set up an encompassing theory of stable operations, we now seek to give a formula for the cyclic Tate power operation in this framework.  In order to approach this, we initially set our sights on the too-lofty goal of computing the total power operation $P^{C_n}(f)$ for $f \in MU^{2q}(X)$ an $MU$--cohomology class on a finite complex $X$.  Because of the definition $MU = \colim_k MU(k)$ and because $P^{C_n}$ is natural under pullback, it will suffice for us to study the effect of $P^{C_n}$ on the universal classes\todo{Do you need to put suspensions in here so that this is a map off of a suspension spectrum?} \[u_m\co MU(m) \to MU,\] beginning with the canonical orientation \[u_1 = x \in h\CatOf{Spectra}(MU(1), MU) \cong MU^2 \CP^\infty.\]  In order to understand the effect $P^{C_n}(x)$ of the power operation on $x$, we recall a different interpretation of $x$: it is also the $1${\st} Conner--Floyd--Chern class of the tautological bundle $\L$ on $\CP^\infty$, i.e., \[x\co MU(1) \to MU\] \todo{Is this confused about suspensions?} is the Thomification of the block inclusion \[\L\co BU(1) \to BU.\]  The construction defining $P^{C_n}_{\mathrm{ext}}(x)$ thus fits into the following diagram:
\begin{center}
\begin{tikzcd}
...
\begin{align*}
P^{C_n}(u_m) & = \overset{\text{each of the $m$ factors}}{\overbrace{P^{C_n}(x_1) \cdot \cdots \cdot P^{C_n}(x_m)}} \\
& = \left(\sum_{|\alpha| \le 1} w^{1 - |\alpha|} a_\alpha(t) s_\alpha(x_1)\right) \cdot \cdots \cdot \left( \sum_{|\alpha| \le 1} w^{1 - |\alpha|} a_\alpha(t) s_\alpha(x_m) \right) \\
...
\todo{The previous lecture calls the Euler class ``$x$'', which does not match the notation here.  Also, the conclusion of this lecture doesn't mention the connection to the Tate construction.  Also, the ligament of this lecture doesn't point out that we're trying to compute the total power operation and \emph{not} its Tate projection, but our strategy ends up most naturally giving us information about the Tate projection and we could phrase it in a Corollary describing $P^{C_n}_{\Tate}(f) = w^{-m} \cdots$.}
...
\begin{remark}
This proof actually also goes through for $MO$ as well.  In that case, it's even easier, since the equation $2 = 0$ in $\pi_0 MO$ causes much of the algebra to collapse.  The proof does not extend further to cases like $MSO$ or $M\textit{Sp}$, as explained in the introduction to this Case Study: these bordism theories do not have associated formal group laws, and so we lose the control we had in \Cref{StabilizingTheMUSteenrodOps}.
\end{remark}
