...
\begin{remark}
These operations $P^n$ are visibly multiplicative: \[P^n(f \smile g) = P^n(f) \smile P^n(g).\]  Additionally, $P^n(f)$ reduces to the $n$--fold internal cup product $f^{n}$ upon restriction to the basepoint in $BC_n$.  In general, classes in $MU^* BC_n$ can be used to give rise to other ``restricted power operations'' that encode unstable operations other than the literal $n${\th} cup power.
\end{remark}
...
The chain model for ordinary homology is actually rigid enough to define power operations there, too.  Curiously, they are all generated by the quadratic power operations (i.e., the ``squares''), and all the quadratic power operations turn out to be \emph{additive} --- that is, you just get the Steenrod squares again!  Specifically, the $C_2$--construction \[(\Susp^n X \xrightarrow{f} H\F_2) \xrightarrow{P^2} (\Susp^{2n} X \times BC_2{}_+ \xrightarrow{P^2(f)} H\F_2)\] gives a class in $H\F_2^{2n-*}(X) \otimes H\F_2^*(\RP^\infty)$, which decomposes as \[P^2(f) = \sum_{j=0}^{2n} \Sq^{2n-j}(X) \otimes x^j.\]  This appears to be a lucky degeneracy, but in the next Lecture we will exploit something similar with a particular power operation in complex cobordism.
\end{remark}
...
For $X$ a finite complex and $f \in MU^{2q}(X)$ some $MU$--cohomology class, we set our sights on the too-lofty goal of computing $P^n(f)$.  Because of the definition $MU = \colim_k MU(k)$ and because $P^n$ is natural under pullback, it will suffice for us to study the effect of $P^n$ on the universal classes \[MU(k) \to MU,\] beginning with the canonical orientation \[x \in h\CatOf{Spectra}(MU(1), MU) \cong MU^2 \CP^\infty.\]  In order to understand the effect $P^n(x)$ of the power operation on $x$, we recall a different interpretation of $x$: it is also the $1${\st} Conner--Floyd--Chern class of the tautological bundle $\L$ on $\CP^\infty$, i.e., \[x\co MU(1) \to MU\] is the Thomification of the block inclusion \[\L\co BU(1) \to BU.\]  The construction defining $P^n_{\mathrm{ext}}(x)$ thus fits into the following diagram:
\begin{center}
\begin{tikzcd}
& (\Susp^\infty_+ BU(1))^{\sm n} \arrow{r} \arrow[bend left=15, "\oplus"]{rr} \arrow["c_1^{\sm n}"]{d} & \Susp^\infty_+ BU(1)^{\sm n}_{hC_n} \arrow{r} \arrow{d} & \Susp^\infty_+ BU(n) \arrow["c_n"]{d} \\
& (\Susp^2 MU)^{\sm n} \arrow{r} & \Susp^{2n} MU^{\sm n}_{hC_n} \arrow{r} & \Susp^{2n} MU.
\end{tikzcd}
\end{center}
Hence, the the internal Steenrod power operation $P^n(x)$ is defined by the composite \[\Susp^\infty_+ \CP^\infty \sm \Susp^\infty_+ BC_n \simeq \Susp^\infty_+ \CP^\infty_{hC_n} \xrightarrow{\Delta_{hC_n}} (\Susp^\infty_+ \CP^\infty)^{\sm n}_{hC_n} \to \Susp^{2n} MU^{\sm n}_{hC_n} \to \Susp^{2n} MU,\] which is to say \[P^n(x) = c_n(\Delta_{hC_n}^* \L(n)).\]  We have thus reduced to computing a particular Chern class of a particular bundle.
...
We thus proceed to analyze $\Delta^*_{hC_n} \L(n)$ by studying the $C_n$--equivariant bundle $\Delta^* \L^{\oplus n}$ instead.  The $C_n$--action is given by permutation of the factors, and hence we have an identification \[\Delta^* \L^{\oplus n} \cong \L \otimes \pi^* \rho,\] where $\rho$ is the permutation representation of $C_n$ (considered as a vector bundle over a point) and $\pi\co \CP^\infty \to *$ is the constant map.  The permutation representation for the abelian group $C_n$, also known as its regular representation, is accessible by character theory.  Namely, using the generating character $\chi\co U(1)[n] \to U(1)$ we gain a decomposition \[\rho \cong \bigoplus_{j=0}^{n-1} \chi^{\otimes j}\] of the associated vector bundles.  Applying this to our situation, we get a sequence of isomorphisms of $C_n$--equivariant vector bundles \[\Delta^* \L^{\oplus n} \cong \L \otimes \pi^* \rho \cong \L \otimes \bigoplus_{j=0}^{n-1} \pi^* \chi^{\otimes j} \cong \bigoplus_{j=0}^{n-1} \L \otimes \pi^* \chi^{\otimes j}.\]

Applying \Cref{GEquivBundlesVsBundlesOverBG}, we recast this as a calculation of the bundle $\Delta_{hC_n}^* \L(n)$: \[\Delta_{hC_n}^* \L(n) = \bigoplus_{j=0}^{n-1} \pi_1^* \L \otimes \pi_2^* \eta^{\otimes j},\] where $\eta$ is the bundle classified by $\eta\co BU(1)[n] \to BU(1)$ and $\pi_1$, $\pi_2$ are the two projections off of $BU(1) \times BC_n$.  We now use this to access $c_n(\Delta_{hC_n}^* \L(n))$.  As the top Chern class of this $n$--dimensional vector bundle, we recast this as a calculation of its Euler class, which lets us lean on multiplicativity as follows:
\begin{align*}
...
\end{align*}
Here $x$ is still the $1${\st} Conner--Floyd--Chern class of $\L$, and $t$ is the Euler class of $\eta$.  We can understand $t$ as follows:
...
\end{tikzcd}
\end{center}
The Euler class of the first bundle is the class $x$, and it pulls back along the map $n\co BU(1) \to BU(1)$ to give the Euler class $[n](x)$ of the second bundle.  The induced long exact sequence (known as the Gysin sequence, or as the couple in the Serre spectral sequence for the first fibration) takes the form
\begin{center}
\begin{tikzcd}
...
\noindent In all, we learn that the Euler class $t = e(\eta)$ corresponds to the restriction of the coordinate $x$ along the inclusion \[BU(1)[n]_{MU} \cong BU(1)_{MU}[n] \to BU(1)_{MU}.\]
...
\end{theorem}
\begin{proof}
We take $m$ large enough so that $f$ is represented by an unstable map \[g\co \Susp^{2m} X \to T_m BU(m+q),\] in the sense that $g$ intertwines $f$ with the universal class $u_{m+q}$ by the formula \[g^* u_{m+q} = \sigma^{2m} f.\]  Since $P^n$ is multiplicative and $P^n(\sigma^{2m}) = w^m \sigma^{2m}$, we use naturality to conclude
\begin{align*}
w^m P^n(f) & = P^n(\sigma^{2m} f) = P^n(g^* u_{m+q}) = g^* P^n(u_{m+q}) \\
& = g^* \left(\sum_{|\alpha| \le m} w^{q+m - |\alpha|} a_\alpha(t) s_\alpha(u_{m+q}) \right) \\
& = \sum_{|\alpha| \le q} w^{q+m - |\alpha|} a_\alpha(t) s_\alpha(f). \qedhere
\end{align*}
\end{proof}
