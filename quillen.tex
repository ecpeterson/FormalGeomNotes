...
\begin{example}\todo{Make this clearer: point out that the cofiber definition of the Thom space helps you make the first identification (that's what the ``zero section'' refers to), and then point out that $\zeta^*(M) = M / \sheaf I(0) \cdot M$ helps you over the last hump.}
...
The proof of the following Lemma is fairly easy, but not especially useful to us, so we omit it.
...
With \Cref{QuillensKeyRelation} in hand, we deduce Quillen's major structural theorem about $MU_*$.
\begin{theorem}\citeme{Quillen Theorem 5.1}
Again write $C$ for the subring of $MU_*$ generated by the coefficients of the formal group law associated to the identity complex--orientation.  If $X$ has the homotopy type of a finite complex, then
\begin{align*}
...
\textbf{Now show that this map is injective.  Finally, show that $\pi_*(MU \sm MU)$ carries the universal formal group law isomorphism.}
