...
\todo{Wait a second, have you introduced this terminology yet?}
...
\todo[inline]{Work this in, comparing property with structure.}
\begin{theorem}\label{DetectingFormalVarieties}
\todo{Get rid of this obnoxious auxiliary module $M$ and just call $I \otimes_R M$ by its real name: $I^n$.}
...
\todo[inline]{Below this, things are figured out.}
...
\todo{I think this section is a little short. Maybe we could expand the proof of the comparison between Cartier and Weil divisors at the end? Or we would include a proof of the universality of the divisor scheme?  Something to give this a little more meat, anyway.}
...
\Susp^{2nr} (\Susp^\infty_+ X)^{\sm n}_{hC_n}. \arrow[equal]{u} \arrow["P^{C_n}_{\mathrm{ext}}(f)"']{urr}
\end{tikzcd}
\end{center}
...
\todo{Wait a second, is that vertical equality actually an equality? What about the $S^{2r\rho}$ concern?}
\end{definition}
...
\begin{remark}
This construction can be mimicked using \emph{any} group $G$ that acts on $(\Susp^\infty_+ X)^{\sm n}$.  For instance, $\Sigma_n$ is another natural choice.  This flexibility in the choice of $G$ shows up naturally when trying to understand compositions like $P^{C_n}_{\mathrm{ext}} \circ P^{C_m}_{\mathrm{ext}}$, which involve $X^{\sm(nm)}$ and the action of the wreath product $C_n \wr C_m$.  The adjective ``cyclic'' is meant to record our choice of group $G = C_n$.  
\end{remark}

\begin{remark}\label{RemarkSteenrodOpnsAreNotStable}\label{RestrictingSteenrodOpToBasepoint}
This assignment \[\left\{ \Susp^\infty_+ X \to MU \right\} \xrightarrow{P^{C_n}} \left\{ \Susp^\infty_+ X \sm \Susp^\infty_+ BC_n \to MU \right\}\] extends to a map of function spaces\todo{Is this right? You can't interpret the image of an element of $\pi_{2r}$ on the left as being the power operation applied to another cohomology class, since you'd expect the image to lie in degree $\pi_{2nr}$.} \[\CatOf{Spaces}(X, MU) \xrightarrow{P^{C_n}} \CatOf{Spaces}(X \times BC_n, MU).\]  However, this is \emph{not} an infinite loop map, i.e., it does \emph{not} descend from a map of function spectra.  The basic obstruction is that power operations are not additive, which is what the infinite loopspace structure governs, but rather satisfy a kind of binomial theorem~\cite[pg.\ 25]{BMMS}.  On the other hand, these operations $P^{C_n}$ are visibly multiplicative: \[P^{C_n}(f \cdot g) = P^{C_n}(f) \cdot P^{C_n}(g).\]  Additionally, $P^{C_n}(f)$ reduces to the $n$--fold internal cup product $f^{n}$ upon restriction to the basepoint in $BC_n$.
\end{remark}

\begin{definition}
More generally, classes in $\sigma \in MU_* BC_n$ can be used to give rise to operations using the Kronecker pairing: \[\{\Susp^\infty_+ X \xrightarrow{f} MU\} \to \{\Susp^\infty_+ X \sm \S \xrightarrow{1 \sm \sigma} \Susp^\infty_+ X \sm \Susp^\infty_+ BC_n \sm MU \xrightarrow{P^n F \sm 1} MU \sm MU \xrightarrow{\mu} MU\}.\]  This construction encodes other \textit{restricted power operations} than the literal $n${\th} cup power.
\end{definition}

\citeme{Where can you find Steenrod squares defined like this?  The VFoS notes, but that's not a great reference.}
With this in mind, we now try to understand what it is that we've constructed by means of a simpler example.  The chain model for $H\F_2$--homology is actually also rigid enough to define cyclic squaring operations.  Curiously, these power operations turn out to be \emph{additive}.  Specifically, the $C_2$--construction \[(\Susp^n \Susp^\infty_+ X \xrightarrow{f} H\F_2) \xrightarrow{P^{C_2}} (\Susp^{2n} \Susp^\infty_+ X \sm \Susp^\infty_+ BC_2 \xrightarrow{P^{C_2}(f)} H\F_2)\] gives a class in $H\F_2^{2n-*}(X) \otimes H\F_2^*(\RP^\infty)$, which decomposes as \[P^{C_2}(f) = \sum_{j=0}^{2n} \Sq^{2n-j}(f) \otimes x^j.\]  This appears to be a lucky degeneracy owing to the binomial identity $(x + y)^2 \equiv x^2 + y^2 \pmod 2$, but we will see that this behavior is actually quite generic.


---


Try to construct an exact functor to the right of fixed points.

The equivariant map $\sum_g g^* i\co X \to G \times X$ has homotopy orbits the \textit{transfer map} $X_{hG} \to X$, and this witnesses $X_{hG}$ as a cone over the $* \mmod G$--indexed diagram $X$, hence gives a \textit{norm map} $X_{hG} \to X^{hG}$.  Its cofiber is the Tate construction $X^{tG}$.

The composite $X \mapsto (\Susp^\infty_+ X^{\times n})^{tC_n}$ preserves cofiber sequences, i.e., it is \emph{stable}.  (This is ``Tyler's argument'' from \texttt{hinfty.tex}.)

Remark: in fact, the Tate construction is the universal additive functor under fixed points.

Thm: The effect of the map to the Tate construction is to invert the Euler class.

Cor: The total cyclic power operation decomposes as a sum of stable operations after inverting the Euler class in $MU^* BC_n$.
...
The cyclic power operation for $MU$ has signature \[P^{C_n}\co F(X, \Susp^{2m} MU) \to F(X_{hC_n}, \Susp^{2nm} MU),\] and its target can be reidentified as \[F(X_{hC_n}, \Susp^{2nm} MU) \simeq F(X, \Susp^{2nm} MU)^{hC_n}.\]  For any $C_n$--equivariant spectrum $X$, there is a special map called the \textit{transfer map}\citeme{Greenlees--May?} \[X_{hC_n} \to X^{hC_n} \xrightarrow{\cofib} X^{tC_n}\] whose cofiber is called a \textit{Tate spectrum}.  The construction of Tate spectra is \emph{homological}: cofiber sequences of spaces are sent to cofiber sequences of Tate spectra, and hence there is a natural \emph{stable} map \[F(X, \Susp^{2m} MU) \to F(X, \Susp^{2nm} MU)^{tC_n}.\]  In this setting, passing to the Tate construction has the effect of inverting $t$ on homotopy\citeme{Greenlees--May?}, and so this is a homotopical witness to a generic kind of relation expressed in \Cref{QuillensKeyRelation}: the cyclic power operations become stable after multiplication by huge powers of $t$.  The method described here gives a precise formula for the relationship, and in the proof of \Cref{CGenerationForFiniteCplx} in the next Lecture we will discover the surprising fact that we only need to multiply by a \emph{single} $t$.
...
\textbf{Our goal in the next Lecture is to make this decomposition of $P^{C_n}$ more explicit.}
...
We have left untouched a major component from the previous Lecture: any kind of example calculation of the cyclic power operations.  Power operations turn out to be prohibitively complicated, preventing us from ever really doing this --- but in trying, we \emph{will} find a natural approximation formula in terms of the stable Landweber--Novikov operations.  Giving a precise formula for this approximation is the goal of this Lecture.

For $X$ a finite complex and $f \in MU^{2q}(X)$ some $MU$--cohomology class, we set our sights on the too-lofty goal of computing $P^{C_n}(f)$.  Because of the definition $MU = \colim_k MU(k)$ and because $P^{C_n}$ is natural under pullback, it will suffice for us to study the effect of $P^{C_n}$ on the universal classes \[u_m\co MU(m) \to MU,\] beginning with the canonical orientation \[u_1 = x \in h\CatOf{Spectra}(MU(1), MU) \cong MU^2 \CP^\infty.\]  In order to understand the effect $P^{C_n}(x)$ of the power operation on $x$, we recall a different interpretation of $x$: it is also the $1${\st} Conner--Floyd--Chern class of the tautological bundle $\L$ on $\CP^\infty$, i.e., \[x\co MU(1) \to MU\] is the Thomification of the block inclusion \[\L\co BU(1) \to BU.\]  The construction defining $P^{C_n}_{\mathrm{ext}}(x)$ thus fits into the following diagram:
\begin{center}
\begin{tikzcd}
...
Hence, the the internal cyclic power operation $P^{C_n}(x)$ is defined by the composite \[\Susp^\infty_+ BU(1) \sm \Susp^\infty_+ BC_n \simeq \Susp^\infty_+ BU(1)_{hC_n} \xrightarrow{\Delta_{hC_n}} (\Susp^\infty_+ BU(1))^{\sm n}_{hC_n} \to \Susp^{2n} MU^{\sm n}_{hC_n} \to \Susp^{2n} MU,\] which is to say \[P^{C_n}(x) = c_n(\Delta_{hC_n}^* \L(n)).\]  We have thus reduced to computing a particular Conner--Floyd--Chern class of a particular bundle.
...
This is about as much information as we can hope to extract in the $1${\st} universal case.  We thus return to our original goal: understanding the action of $P^{C_n}$ on each of the canonical classes \[u_m\co MU(m) \to MU.\]  Our best hope to approach this is to use the splitting principle, so we rewrite the formula in \Cref{AjAndBjAreInTheFGLSubring} in a form amenable to direct sums:
\begin{align*}
...
With \Cref{QuillensKeyRelation} in hand, we will deduce Quillen's major structural theorem about $MU_*$.  We will preserve the notation used in \Cref{AjAndBjAreInTheFGLSubring} and \Cref{QuillensKeyRelation}:
\begin{itemize}
...
\begin{remark}
This proof actually also goes through for $MO$ as well.  In that case, it's even easier, since the equation $2 = 0$ in $\pi_0 MO$ causes much of the algebra to collapse.  The proof does not extend further to cases like $MSO$ or $MSp$, as explained in the introduction to this Case Study: these bordism theories do not have associated formal group laws, and so we lose the control we had in \Cref{StabilizingTheMUSteenrodOps}.
\end{remark}
