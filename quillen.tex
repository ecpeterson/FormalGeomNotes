...
We define a \textit{nilpotent thickening} to be a closed inclusion of schemes whose associated ideal sheaf is nilpotent.  An \(S\)--point \(s\co S \to X\) of a Noetherian scheme is formally smooth exactly when \(T_s X\) is a free \(R\)--module and for any nilpotent thickenings \(S \to \Spec B \to \Spec B'\) and any solid diagram
\begin{center}
...
Select \(\omega \in MU^*(X \times BC_n)\) and write \[t \cdot \<n\>_{MU}(t) = [n]_{MU}(t).\]  If \(\omega\) satisfies \(t \cdot \omega = 0\), then there exists a class \(y\) with \(\omega = y \cdot \<n\>_{MU}(t)\).
\end{lemma}
\begin{proof}[Proof sketch]
Consider the Gysin sequence for the bundle \(\eta\) as viewed in the cohomology theory \(MU^*(X \times -)\):
\begin{center}
\begin{tikzcd}
MU^*(X \times BC_n) \arrow{rr}{t} & & MU^{*+2}(X \times BC_n) \arrow{ld}{\pi^*} \\
& \widetilde{MU}^{*+1}(X \times S^1_{hC_n}) \arrow{lu}{\partial}
\end{tikzcd}
\end{center}
Because the action of \(C_n\) on the total space \(S^1\) is free, we have \(S^1_{hC_n} \simeq S^1\).  The suspension isomorphism then rewrites the bottom node as
\begin{center}
\begin{tikzcd}
MU^*(X \times BC_n) \arrow{rr}{t} & & MU^{*+2}(X \times BC_n) \arrow{ld}{\pi^*} \\
& \widetilde{MU}^*(X) \arrow{lu}{\<n\>_{MU}(t)}.
\end{tikzcd}
\end{center}
In order to justify the image of \(\partial\), one needs to unwrap the definition of the Gysin sequence~\cite[Equation 4.7, Proposition 4.2]{Quillen}, which we elect not to do.  Given this, however, the Lemma statement follows from exactness at the top-left node.
\end{proof}
