...
\begin{remark}
This proof actually also goes through from $MO$ as well.  In that case, it's even easier, since the characteristic of $MO$ forces $2 = 0$ and much of the algebra collapses.  One can try to further perturb this proof in two ways:
\begin{enumerate}
...
We now have a foothold on $\pi_* MU$, and this alone is enough to move us to study $\moduli{fgl}$, the moduli scheme of formal group laws.  However, while we're here, it's possible for us to prove the rest of Quillen's theorem, if we get just slightly ahead of ourselves and assume one algebraic fact.  The place to start is with the following topological observation about mixing complex--orientations:
...
\begin{align*}
x^{MU} & = \sum_{j=0}^\infty b_j^E (x^E)^j = g(x^E), &
...
\end{tikzcd}
\end{center}
allows us to pair $x^{MU}$ with $(x^E)^{j}$ to determine the coefficients of the series.
\end{proof}
...
However, one also notes that $H\Z_* MU = \Z[b_1, b_2, \ldots]$ carries the universal example of a formal group law with a logarithm --- this observation is independent of any description of $MU_*$, which we still do not fully understand.  It turns out that this brings us one algebraic fact away from understanding $MU_*$:
...
\begin{corollary}
The ring $\pi_*(MU \sm MU)$ carries the universal example of two isomorphic formal group laws.
\end{corollary}
\begin{proof}
