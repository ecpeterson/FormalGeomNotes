...
\begin{example} \todo{Expand me. This is a good way to close the day.}
...
\item Let $\xi: X \to BU(n)$ classify a vector bundle and let $\phi$ denote the Thom isomorphism \[\phi: MU^* X \to MU^* T(\xi).\]  Then the Chern classes of $\xi$ are related to the Landweber--Novikov operations by the formula \[\sum_\alpha \phi c_\alpha(\xi) \t^\alpha = \sum_\alpha s_\alpha \phi(1) \t^\alpha.\]
\end{itemize}
\end{remark}
...
\begin{definition}
Let $X$ be a manifold.  Two closed submanifolds $Y$ and $Z$ are said to \textit{intersect cleanly} when $W = Y \cap Z$ is a submanifold and for each $w \in W$, $T_w W = T_w Y \cap T_w X$.  In this case, we draw a Cartesian square
\begin{center}
\begin{tikzcd}
...
\begin{remark}
If $\nu_{i'}$, $\nu_i$, and $F$ are endowed with complex structures compatible with this exact sequence, then the Gysin maps induce Thom isomorphisms
\begin{align*}
i_* \co MU^*(Z) & \xrightarrow{\simeq} MU^{*+a}(X, X \setminus Z), \\
i'_* \co MU^*(W) & \xrightarrow{\simeq} MU^{*+b}(Y, Y \setminus W),
\end{align*}
where $a$ and $b$ are the real dimensions of $\nu_i$ and $\nu_{i'}$ respectively.
\end{remark}

\begin{lemma}\citeme{Proposition 3.3 of Quillen}\needproof{Euler classes}
For $z \in MU^*(Z)$, \[j^* i_* z = i'_*(e(F) \cdot (j')^* z)\] in $MU^{*+a}(Y, Y \setminus W)$.
\end{lemma}
\begin{proof}
\todo{Maybe we should prove this? It's not hard. It doesn't have anything to do with formal geometry, though.}
\end{proof}

Now let $G$ be a compact Lie group and let $i: Z \to X$ be an embedding of $G$--manifolds. Then the $G$--fixed submanifold $X^G$ and $Z$ intersect cleanly in the diagram
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
Since $r_Z^*(\nu_i)$ is a $G$--bundle over a trivial $G$ space, there is a decomposition $r_Z^*(\nu_i) = \nu_{i^G} \oplus \mu_i$, where $\nu_{i^G}$ has no $G$--action and $\mu_i = F$, the excess bundle, splits further as a sum of eigenbundles corresponding to the nontrivial irreducible representations of $G$.  The clean intersection formula then gives \[r_X^* i_* z = i^G_*(e(\mu_i) \cdot r_Z^* z)\] for $z \in MU^*(EG \times_G Z)$.  More generally, if $f: Z \to X$ is a proper complex-oriented $G$-map, we pay take the complex orientation of $f$ to be represented by a factorization \[Z \xrightarrow i E \xrightarrow p X,\] where $E$ is a complex $G$-bundle on $X$ and $i$ is an embedding with an equivariant complex structure on its normal bundle.  Let $\mu(E)$ be the sum of the eigenbundles of $r_X^* E$ corresponding to the nontrivial irreducible representations of $G$, where, as before, $r_X$ is the inclusion of the fixpoint submanifold $X^G \subseteq X$.

\begin{lemma}\citeme{Proposition 3.8 of Quillen}
If $z \in MU^*(EG \times_G Z)$, then in $MU^*(BG \times X^G)$: \[e(\mu(E)) \cdot r_X^* f_* z = f^G_*(e(\mu_i) \cdot r_Z^* z). \qed\]
\end{lemma}

\begin{lemma}\citeme{Proposition 3.12 of Quillen}
Suppose $G$ acts transitively on $\{1, \ldots, k\}$ and let $\rho$ denote the induced reduced regular $G$-representation. Suppose that $f: Z \to X$ is a proper complex-oriented map of dimension $2q$ and that $m$ is an integer larger than the dimension of $Z$, so that $m \eps + \nu_f$ is a vector bundle over $Z$, well-defined up to isomorphism, where $\eps$ is the trivial complex line bundle. Then \[e(\rho)^m P(f_* 1) = f_* e(\rho \otimes (m \eps + \nu_f)) \in MU^{2m(k-1)-2qk}(BG \times X). \qed\]
\end{lemma}

Set $G = \Z/k$.  The bundle $\rho$ splits as $\bigoplus_{i=1}^{k-1} \eta^i$ for $\eta$ the line bundle coming from an inclusion $\Z/k \into U(1)$.  Then, if $\L$ is any other line bundle with a trivial $G$--action,
\begin{align*}
...
\end{align*}
where $v = e(\eta)$, $a_j(T) \in C\llbracket T \rrbracket$ for $C$ the subring of $MU^*(*)$ generated by the FGL, and \[w = e(\rho) = (k-1)! v^{k-1} + \sum_{j \ge k} b_j v^j\] for $b_j \in C$.  In general, $e(\rho \otimes E)$ has \[e(\rho \otimes E) = \sum_{|\alpha| \le r} w^{r-|\alpha|} a(v)^\alpha c_\alpha(E).\]

\begin{corollary}\citeme{Quillen Prop 3.17}
In the notation above, the Steenrod operation and Landweber operations are related to each other by the formula \[w^{n+q} P x = \sum_{|\alpha| \le n} w^{n - |\alpha|} a(v)^\alpha s_\alpha(x)\] for $x \in MU^{-2q}(X)$ and $n$ is any integer sufficiently large with respect to $\dim X$ and $q$. \qed
\end{corollary}
