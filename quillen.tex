...
\begin{lemma}
The Serre spectral sequence \[E_2^{*, *} = H^*(S^{2n+1}; E^* BU(n)) \Rightarrow E^* BU(n+1)\] associated to the coset fibration \[BU(n) \to BU(n+1) S^{2n+1}\] degenerates at $E_2$. \qed
\end{lemma}
...
\todo{Say something about what the splitting principle buys you, and how divisors can be either totally disconnected or clumped together in non-algebraically-closed settings.}

\todo{Give some properties of the Chern classes, like Whitney sum is sent to the product of the Chern polynomials.  This will reinforce the name ``divisor'' and it will be useful in the section on operations.}
...
Our job today is to define these two kinds of cohomology operations.
...
power operations in $MU$--cohomology.  remark that quadratic power operations in ordinary cohomology give the usual Steenrod operations.
...
riemann-roch formulas and relating the two kinds of operations

quillen's main theorem
...
landweber--novikov operations, their basic properties
...
The map $X \to BU(n)$ induces a map $X_E \to BU(n)_E \cong \Div_n^+ \CP^\infty_E$, classifying a rank $n$ divisor on $\CP^\infty_E$ base-changed to $X_E$.  this subscheme is selected by the ideal sheaf determined by the total Chern class, but it's also modeled topologically by the projectivization construction.
