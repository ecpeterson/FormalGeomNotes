...
\end{tikzcd}
\end{center}
For any identification \(P \simeq \coprod_K (* \mmod K)\),\footnote{This decomposition in terms of subgroups of \(G'\) is \emph{not} canonical.} there is a \index{push-pull}push-pull interchange formula
\[\pushQED{\qed}
...
\begin{definition}[{\cite[Section XXV.6, Section XXI.4]{MayAlaskaNotes}, \cite[Example 6.1.6.22, Definition 6.1.6.24]{LurieHA}}]
Again let \(F\) be an \(G\)--spectrum, and define its \index{equivariant spectrum!homotopy fixed points}\textit{homotopy fixed points} \(F^{hG}\) to be the homotopy limit spectrum.  It is possible to factor the norm map \(F_{hG} \to F\) of \Cref{EquivariantDefns} through the homotopy fixed points, as in \[F_{hG} \to F^{hG} \to F.\]  The cofiber of this first map is denoted \(F^{tG}\) and is called the \index{Tate construction}\textit{Tate spectrum} of \(G\).  In turn, this gives rise to a notion of Tate power operation via
\begin{center}
