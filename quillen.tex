...
Let $E$ be a complex-orientable theory and let $V \to X$ be a complex vector bundle over a space $X$.  According to \Cref{ThomIsomOverC}, the cohomology of the Thom space $E^* T(V)$ forms a $1$--dimensional $E^* X$--module.  Using \Cref{CorrespondenceQCohAndModules}, we construct a line bundle over $X_E$ \[\ThomSheaf{V} := \widetilde{E^* T(V)},\] called the \textit{Thom sheaf} of $V$.
\end{definition}
...
\todo{Various people have been uncomfortable about whether the grading matters here, whether $E$ is periodified, \ldots .}
