...
Finally, we can also consider the tensoring structure maps \[\otimes: BU(n) \times BU(m) \to BU(nm).\]  In the case $n = m = 1$, this is the map \[\CP^\infty \times \CP^\infty \to \CP^\infty\] classifying the tensor product of line bundles that we've been using to imbue $\CP^\infty_E$ with the structure of a formal group.  A direct consequence of this identification and the splitting principle yields the following:\todo{I think arguing that this map exists purely on the level of formal schemes --- that it's not somehow a consequence of the formal group $\CP^\infty_E$ coming from topology --- requires knowing Weierstrass preparation, so that we can extract a divisor polynomial from the potentially infinite power series coming from just hitting the individual divisor polynomials with $+_!$.}
\begin{lemma}
The map \[\Div_n^+ \CP^\infty_E \times \Div_m^+ \CP^\infty_E \to \Div_{nm}^+ \CP^\infty_E\] is the convolution of divisors, using the formal group structure on $\CP^\infty_E$.  On a pair of totally decomposed divisors $\bigsqcup_{i=1}^n [a_i]$ and $\bigsqcup_{j=1}^m [b_j]$, the map acts by \[\left(\bigsqcup_{i=1}^n [a_i] \right) \left( \bigsqcup_{j=1}^m [b_j] \right) = \bigsqcup_{i, j} [a_i +_{\CP^\infty_E} b_j]. \qed\]
\end{lemma}
