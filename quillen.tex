...
We define a \textit{nilpotent thickening}\textit{nilpotent thickening}\index{scheme!nilpotent thickening} to be a closed inclusion of schemes whose associated ideal sheaf is nilpotent.  An \(S\)--point \(s\co S \to X\) of a Noetherian scheme is formally smooth exactly when \(T_s X\) is a free \(R\)--module and for any nilpotent thickenings \(S \to \Spec B \to \Spec B'\) and any solid diagram
\begin{center}
...
(\Susp^\infty_+ BU(1))^{\sm n} \arrow{r} \arrow[bend right=15, "\L^{\times n}"' near start]{rr} \arrow["c_1^{\sm n}"]{d} & \Susp^\infty_+ BU(1)^{\sm n}_{hC_n} \arrow["\L(n)"]{r} \arrow[crossing over]{d} \arrow["P^{C_n}_{\mathrm{ext}}" near end, crossing over]{rd} & \Susp^\infty_+ BU(n) \arrow["c_n"]{d} \\
...
We now turn from the \(1\)\textsuperscript{st} universal case to our original goal: understanding the action of \(P^{C_n}\) on each of the canonical classes \[u_m\co MU(m) \to MU.\]  We approach this via the splitting principle by first rewriting the formula in \Cref{AjAndBjAreInTheFGLSubring} in a form amenable to wrangling direct sums with the Cartan formula:
\begin{align*}
...
\end{corollary}
\begin{proof}
Writing \(\phi\) for the canonical Thom isomorphism associated to \(MU\)--cohomology, this follows from compatibility of \(\phi\) with the \(MU\)--power operations (as in the construction given in \Cref{QuillenPowerOpnsSection}), the splitting principle, multiplicativity of power operations, and application of the properties of Landweber--Novikov operations:
\begin{align*}
P^{C_n}(u_m) & = P^{C_n}(s_m(\phi(1))) = P^{C_n}(\phi(c_m(\xi_m))) = \phi(P^{C_n}(c_m(\xi_m))) \\
& = \phi(P^{C_n}(c_1(\L_1) \cdots c_1(\L_m))) = \phi(P^{C_n}(c_1(\L_1)) \cdots P^{C_n}(c_1(\L_m))) \\
...
\item \(\eta\co BC_n \to BU(1)\) is the line bundle associated to the inclusion \(C_n \le U(1)\), and \(t = e(\eta)\) is its Euler class.
...
\end{lemma}
\begin{proof}[Proof sketch]
Consider the following piece of the Gysin sequence for the bundle \(\eta\) as viewed in the cohomology theory \(MU^*(X \times -)\): \[\widetilde{MU}^{*+1}(X \times S^1_{hC_n}) \xrightarrow{\partial} MU^*(X \times BC_n) \xrightarrow{t} MU^{*+2}(X \times BC_n).\]  Because the action of \(C_n\) on the total space \(S^1\) is free, we have \(S^1_{hC_n} \simeq S^1\).  The suspension isomorphism then rewrites the leftmost term as \[\widetilde{MU}^*(X) \xrightarrow{\<n\>_{MU}(t)} MU^*(X \times BC_n) \xrightarrow{t} MU^{*+2}(X \times BC_n).\]  In order to justify the image of \(\partial\), one needs to unwrap the definition of the Gysin sequence~\cite[Equation 4.7, Proposition 4.2]{Quillen}, which we elect not to do.  Given this, however, the Lemma statement follows from exactness at the top-left node.
\end{proof}
