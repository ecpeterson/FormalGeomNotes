...
\todo{You might want to re-segment these sections.  There is enough to say about constructing (and stabilizing) power operations that maybe that deserves its own section, then a discussion of Landweber-Novikov stable operations and Quillen's specific formula, then the discussion of $MU_*$ itself.}
...
\begin{lemma}
\citeme{Lurie must show that the monoidal structure on $\CatOf{Spectra}$ is symmetric}
...
\end{tikzcd}
\end{center}
A ring spectrum $E$ equipped with the indicated factorizations $\mu_n$ of the multiplication map which are furthermore compatible with taking products and iterated powers is called an \textit{$H_\infty$ ring spectrum}.
...
\citeme{where are $H_\infty$ ring spectra defined, BMMS?}
...
\begin{lemma}[{\cite[Corollary 1.6]{BMMS},~\cite[Proposition A.5, Equation 3.6]{AHSHinfty}}]
...
\begin{remark}
Since the failure of the power operations to be additive was a consequence of the binomial formula, it is somewhat intuitive that modulo $2$, where $(x + y)^2 \equiv x^2 + y^2$, that this operation becomes stable.  In fact, more than this is true: if an $H_\infty$ ring spectrum $E$ has $\pi_0 = \F_p$, it must be the case that $E = H\F_p$.
\end{remark}
