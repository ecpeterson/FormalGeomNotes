...
\end{align*}
where \[w = e(\rho) = (k-1)! v^{k-1} + \sum_{j \ge k} b_j v^j\] for $b_j \in C$.  In general, the splitting principle shows that $e(\rho \otimes E)$ has \[e(\rho \otimes E) = \sum_{|\alpha| \le r} w^{r-|\alpha|} a(v)^\alpha c_\alpha(E).\]  Setting $r = \dim(m \eps + \nu_f)$, we insert this into \Cref{PowerOpAndEulerClasses} to deduce
\begin{align*}
w^m P(f_* 1) & = f_* \left( \sum_{|\alpha| \le r} w^{r - |\alpha|} a(v)^\alpha c_\alpha(m \eps + \nu_f) \right) \\
& = \sum_{|\alpha| \le r} w^{r - |\alpha|} a(v)^\alpha f_* c_\alpha(m \eps + \nu_f)
\end{align*}
\todo{I get lost here. The goal is to get $c_\alpha(\nu_f)$ on its own, so that we can use $f_* c_\alpha(\nu_f) = s_\alpha(f_* 1)$, but\ldots}
\end{proof}
