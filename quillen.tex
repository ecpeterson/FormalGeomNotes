...
Applying the Lemma to the universal case produces a factorization \[MU(k)^{\sm n} \to MU(k)^{\sm n}_{hC_n} \to MU(nk)\] of the unstable multiplication map, and hence a stable factorization \[MU^{\sm n} \to MU^{\sm n}_{hC_n} \xrightarrow{\mu_n} MU.\]  In fact, this enriches to an $H_\infty^2$ structure: by applying \Cref{ThomSpectrumOfAssocBundle} in reverse (and harmlessly trading $C_n$ for $\Sigma_n$), the basic construction is
\begin{align*}
