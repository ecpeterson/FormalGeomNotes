...
\begin{align*}
f(x) dx = \omega & = \tau_y^* \omega = f(x +_\phi y) d(x +_\phi y). \\
\intertext{Writing $F(x) = \int f(x) dx$ for its formal integral, we use the multivariate chain rule to extend this equation to}
\frac{\partial F(x)}{\partial x} dx = f(x) dx = \omega & = \tau_y^* \omega = f(x +_\phi y) d(x +_\phi y) = \frac{\partial F(x +_\phi y)}{\partial x} dx.
\end{align*}
...
For $R$ any ring, there is an isomorphism $\omega_{\G} \cong T_0^* \G$.
\end{lemma}
\begin{proof}
...
\begin{example}
The Tate spectrum is quite a destructive operation in the case of $G = C_p$.\footnote{In fact, its job in general is to be destructive, and the failure of this Example for other groups is a sign that we have chosen the ``wrong'' definition. For a general group $G$, the definition of ``Tate construction'' can be modified to kill all intermediate transfers, not just those which transfer up from the trivial subgroup---but this requires understanding \textit{families} of subgroups, which is more equivariant homotopy theory than we otherwise need.}  For example, take $X$ to be a trivial $C_p$--spectrum, and let $\bigvee_{C_p} X$ be the $C_p$--indexed wedge with the free action.  Because the norm map composed with the restriction map encodes multiplication by $p$, this forces the norm map itself to be an equivalence, and hence the cofiber seuqnece \[\left(\bigvee_{C_p} X \right)_{hC_p} \xrightarrow{\simeq} \left( \bigvee_{C_p} X \right)^{hC_p} \to \left( \bigvee_{C_p} X \right)^{tC_p}\] causes $(\bigvee_{C_p} X)^{tC_p}$ to be contractible.
\end{example}
...
\begin{center}
\begin{tikzcd}
X^{\times n} \arrow["\xi^{\oplus n}"]{r} \arrow{d} & BU(k)^{\times n} \arrow["\oplus"]{r} & BU(nk) \arrow[leftarrow, "\xi(n)"' near end]{lld} \\
X^{\times n}_{hC_n} \arrow["\xi^{\oplus n}_{hC_n}"']{r} & BU(k)^{\times n}_{hC_n}. \arrow[densely dotted]{ru}[description]{\widetilde \mu_n} \arrow[crossing over, leftarrow]{u}
\end{tikzcd}
\end{center}
