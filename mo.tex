...
\begin{definition}
An \textit{affine formal scheme} is an ind-system of finite\todo{How are finite schemes characterized in the functor of points perspective?} affine schemes.  \todo{Several people had questions about the utility of this. Does it just add certain colimits to the category of \emph{finite} affine schemes?}  The morphisms between such schemes are computed by \[\CatOf{FormalSchemes}(\{X_\alpha\}, \{Y_\beta\}) = \lim_\alpha \colim_\beta \CatOf{Schemes}(X_\alpha, Y_\beta).\]\todo{Question: can we reverse the order of taking lim and colim here?-danny ; inserted answer+explanation in the source, AY}
\end{definition}
%answer to danny's question, could be added as a remark
%For any category $\mathcal{C}$, one can form the category $\CatOf{Ind}(\mathcal{C})$ of ind-systems in $\mathcal{C}$ which formally adjoins colimits of filtered diagrams.  For example, this produces the category of groups from the category of finitely presented groups.  It is instructive to make $\mathbb{Q}$ a category via ordering and note that $\CatOf{Ind}(\mathbb{Q}) = \mathbb{R}$ and see what goes wrong when you switch the limit and the colimit in the definition of morphism.  
