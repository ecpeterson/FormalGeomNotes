...
\end{tikzcd}
\end{center}
This gives an equivalence $\theta_{MO}\co MO \sm T(\xi) \rightarrow MO \sm \Susp^\infty_+ X$.  To introduce $E$, note that there is a diagram of $MO$--module spectra
\begin{center}
\begin{tikzcd}
E \sm T(\xi) \arrow{d}{\eta_{MO} \sm \id \sm \id=f} \arrow[densely dotted]{r}{\theta_E} & E \sm \Susp^\infty_+ X \arrow{d}{\eta_{MO} \sm \id \sm \id} \\
...
E \sm T(\xi) \arrow[densely dotted]{r}{\theta_E} & E \sm \Susp^\infty_+ X
\end{tikzcd}
\end{center}
whose columns are retractions.  The dotted arrows $\theta_E$ are defined by following the solid path from corner to corner, and the retraction property shows that the two squares they form commute.  Since $\theta_{MO}$ is an equivalence, it has an inverse $\alpha_{MO}$, and this similarly begets a map $\alpha_E$, which we now want to show is the inverse to $\theta_E$.  This, too, follows from the fact that the vertical edges are retractions.
\end{proof}
