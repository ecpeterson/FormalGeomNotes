...
Here is where we could divert to talking about the construction of the Adams spectral sequence.\todo{You could actually do this. The unstable Adams spectral sequence is also most easily defined using a really similar construction. Just a thought.}  There is a light presentation which we could think about now, but it will fit nicely into a story later on.  So, we will leave this task for later in the semester, when we can do a better job of it.  For now, we will record the following utility lemma about the Adams spectral sequence, which we will need to use before we get around to giving a good construction:
\begin{lemma}
...
\todo{Dylan says that ``cotensor of comodules over Hopf algebroids agrees with global sections of tensor product of sheaves''.  Maybe you can introduce this now, or maybe it belongs in the opening bits of Finite Spectra.}
