...
Our goal in this section is to use our results so far to make a calculation of \(\pi_* MO\), the unoriented bordism ring.  Our approach is the same as in the examples at the end of the previous section: we will want to use the Adams spectral sequence of signature \[H^*_{\mathrm{gp}}(\InternalAut_1(\G_a); \HFtwo P_0(MO)\widetilde{\:}) \Rightarrow \pi_* MO,\] where \(\HFtwo P_0(MO)\widetilde{\:}\) denotes the associated quasicoherent sheaf over \(\Spec k \mmod \Spec A_*\) as in \Cref{TrustMeAboutQCoh}---and we will therefore want to understand \(\HFtwo P_0(MO)\) as a comodule for the dual Steenrod algebra.
