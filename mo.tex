...
\begin{definition}
An \textit{affine formal scheme} is an ind-system of finite\todo{How are finite schemes characterized in the functor of points perspective?} affine schemes.  \todo{Several people had questions about the utility of this. Does it just add certain colimits to the category of \emph{finite} affine schemes?}  The morphisms between such schemes are computed by \[\CatOf{FormalSchemes}(\{X_\alpha\}, \{Y_\beta\}) = \lim_\beta \colim_\alpha \CatOf{Schemes}(X_\alpha, Y_\beta).\]\todo{Question: can we reverse the order of taking lim and colim here?-danny ; inserted answer+explanation in the source, AY}
\end{definition}
