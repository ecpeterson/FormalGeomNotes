...
A graded ring \(A\) is equivalent data to an affine scheme \(\Spec A\) with an \index{group scheme!multiplicative group, \(\Gm\)!action}action by \(\mathbb G_m\).  Additionally, a map \(A \to B\) respects the grading exactly when the induced map \(\Spec B \to \Spec B\) is \(\mathbb G_m\)--equivariant.
\end{lemma}
\begin{proof}
