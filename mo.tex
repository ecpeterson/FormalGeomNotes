...
\end{align*}
This powers an induction that shows $H\F_2{}_* \RP^\infty$ has a single class in every degree.  The cohomological version of the Thom isomorphism\todo{This could use a reference or a remark or something.  Is there a Mahowaldian version of the cohomological Thom isomorphism?}, together with the $H\F_2^* \RP^n$--module structure of $H\F_2^* T(\L-1)$, also gives the ring structure: \[H\F_2^* \RP^n = \F_2[x] / x^{n+1}.\]
\end{example}
...
\begin{definition}
When $X$ is an infinite complex, filter it by its subskeleta $X^{(n)}$ and define $X_{H\F_2}$ to be the ind-system $\{X^{(n)}_{H\F_2}\}_{n=0}^\infty$ of finite schemes.\footnote{More canonically, when $X$ is ``compactly generated'', it can be written as the colimit of its compact subspaces $X^{(\alpha)}$, and we define $X_{H\F_2}$ using the ind-system $\{X^{(\alpha)}_{H\F_2}\}_\alpha$.  Any system arising from a choice of skeletal structure is cofinal in this system, hence ind-isomorphic.}
\end{definition}
...
\begin{definition}[{\cite[Definition 1.1]{HoveyMoritaThy}, \cite[Definition 2.42]{StricklandFSFG}}]
...
\begin{lemma}[{\cite[Proposition 2.47]{StricklandFSFG}}]
...
\begin{example}
At the other extreme, we can pick the extremely nondegenerate case $X = \S$, where $\InternalAut_1 \G_a$ acts maximally nonfreely on $\Spec k$.  The resulting spectral sequence is pictured through a range in \Cref{HF2ASSFigure}.
\end{example}
...
\caption{The $H\F_2$--Adams spectral sequence for $kO$, which collapses at the second page.  North and north-east lines denote multiplication by $2$ and by $\eta$.}
\end{figure}
\begin{figure}[b]
...
Our goal in this section is to use the previous results of the Case Study to make a calculation of $\pi_* MO$, the unoriented bordism ring.  This is an extension of the examples at the end of the previous section: we will want to use the Adams spectral sequence of signature \[H^*_{\mathrm{gp}}(\InternalAut_1(\G_a); \widetilde{H\F_2P_0(MO)}) \Rightarrow \pi_* MO,\] which necessitates first understanding $H\F_2P_0(MO)$ as a comodule structure for the dual Steenrod algebra.
...
The natural map \[\Sym \widetilde{H\F_2P}_0(BO(1)) \to H\F_2P_0(BO).\] induces an isomorphism of Hopf algebras and of comodules for the dual Steenrod algebra \[\Sym H\F_2P_0(BO(1)) \xrightarrow{\simeq} H\F_2P_0(BO).\]
\end{lemma}
\begin{proof}
This follows from a combination of various standard facts about Stiefel--Whitney classes.  First, these classes generate the cohomology ring $H\F_2^* BO(n)$: \[H\F_2^* BO(n) \cong \F_2\ps{w_1, \ldots, w_n}.\]  Second, the total Stiefel--Whitney class is exponential, in the sense that $w(V \oplus W) = w(V) \cdot w(W)$.  From this, it follows that the natural map \[H\F_2^* BO(n) \xrightarrow{\bigoplus_{j=1}^n \L_j} H\F_2^* BO(1)^{\times n} \cong (H\F_2^* BO(1))^{\otimes n}\] is the inclusion of the symmetric polynomials, by calculating the total Stiefel--Whitney class \[w\left( \bigoplus_{j=1}^n \L_j \right) = \prod_{j=1}^n (1 + w_1(\L_j)) = \sum_{j=0}^n \sigma_j(w_1(\L_1), \ldots, w_1(\L_n)) t^j.\]  Dually, the homological map \[(H\F_{2*} BO(1))^{\otimes n} \to H\F_{2*} BO(n)\] is surjective, modeling the quotient from the tensor product to the symmetric tensor product.  Stabilizing as $n \to \infty$, we recover the statement of the Lemma.
\end{proof}

With this in hand, we can uncover the ring structure on $H\F_2P_0(MO)$:
\begin{corollary}\label{HF2MOisFree}
\todo{There's some confusion in these propositions about whether we're taking the symmetric algebra on the reduced homology module or what.  This really matters: it has something to do with the difference between any old $H\F_2P^0(MO(1))$ class and one which factors the ring spectrum unit. --- In fact, here you might mean $\Susp MO(1)$, the actual Thom \emph{space} associated to $BO(1)$.}
There is an analogous isomorphism \[\Sym H\F_2P_0(\Susp MO(1)) \xrightarrow{\simeq} H\F_2P_0(MO).\]  In particular, $H\F_2P_0(MO) \cong \F_2[b_1, b_2, \ldots]$.
\end{corollary}
\begin{proof}
...
\end{corollary}
\begin{proof}
Given a ring map $MO \to H\F_2P$, we can restrict it along the inclusion $MO(1) \to MO$ to produce a particular cohomology class \[f \in H\F_2P^0(MO(1)) = \CatOf{Modules}_{\F_2}(H\F_2P_0(MO(1)), \F_2).\]  No two distinct maps $MO \to H\F_2P$ restrict to the same map $MO(1) \to MO \to H\F_2P$ --- this is the content of \Cref{DetectingMORingMapsInHomotopy} and \Cref{HF2MOisFree}.  In the other direction, a cohomology class $H\F_2P^0 MO(1)$ factoring the unit can be reinterpreted as a module map $\widetilde{H\F_2P}_0(MO(1)) \to \F_2$ satisfying \todo{XXXXX}.  We can then employ \Cref{HF2MOisFree} to produce an algebra map $H\F_2P_0(MO) \to \F_2$, and \Cref{DetectingMORingMapsInHomotopy} then gives a ring spectrum map $MO \to H\F_2P$.
\end{proof}

\begin{corollary}
There is an $\InternalAut(\G_a)$--equivariant isomorphism of schemes \[\Spec H\F_2P_0(MO) \cong \Coord_1(\RP^\infty_{H\F_2P}),\] where the latter is the subscheme of functions $\RP^\infty_{H\F_2P} \to \A^1$ which restrict to the canonical identification of tangent spaces $\RP^1_{H\F_2P} = \A^{1,(1)}$ (i.e., the subscheme of functions which are \emph{isomorphisms} of formal schemes).
\end{corollary}
\begin{proof}
The method of the previous proof is to exhibit a isomorphism between these two schemes: the elements of $H\F_2P_0 MO$ serve as functions $\Spec H\F_2P_0 MO \to \A^1$ selecting the Taylor coefficients of the coordinate.  Since the comodule map \[H\F_2P_0 MO(1) \to \Sym H\F_2P_0 MO(1) \cong H\F_2P_0 MO\] selects the algebra generators, and the action of the group scheme $\InternalAut \G_a$ on $\Spec H\F_2P^0 BO(1)$ is by precomposition, it follows that the action on $\Spec H\F_2P_0 MO$ can also be interpreted as acting by precomposition on the algebraic model $\Coord_1(\RP^\infty_{H\F_2P})$.
\end{proof}
...
The action of $\InternalAut_1(\G_a)$ on $\Coord_1(\G_a)$ is \emph{free}: \[\Coord_1(\G_a) \cong \Spec \F_2[b_j \mid j \ne 2^k - 1] \times \InternalAut_1(\G_a).\]
\end{theorem}
\begin{proof}
