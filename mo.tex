...
\todo{Mention FPFP's Polarised Schemes (Sections 11 and 14) in a footnote.}
...
\todo{This doesn't connect back with the first Adams SS.}
\end{example}
...
\todo{Another thing that might belong here is the deformation picture: $H\F_2{}_* H\Z$ is also computable using the same methods, and the change-of-rings theorem then says that the Adams spectral sequence computing $H\Z_* X$ from $H\F_2{}_* X$ is controlled by $\Ext$ over the exterior algebra generated by that one Bockstein.  (This might also belong much later, say, when we talk about the Morava $K$--theory cooperations or if we ever describe $H\F_2{}_* BP$.)}
