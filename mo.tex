...
\end{tikzcd}
\end{center}
whose columns are retractions.  The dotted arrows $\theta_E$ are defined by following the solid path from corner to corner.  We claim that the two trapezoids so formed both commute.  To see this, consider the repeated diagram
\begin{center}
\begin{tikzcd}
& MO \sm E \sm T(\xi) \arrow{r}{\theta_{MO} \sm 1} \arrow{dl}[description]{(\mu \circ (\phi \sm 1)) \sm 1} & MO \sm E \sm \Susp^\infty_+ X\arrow{dr}[description]{(\mu \circ (\phi \sm 1)) \sm 1} \\
E \sm T(\xi) \arrow{dr}[description]{\eta_{MO} \sm 1 \sm 1} \arrow[bend right,equal]{dd} & & & E \sm \Susp^\infty_+ X \arrow{dl}[description]{\eta_{MO} \sm 1 \sm 1} \arrow[bend left,equal]{dd} \\
& MO \sm E \sm T(\xi) \arrow{r}{\theta_{MO} \sm 1} \arrow{dl}[description]{(\mu \circ (\phi \sm 1)) \sm 1} & MO \sm E \sm \Susp^\infty_+ X\arrow{dr}[description]{(\mu \circ (\phi \sm 1)) \sm 1} \\
E \sm T(\xi) \arrow[densely dotted]{rrr}{\theta_E} & & & E \sm \Susp^\infty_+ X,
\end{tikzcd}
\end{center}
where the hexagon commutes because it expresses that the trace of the identity on an $MO \sm E$--module is an isomorphism invariant.  Using this, we follow the chains
\begin{align*}
& \quad [MO \sm E \sm T(\xi) \to E \sm T(\xi) \to E \sm \Susp^\infty_+ X] \\
& \equiv [MO \sm E \sm T(\xi) \to E \sm T(\xi) \to MO \sm E \sm T(\xi) \\
& \quad \quad \to MO \sm E \sm \Susp^\infty_+ X \to E \sm \Susp^\infty_+ X] \\
& \equiv [MO \sm E \sm T(\xi) \to MO \sm E \sm \Susp^\infty_+ X \to E \sm \Susp^\infty_+ X \\
& \quad \quad \to MO \sm E \sm \Susp^\infty_+ X \to E \sm \Susp^\infty_+ X] \\
& \equiv [MO \sm E \sm T(\xi) \to MO \sm E \sm \Susp^\infty_+ X \to E \sm \Susp^\infty_+ X = E \sm \Susp^\infty_+ X] \\
& \equiv [MO \sm E \sm T(\xi) \to MO \sm E \sm \Susp^\infty_+ X \to E \sm \Susp^\infty_+ X],
\end{align*}
which yields one of the two trapezoids.  The other is similar.

Since $\theta_{MO}$ is an equivalence, it has an inverse $\alpha_{MO}$, and this similarly begets a map $\alpha_E$, which participates in its own pair of trapezoids.  Stacking them yields the identity
\begin{align*}
& \quad [E \sm T(\xi) \xrightarrow{\theta_E} E \sm \Susp^\infty_+ X \xrightarrow{\alpha_E} E \sm T(\xi)] \\
& \equiv [E \sm T(\xi) \to E \sm \Susp^\infty_+ X \to MO \sm E \sm \Susp^\infty_+ X \\
& \quad \quad \to MO \sm E \sm T(\xi) \to E \sm T(\xi)] \\
& \equiv [E \sm T(\xi) \to MO \sm E \sm T(\xi) \to MO \sm E \sm \Susp^\infty_+ X \\
& \quad \quad \to MO \sm E \sm T(\xi) \to E \sm T(\xi)] \\
& \equiv [E \sm T(\xi) \to MO \sm E \sm T(\xi) \to E \sm T(\xi)] \\
& \equiv [E \sm T(\xi) = E \sm T(\xi)].
\end{align*}
Showing that $\alpha_E$ is the left-inverse of $\theta_E$ is also similar.
\end{proof}
...
\end{align*}
The functor $\mathbb A^n$ is called \index{scheme!affine $n$--space}\textit{affine $n$--space}---reasonable, since the value $\mathbb A^n(T)$ is isomorphic to $T^n$.  In particular, we refer to $\mathbb A^1$ as the \textit{affine line}.  Note that the quotient map $R_1 \to R_1^{(j)}$ induces an inclusion $\mathbb A^{1, (j)} \to \mathbb A^1$ and that $\mathbb A^{1, (0)}$ is a constant functor: \[\mathbb A^{1, (0)}(T) = \{f: \F_2[x] \to T \mid f(x) = 0\}.\]  Accordingly, we declare ``$\mathbb A^{1, (0)}$'' to be the \textit{origin on the affine line} and $\mathbb A^{1, (j)}$ to be the \textit{$(n+1)$\textsuperscript{st} order (nilpotent) neighborhood of the origin} in the affine line.
\end{example}
