...
Today we will construct an important classical tool: the Adams spectral sequence.  There are many ways to introduce this tool, but our approach will be to focus on how to get information out of the Hurewicz homomorphism.  \todo{I first saw this presentation from Matt Ando. He must have learned it from someone. I'd like to know who to attribute this to.}Begin by considering the following three self-maps of the stable sphere:
\begin{align*}
...
\end{tikzcd}
\end{center}
where the vertical line indicates the nontrivial coaction involving $\xi_1$.  We can now see what trading maps for cofiber sequences has bought us: mod--$2$ cohomology can distinguish the defining sequences for $C(0)$ and $C(2)$ by considering their induced extensions of comodules over $\mathcal A_*$.  The Adams spectral sequence, which we will presently construct, says that this process is in a certain sense \emph{exhaustive}.  It has the signature \[\Ext_{\mathcal A_*}^{*, *}(\F_2, \F_2) \Rightarrow (\pi_* \S^0)^\wedge_2,\] i.e., the $2$--primary component of the maps $\Susp^* \S^0 \to \S^0$ can be detected as a subquotient of all possible extensions of comodules over the dual Steenrod algebra.

To construct the spectral sequence, we will write down the first thing that comes to mind involving the Hurewicz homomorphism.  Our fuss above can be interpreted as trying to distinguish elements which are in the kernel of the Hurewicz image.  This kernel has a topological incarnation in the fiber sequence: \[\overline{H\F_2} \to \S \to H\F_2.\]  It, too, carries a Hurewicz fiber sequence \[\overline{H\F_2}^{\sm 2} \to \overline{H\F_2} \to H\F_2 \sm \overline{H\F_2}\] which we can hope to use to further distinguish elements.  Iterating this process yields a filtration:
\begin{center}
\begin{tikzcd}
\S \arrow{d} & \overline{H\F_2} \arrow{l} \arrow{d} & \overline{H\F_2}^{\sm 2} \arrow{l} \arrow{d} & \overline{H\F_2}^{\sm 3} \arrow{l} \arrow{d} & \cdots \arrow{l} \\
H\F_2 & H\F_2 \sm \overline{H\F_2} & H\F_2 \sm \overline{H\F_2}^{\sm 2} & H\F_2 \sm \overline{H\F_2}^{\sm 3} & \cdots
\end{tikzcd}
\end{center}
and hence a spectral sequence by applying $\pi_*$.

\begin{example}
Let's consider what happens to the element $2 \in \pi_0 \S$ considered above.  It lies in the kernel of the map to $\pi_0 H\F_2$, so it lifts (uniquely) to an element $\overline 2 \in \pi_0 \overline{H\F_2}$.  The map $\overline{H\F_2} \to H\F_2 \sm \overline{H\F_2}$ is nontrivial on $\pi_0$, and since $\overline 2$ generates $\pi_0 \overline{H\F_2}$ it follows that $\overline 2$ must have nontrivial image in $\pi_0 (H\F_2 \sm \overline{H\F_2})$.\todo{Yuck, this line splitting.}  We can re-express this target using the cofiber sequence \[H\F_2 \sm \S \xrightarrow{\eta_R} H\F_2 \sm H\F_2 \xrightarrow{/\eta_R} \Susp H\F_2 \sm \overline{H\F_2},\] so that $\pi_0 (H\F_2 \sm \overline{H\F_2})$ is isomorphic to $\pi_1(H\F_2 \sm H\F_2) \cong \mathcal A_1$.  The only nontrivial element for $\overline 2$ to be sent to is $\xi_1$ --- not coincidentally, the element appearing in our extension.\todo{I wonder if this can be phrased more tightly than ``oh, look, that's probably not a coincidence.''}
\end{example}

\todo{Also include a discussion of when $X$ is a spectrum other than the sphere.}

\begin{theorem}
If $X$ is a spectrum with finitely many homotopy groups, all finite, then this spectral sequence converges to $\pi_* X$. \qed \todo{Prove this.}
\end{theorem}

\begin{theorem}
If $X$ is any spectrum that is bounded below, then this spectral sequence converges to the $2$--completion $\pi_* X^\wedge_2$. \qed \todo{Maybe don't prove this? I don't know how to deal with $\Z$--towers, actually. At least cite it, anyway.}
\end{theorem}

\begin{theorem}
The Hurewicz homomorphism is detected by the $0$--line. \qed \todo{Prove this. Maybe compare the Adams tower with the constant tower at $\S$.}
\end{theorem}

What remains to do is to identify the information in this spectral sequence with the story that we began the hour with.  In order to do this, we will have to investigate a little homological algebra.  We will talk about this next time.
...
Our goal for today is to get a feel for what the input to the Adams spectral sequence is.  To remind you, here's the resolution we were thinking about last time:
\begin{center}
\begin{tikzcd}
\S \arrow{d} & \overline{H\F_2} \arrow{l} \arrow{d} & \overline{H\F_2}^{\sm 2} \arrow{l} \arrow{d} & \overline{H\F_2}^{\sm 3} \arrow{l} \arrow{d} & \cdots \arrow{l} \\
H\F_2 & H\F_2 \sm \overline{H\F_2} & H\F_2 \sm \overline{H\F_2}^{\sm 2} & H\F_2 \sm \overline{H\F_2}^{\sm 3} & \cdots.
\end{tikzcd}
\end{center}
Applying $\pi_*$ and considering the bottom row, this gives a complex computing the $E_2$--page: \[\F_2 \to \overline{\mathcal A_*} \to \overline{\mathcal A_*} \otimes \overline{\mathcal A_*} \to \cdots,\] where $\overline{\mathcal A_*}$ denotes the $\mathcal A_*$--comodule $\mathcal A_* / \{1\}$.\todo{What are the maps in this complex? Probably we want to identify them with the $\Delta$s below.}  We would like to interpret this as a resolution computing some sort of derived functor.  This is a common pattern: many constructions in ``homotopical algebra'' come with spectral sequences comparing its homotopy with an analogous construction in homological algebra.

We know that we're in search of a theory of extensions of comodules for $\mathcal A_*$, so we'll begin by inventing homological algebra for comodules over a Hopf algebra.  In the usual development of homological algebra for modules, the key observations are the existence of projective and injective modules, and something similar is at work here.
...
The Corollary constructs a resolution of the trivial $\mathcal A_*$--comodule $\F_2$:\todo{It could be made clearer why these maps come out to be this way. In particular, $\coker \overline \Delta$ requires a nontrivial iso. Literally $\overline \Delta$ kills $1 \otimes \xi_n + \cdots$ with a nontrivial tail, and that we're allowed to ignore the tail is a theorem.}
\begin{center}
\begin{tikzcd}
\tiny{0} \arrow{r} & \F_2 \arrow{r} \arrow{d}{\eta} & \tiny{0} \\
\tiny{0} \arrow{r} & \mathcal A_* \arrow{r}{\overline \Delta} & \mathcal A_* \otimes \overline{\mathcal A_*} \arrow{r} & \mathcal A_* \otimes \mathcal A_* \otimes \mathcal A_* \arrow{r} & \cdots \\
& & \overline{\mathcal A_*} \arrow{u} \arrow[leftarrow]{lu}{\coker \eta} & \overline{\mathcal A_*} \otimes \overline{\mathcal A_*} \arrow{u} \arrow[leftarrow]{lu}{\coker \overline \Delta} .
\end{tikzcd}
\end{center}

The Adams resolution is a resolution by injectives

Push and pull functors.

The $E_2$ page the Adams spectral sequence is a $\Cotor$. Ravenel A1.1.6.b says $\CatOf{Comodules}_{(A, \Gamma)}(M, N) = \CatOf{Modules}_A(M, A) \cotensor_\Gamma N$, so $\CatOf{Comodules}_\Gamma(A, N) = A \cotensor_\Gamma N$.
...
Example: the cohomology of an induced module is trivial

Idea: the Adams $E_2$--page is like group cohomology for a group scheme

\todo{In fact, the functions on a dihedral group gives $\mathcal A(1)_*$.}

Models for $\Cotor$: (i) the cobar complex, (ii) Yoneda extensions in very good cases

A picture of the Adams spectral sequence, for good measure
...
\section{The structure of $\moduli{fg}$: large scales}

Rational logarithms

\begin{theorem}[Lazard, part 1]
The ring of functions on $\moduli{fgl}$ is polynomial in infinitely many variables. \qed \todo{Should we prove this?}
\end{theorem}
\begin{remark}
...
\section{The structure of $\moduli{fg}$: small scales}
