...
examples: trivial bundles give $T(S^n \times X) \simeq S^{n+1} \sm (X_+)$, and $T(\L; BO(1)) = BO(1)$, with subexamples $T(\L; \RP^n) = \RP^{n+1}$

identification of spherical bundles with maps to $BGL_1 \hAut S^{n-1}$, Thom space as a functor off of this slice category

the $J$--homomorphism as a map $BO(n) \to BGL_1 \hAut S^{n-1}$. compatibility with sums, and especially the block maps $BO(n) \to BO(n+1)$

definition of a normalized Thom spectrum, the spectra $MO(n)$, and the limiting spectrum $MO$
...
using the splitting principle to describe $\Div^+_n \CP^\infty_E = BU(n)_E$, and in particular that projectivization theorem.
