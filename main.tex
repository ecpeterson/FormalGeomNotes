...
\end{tikzcd}
\end{center}
Applying $\pi_*$ and considering the bottom row, this gives a complex computing the $E_2$--page: \[\F_2 \to \overline{\mathcal A_*} \to \overline{\mathcal A_*} \otimes \overline{\mathcal A_*} \to \cdots,\] where $\overline{\mathcal A_*}$ denotes the $\mathcal A_*$--comodule $\mathcal A_* / \{1\}$.  We would like to interpret this as a resolution computing some sort of derived functor.  This is a common pattern: many constructions in ``homotopical algebra'' come with spectral sequences comparing its homotopy with an analogous construction in homological algebra.
...
Using the Corollary, we can construct a resolution of the trivial $\mathcal A_*$--comodule $\F_2$:
\begin{center}
\begin{tikzcd}
...
Example: the Hopf algebra of functions on a group
