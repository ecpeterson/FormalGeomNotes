...
\begin{itemize}
\item Given $r \in R$, we can define a \textit{homothety} by rescaling 
...
\begin{theorem}[{\cite[Th\'eor\`eme IV]{Lazard}}]
...
Recall that the moduli scheme of $p$--typical formal group laws is presented as \[\moduli{fgl}^{\ptyp} = \Spec \Z_{(p)}[v_1, v_2, \ldots, v_d, \ldots].\]  Suppose $g(x) = \sum_{j=0}^\infty t_j x^{p^j}$ is a coordinate transformation which conjugates the universal group law on to a second $p$--typical group law with coefficients $\eta_R(v_j)$.  Modulo $p$, there is the relation:
...
Let $+_\phi$ be a formal group law over a ring $R$ and let $R[h]/h^2 \to R$ be the square--zero extension with kernel $(h)$.
\begin{enumerate}
\item Automorphisms of $+_\phi$ over $R[h]/h^2$ correspond to elements in $\widehat Z^1(\phi)$.
\item Extensions of $+_\phi$ to $R[h]/h^2$ correspond to elements in $\widehat Z^2(\phi)$.
\item Two such extensions are isomorphic as formal group laws over $R[h]/h^2$ if their cocycles differ by an element in $\widehat B^2(\phi)$. \qed
\end{enumerate}
\end{theorem}
...
This is quasi-isomorphic to the usual bar complex by the map
...
\begin{lemma}
Let $\G$ be a formal group of finite height $d$ over a field $k$.  Then, $H^2(\G; M \otimes \G_a)$ is a free $k$--vector space of dimension $(d - 1)$.
\end{lemma}
\begin{proof}
Select a coordinate on $\G$ of the form \[x +_\phi y = x + y + (\mathrm{unit}) c_{p^d} + \cdots.\]  Then, filter $\G$ by degree and consider the resulting spectral sequence of signature \[H^*(\G_a; M \otimes \G_a) \Rightarrow H^*(\G; M \otimes \G_a).\]  To compute the differentials in this spectral sequence, one computes by hand the formula for the differential in the bar complex, working up to lowest visible degree.  For example: \[(x +_\phi y)^{p^r} - (x^{p^r} + y^{p^r}) = (\text{unit}) \cdot c_{p^{d + r}}(x, y) + \cdots,\] where we used $c_{p^d}^{p^r} = c_{p^{r+d}}$.  So, we see that there are at least $d - 1$ things at the bottom of the spectral sequence which are not coboundaries, and we need to check that they are indeed permanent cocycles.  To do this, we need only show that they are realized by deformations, which Lubin and Tate accomplish in \Cref{LubinTateRealizationLemma}.
\end{proof}
