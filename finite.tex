...
\todo{In either this section, near the definition of chromatic homology theories, or in the next section, where the acyclicity of $K(d) \sm K(d')$ is stated, we should discuss $\HFp_* BP$ (perhaps in terms of $\HFp_* MU$ but certainly in terms of a quotient algebra of the dual Steenrod algebra), as well as the utility of the Adams spectral sequence in computing extraordinary homology theories from ordinary ones, and the role that Bocksteins play in those calculations (and, in particular, where all the $\tau_j$ operators went in the deformation from $\HFp$ to $BP$).}
