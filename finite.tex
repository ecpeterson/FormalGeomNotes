...
\begin{example}
The unit-speed invariant differential associated to the usual coordinate on the muliplicative group has the form \[\omega(x) = (1 + x + x^2 + \cdots + x^{p-1} + \cdots) dx.\]  The radius of convergence of this ordinary differential equation in $\Z_{(p)}$ is $p$, owing to the term \[\int x^{p-1} dx = \frac{x^p}{p} \not\in \Z_{(p)}[x].\]  We thus see that $\G_m$ has height $\log_p(p) = 1$.
\end{example}
...
\begin{itemize}
\item Given $r \in R$, we can define a \index{homothety}\textit{homothety} by rescaling the coordinate by $r$: \[\log(\theta_r g_0(x)) = \log(g_0(rx)) = \sum_{n=1}^\infty (a_n r^n) x^n.\]
