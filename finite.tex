...
Appealing to B\'ezout's lemma, select elements \(t_n \in \sheaf O_{\moduli{fgl}}\) such that \(Qu(t_n)\) generates \(T_{2n}\), i.e., so that \(Qu(t_n) = d_{n+1} b_n\).  Consider the induced map \(v\) as in \[\Z[t_n \mid n \ge 1] \xrightarrow{v} \sheaf O_{\moduli{fgl}} \xrightarrow{u} U,\] which is designed so that the composite satisfies \(Quv(t_n) = d_{n+1} b_n\).  This formula shows that \(Quv\) is injective, and because the source and target are free, \(uv\) is itself injective.  Thus, so is \(v\).  Additionally, \Cref{LazardSplittingLemma} below shows that \(Q(v(uv))\) is surjective, but \(Qvu\) is the identity, so \(Qv\) and hence \(v\) are surjective.
\end{proof}

We have yet to prove the following Lemma:

\begin{lemma}\label{LazardSplittingLemma}
There is a canonical retraction \(T_{2n} \to (Q \mathcal O_{\moduli{fgl}})_{2n}\) of \(Qu\).
\end{lemma}
