...
\item (Needy:) What conditions can be placed on $f$ so that the category of descent data for any given module is always contractible?\todo{Change these silly N phrases.}
\end{itemize}
...
talk about quasicoherent sheaves over affine schemes
...
\begin{lemma}
When $E$ is an $A_\infty$--ring spectrum, the descent object $X^\wedge_E$ can be naturally considered cosimplicial object in the $\infty$--category of spectra. \qed \todo{I don't intend to prove this, but maybe we could say some mealy words about why it's true. At worst, we could give reference to the relevant part of Higher Algebra.}
\end{lemma}
\begin{definition}
...
I think you can make sense of ``Cartesian'' directly on $\sheaf D_E(X)$, and that such sheaves should be Cartesian over $\sheaf D_E$.  You'd then want to remark that $\pi_*$ preserves Cartesian-ness exactly when just that one morphism is Cartesian; then everything else follows.  That's a pretty cool result, all told.
...
\begin{center}
\begin{tikzcd}
\Spec R \arrow{r}{i} & \moduli{fgl} \arrow[-,double]{r} \arrow{d} & \context{MU}[0] \arrow{d} \arrow[-,double]{r} & \Spec MU_* \\
& \moduli{fg} \arrow[-,double]{r} \arrow[leftarrow]{lu}{\mathrm{flat}} & \context{MU}
\end{tikzcd}
\end{center}
...
\end{itemize}
This will occupy us for the next two lectures.  Today, we will embark on this analysis by studying the scheme $\moduli{fgl}$ which naturally covers $\moduli{fg}$.

\todo{The rest of this lecture is written with respect to logical order rather than with respect to narrative order. The narrative could use a lot of beefing up.}
...
This presentation of $\moduli{fgl}$ as a subscheme appears to be extremely complicated, but $\moduli{fgl}$ itself is actually not complicated at all.  We will prove the following theorem:
\begin{theorem}\label{LazardsTheorem}
There is a noncanonical isomorphism \[\sheaf{O}_{\moduli{fgl}} \cong \Z[t_n \mid 1 \le n < \infty].\]
\end{theorem}

The most important consequence that we can draw from this is \emph{smoothness}, stated as follows:
\begin{corollary}
...
Let $U = \Z[b_0, b_1, b_2, \ldots] / (b_0 - 1)$ be the universal ring supporting an exponential \[\exp(x) := \sum_{j=0}^\infty b_j x^{j+1}\] with compositional inverse \[\log(x) := \sum_{j=0}^\infty m_j x^{j+1}.\]  They induce a formal group law on $U$ by the formula \[x +_U y = \exp(\log(x) + \log(y)),\] classified by a map $u: L \to U$.  Modulo decomposables, this map can be computed as \[u(a_{n(n-i)}) = \binom{n}{i} b_{n-1} \pmod{I^2}.\]  Equivalently stated, writing $d_n = \gcd\left( \binom{n}{i} \middle| 0 < k < n \right)$, the map $Qu$ on degree $2n$ is the subgroup generated by $d_{n+1} b_n$.  We \emph{choose} a splitting from \Cref{Symmetric2CocycleLemma}.4 below and the freeness of $U$ to construct an algebra splitting \[U \xrightarrow{v} L \xrightarrow{u} U.\]  The map $v$ is an isomorphism because $uv$ is injective and because we have checked that $v$ is surjective on indecomposables.\todo{Why do the intermediate rings matter? It must matter just in the tail of this proof.}
\end{proof}
...
\end{enumerate}
\end{lemma}
\begin{proof}[Notes on proof]\todo{Actually prove these reductions?}
...
\todo{Separate this into a calculation of $\Ext(\G_a, \G_a)$ and the separate symmetric $2$--cocycle lemma.}

\begin{proof}[Proof of the symmetric cocycle lemma]\todo{Rework this.}\citeme{This follows Chapter 3 of COCTALOS.}
It suffices to show the lemma over a finitely generated ring.  In fact, the lemma is true for $A \oplus B$ if and only if it's true for $A$ and for $B$, so the structure theorem for finitely generated abelian groups reduces to the cases of $\Z$ and $\Z/p^r$.  If $A \subseteq B$ and the lemma is true for $B$, it's true for $A$, so we can reduce the $\Z$ case to $\Q$.\todo{Rephrase this in terms of localizations.}  We can also reduce from $\Z/p^r$ to $\Z/p$ using an inductive Bockstein type argument.  In particular, we can now freely assume that our ground object is a field.

For any formal group scheme $\G$, we can form a simplicial scheme $B\G$ in the usual way:
\[B\G := \left\{
\begin{tikzcd}[ampersand replacement=\&]
...
\begin{remark}
\todo[inline]{Maybe we can say something about $H\Z \sm MU$ here, since that's the ring used in the proof of Lazard's theorem.  Also, people seem to say things about the Mischenko logarithm rather than the invariant differential, but I think we should phrase things in those terms.  Also also, Section 12 of Neil's FG notes talk about the infinite height subscheme of $\moduli{fgl}$. He compares it to $H_* MO$ and to the Hurewicz image $\pi_* MU \to H_*(MU; \F_p)$.}
\end{remark}

\needproof{$MU_* MU$ is universal for FGL isomorphisms}

\begin{corollary}\todo{Explain what $\moduli{ps}$ is.}
...
Of course, not every formal group law supports a logarithm --- after all, this is the point of ``height''.  We therefore should an intermediate definition that applies to all formal group laws and which specializes to the one above in the presence of a logarithm.  To do this, we consider what computations are made easier with this sort of formula for a logarithm, and we arrive at the following:
...
\begin{lemma}\label{EveryFGLIsPTypical}\citeme{This is due to Hazewinkel}
Every formal group law is uniquely isomorphic to one a $p$--typical formal group law, called its $p$--typification.
\end{lemma}
\begin{proof}
...
\textbf{Now analyze the open sets.}
...
\begin{remark}
\todo[inline]{It's worth pointing out how strange this is. In Euclidean geometry, open subspaces are always top-dimensional, and closed subspaces can drop dimension.}
\end{remark}
...
Let $F(x, y)$ be a formal group law over $R$; a deformation of $F$ is a formal group law $F_h(x, y)$ over $R\ps{h}$ reducing to $F$ when $h = 0$; an isomorphism of deformations is a strict isomorphism covering the identity on $h = 0$.

Define the deformation complex $C^*(F)$ by \[R \to R\ps{x_1} \to R\ps{x_1, x_2} \to R\ps{x_1, x_2, x_3} \to \cdots\] with differential
\begin{align*}
...
Write a deformed automorphism $f$ of $+_\phi$ as $f(x) = f_0(x) + h f_1(x) + h^2 f_2(x) + \cdots$, so that \[f(x +_\phi y) = f(x) +_\phi f(y).\]  Applying $\left.\frac{\partial}{\partial h}\right|_{h=0}$ to this equality gives \[f_1(x +_\phi y) = \phi_1(x, y)f_1(x) + \phi_2(x, y)f_1(y)\] and thus $f_1$ is a $1$--cocycle in the deformation complex.
...
% \item In particular, $BP_*$ is itself such a module, and so gives rise to a homology theory $BP$ with $\context{BP} \simeq \moduli{fg} \times \Spec \Z_{(p)}$.
