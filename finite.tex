...
Appealing to \Cref{LazardSplittingLemma} below, select elements \(t_n \in \sheaf O_{\moduli{fgl}}\) so that \[t_n \equiv r(d_{n+1} b_n) \pmod{\text{decomposables}},\] and consider the induced map \[\Z[t_n \mid n \ge 1] \xrightarrow{v} \sheaf O_{\moduli{fgl}} \xrightarrow{u} U.\]  \Cref{LazardSplittingLemma} below shows that \(Q(r(uv))\) is surjective, but \(Qru\) is the identity, so \(Qv\) and hence \(v\) are surjective.  Additionally, because \[Q(uv)_{2n}\co \Z\{t_n\} \to \Z\{d_{n+1} b_n\}\] is a surjective map of free \(\Z\)--modules, it is also injective.  It follows that the map \(uv\) of free \(\Z\)--algebras is injective, and hence \(v\) itself is injective.
\end{proof}
