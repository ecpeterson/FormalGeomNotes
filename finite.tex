...
\begin{remark}
\todo{Compute the $p$--typification of the standard formal group law on $\G_m$.  You can use this to define the Artin--Haase exponential, which arises as the coordinate change from the $p$--typical group law (by its logarithm to $\G_a$) to the standard group law (by its exponential from $\G_a$).}
\end{remark}
