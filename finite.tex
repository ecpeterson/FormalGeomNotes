...
\begin{remark}[{\cite[Equation 20.1]{HopkinsCOCTALOS}}]
...
\todo{Section 24 of FPFP has some facts about the behavior of the Morava stabilizer on Lubin--Tate space.  It could be nice to include this, perhaps even with proofs.}
\todo{Another fun fact is that because $p$--local finite spectra $F$ are $\bigvee_{d < \infty} K(d)$--local, there are no nontrivial maps $\HFp \to F$ (and, in particular, the Spanier--Whitehead dual of $\HFp$ is null).  (In fact, the Hovey paper \textit{Bousfield Localization Functors and Hopkins' Chromatic Splitting Conjecture} shows that they are local for any properly infinite collection of Morava $K$--theories.)}
...
\todo{Tobi had a cute argument: the cofiber of $\bigvee_n K(n) \to \prod_n K(n)$ is concentrated in degree $0$ (hence Eilenberg--Mac Lane) because each nonzero homotopy degree receives contributions from only finitely many $K(n)$ spectra.  I'm not sure what this is good for, but it's a neat fact.  (It's important to work with the minimally periodic spectra here.)}
