...
\item Writing \(j_{D+1}\co \moduli{fg}^{\ge D+1} \to \moduli{fg}\) for the inclusion of the prime closed substack, we identify \(\sheaf F / I_{D+1}\) with \(j_{D+1}{}_* j_{D+1}^* \sheaf F\) and use this to restrict both sides: \[\Tor_{D+1}(\sheaf F/I_{D+1}, \sheaf N) = \Tor_{D+1}(j_{D+1}{}_* j_{D+1}^* \sheaf F, \sheaf N) = \Tor_{D+1}(j_{D+1}^* \sheaf F, j_{D+1}^* \sheaf N).\]  If \(\sheaf N = \context{MU}(X)\) is a \emph{finite} complex \(X\), then \(\sheaf N\) is coherent and hence \(j_{D+1}^* \sheaf N\) is free for large \(D\).  Its \(\Tor\) groups thus vanish.
\end{itemize}
