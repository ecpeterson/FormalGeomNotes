...
Appealing to \Cref{LazardSplittingLemma} below, select elements \(t_n \in \sheaf O_{\moduli{fgl}}\) so that \[t_n \equiv r(d_{n+1} b_n) \pmod{\text{decomposables}},\] and hence so that \(Qu(t_n) = d_{n+1} b_n\) generates \(T_{2n}\).  Consider the induced map \[\Z[t_n \mid n \ge 1] \xrightarrow{v} \sheaf O_{\moduli{fgl}} \xrightarrow{u} U,\] which is designed so that the composite satisfies \(Quv(t_n) = d_{n+1} b_n\).  This formula shows that \(Quv\) is injective, and because the source and target are free, \(uv\) is itself injective.  Thus, so is \(v\).  Additionally, \Cref{LazardSplittingLemma} below shows that \(Q(r(uv))\) is surjective, but \(Qru\) is the identity, so \(Qv\) and hence \(v\) are surjective.
\end{proof}
