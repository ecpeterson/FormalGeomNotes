...
\end{theorem}
\begin{proof}
Since \(\CatOf C_d\) are nested by \Cref{CdCategoriesNest} and they form an exhaustive filtration (i.e., \(\CatOf C_\infty\) = 0), it is thus sufficient to show that any object \(X \in \CatOf C\) with \(X \in \CatOf C_d\) induces an inclusion \(\CatOf C_d \subseteq \CatOf C\).  Write \(R\) for the endomorphism ring spectrum \(R = F(X, X)\), and write \(F\) for the fiber of its unit map: \[F \xrightarrow{f} \S \xrightarrow{\eta_R} R.\]  Finally, let \(Y \in \CatOf C_d\) be \emph{any} finite spectrum of type at least \(d\).  Our goal is to demonstrate \(Y \in \CatOf C\).  
