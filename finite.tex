...
\todo[inline]{Make sure you come to grips with ind-coherent sheaves!}
...
The essential structure of these answers is easy to guess if we proceed by example, using the few tools available to us.  Suppose that we begin instead with an $R$--module $M$ and we set $N = M \otimes_R S$.  By tensoring up, we have two $R$--algebra maps $S \to S \otimes_R S$, given by including along either factor, and we can further tensor $N$ up to $N \otimes_R S$ or $S \otimes_R N$.  Since $N$ came from the $R$--module $M$, these are canonically isomorphic: $((f \otimes 1) \circ f)^* M = ((1 \otimes f) \circ f)^* M$.  Repeating this process produces more isomorphisms which compose according to the triangle
\begin{center}
\begin{tikzcd}
N \otimes_R S \otimes_R S \arrow{rr}{\simeq} \arrow{rd}{\simeq} & & S \otimes_R S \otimes_R N \\
& S \otimes_R N \otimes_R S \arrow{ru}{\simeq}.
\end{tikzcd}
\end{center}

\begin{definition}
Let $f\co R \to S$ be a map of rings as above.  An $S$--module $N$ equipped with an isomorphism $S \otimes_R N \cong N \otimes_R S$ which causes the above triangle to commute is called a \textit{descent datum} for the map $f$.
\end{definition}
...
talk about how equivalence of groupoids induces equivalence of sheaf categories.  also, a deeper natural equivalence: stackification and locality.

\todo{I suspect you'll have time to move the ``upcoming program'' sketch scheduled for the beginning of the next lecture into the end of this one.  I think that's better narration anyhow.}
...
\todo{Should sections have subsections?  Does that help organize the TOC?}
...
Every formal group law is naturally isomorphic to one a $p$--typical formal group law, called its $p$--typification.
\end{lemma}
\begin{proof}
...
Our first goal for today is to capitalize on our analysis of $\moduli{fg}$ to construct some of the homology theories alluded to in \Cref{StructureOfMfgl}.
