...
\begin{definition}
In the setting of a coordinate on a formal group over a torsion-free ring, the following conditions are equivalent:
\begin{itemize}
...
\item The coordinate is in the image of the \(p\)--typicalizing idempotent \(\eps\).
\end{itemize}
The second and third conditions \emph{do not} require the torsion-free assumption, and so we use the second condition to define a closed subscheme \(\moduli{fgl}^{\ptyp} \subseteq \moduli{fgl}\), the \textit{moduli of \(p\)--typical formal group laws}.
\end{definition}
